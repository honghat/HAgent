...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.hagent/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.hagent/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.hagent/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.hagent/logs/curator/``)
    ...
    authored skill data in ``~/.hagent/skills/``.
    ...
    that bypasses both.
    """
    root = get_hagent_home() / "logs" / "curator"
    try:
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.hagent/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.hagent/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.hagent/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.hagent/skills/.archive/` and are recoverable by `mv`")
