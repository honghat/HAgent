...
Uses a file-based lock (~/.hagent/cron/.tick.lock) so only one tick
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within HAGENT_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against HAGENT_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    scripts_dir = _get_hagent_home() / "scripts"
    ...
    # escape — scripts MUST reside within HAGENT_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        model = job.get("model") or os.getenv("HAGENT_MODEL") or ""
        ...
        agent = AIAgent(
            ...
            # HAGENT_HOME. When a workdir is configured, also inject project
        ...
        # override via HAGENT_CRON_TIMEOUT env var.  0 = unlimited.
        #
