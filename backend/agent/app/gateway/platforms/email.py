"""
Email platform adapter for the Hagent gateway.

Allows users to interact with Hagent by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "hagent-agent" "version" "{_hagent_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Hagent Agent")
        ...
        msg_id = f"<hagent-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
