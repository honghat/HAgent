...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.hagent/.env file
...
# =============================================================================
# Auth Store — persistence layer for ~/.hagent/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_hagent_home() / "auth.json"
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (get_hagent_home() / "auth.json").resolve(strict=False)
        try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.hagent/auth.json`` even when HAGENT_HOME is set to a profile
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".hagent" / "auth.json"
            try:
...
#
# Tokens live in ~/.hagent/auth/google_oauth.json (managed by agent.google_oauth).
...
# Spotify auth — PKCE tokens stored in ~/.hagent/auth.json
# =============================================================================
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.hagent/.env, and return it.
    ...
    print()
    print("Saved HAGENT_SPOTIFY_CLIENT_ID to ~/.hagent/.env")
    print()
    return raw
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.hagent/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hagent auth store (~/.hagent/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Hagent auth store (~/.hagent/auth.json)."""
...
# ``get_default_hagent_root()`` returns — ``~/.hagent`` on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    Linux/macOS classic installs land at ``~/.hagent/shared/``, native
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.hagent/auth.json."""
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hagent auth store (~/.hagent/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
