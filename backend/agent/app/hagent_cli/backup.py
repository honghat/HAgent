...
`hagent backup` creates a zip archive of the entire ~/.hagent/ directory
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.hagent/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a hagent dir name
        if prefix in (".hagent", "hagent"):
            return prefix + "/"
