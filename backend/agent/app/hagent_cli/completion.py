"""Shell completion script generation for hagent CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_hagent_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Hagent Agent bash completion
# Add to ~/.bashrc:
#   eval "$(hagent completion bash)"
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef hagent
# Hagent Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(hagent completion zsh)"
...
_hagent() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_hagent_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'hagent command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Hagent Agent fish completion",
        "# Add to your config:",
        "#   hagent completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hagent_profiles",
        "    echo default",
        "    if test -d $HOME/.hagent/profiles",
        "        ls $HOME/.hagent/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c hagent -f -s p -l profile"
        " -d 'Profile name' -xa '(__hagent_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__hagent_profiles)' -d 'Profile name'"
                )
