...
def _sync_hagent_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_hagent_home` falls back to ``/root/.hagent`` — the wrong
...
from hagent_constants import is_container, is_termux, is_wsl
...
def _profile_arg(hagent_home: str | None = None) -> str:
    ...
    For ``~/.hagent/profiles/<name>``, returns ``"--profile <name>"``.
...
def get_service_name() -> str:
    ...
    Default ``~/.hagent`` returns ``hagent-gateway`` (backward compatible).
    Profile ``~/.hagent/profiles/coder`` returns ``hagent-gateway-coder``.
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.hagent`` → ``ai.hagent.gateway.plist`` (backward compatible).
    Profile ``~/.hagent/profiles/coder`` → ``ai.hagent.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.hagent/hagent-agent  -> /home/alice/.hagent/hagent-agent
...
def _hagent_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      /root/.hagent                    → /home/alice/.hagent
      /root/.hagent/profiles/coder     → /home/alice/.hagent/profiles/coder
    ...
    target_default = Path(target_home_dir) / ".hagent"

    # Default ~/.hagent → remap to target user's default
    if current_hagent == current_default:
        return str(target_default)

    # Profile or subdir of ~/.hagent → preserve the relative structure
    try:
        relative = current_hagent.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.hagent) — keep as-is
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_hagent_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.hagent/plugins/ still require
...
def _setup_weixin():
    ...
    print_info("  3. Hagent will store the returned account_id/token in ~/.hagent/.env.")
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.hagent/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.hagent/.env: {', '.join(required)}")
    else:
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print("  nohup hagent gateway run > ~/.hagent/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup hagent gateway run > ~/.hagent/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hagent 'hagent gateway run'         # persistent via tmux")
                    print("  nohup hagent gateway run > ~/.hagent/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
