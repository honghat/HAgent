"""``hagent logs`` — view and filter Hagent log files.
...
Usage examples::
    ...
    hagent logs                    # last 50 lines of agent.log
    hagent logs -f                 # follow agent.log in real time
    hagent logs errors             # last 50 lines of errors.log
    hagent logs gateway -n 100    # last 100 lines of gateway.log
    hagent logs --level WARNING    # only WARNING+ lines
    hagent logs --session abc123   # filter by session ID substring
    hagent logs --component tools  # only tool-related lines
    hagent logs --since 1h         # lines from the last hour
    hagent logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_hagent_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Hagent runs — try 'hagent chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_hagent_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_hagent_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_hagent_home()}/logs/")
        return

    print(f"Log files in {display_hagent_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'hagent chat' to generate logs)")
