...
- ``hagent_cli.providers``        -- canonical provider identity + overlays
- ``hagent_cli.model_normalize``  -- per-provider name formatting
...
_HAGENT_MODEL_WARNING = (
    "Nous Research Hagent 3 & 4 models are NOT agentic and are not designed "
    "for use with Hagent Agent. They lack the tool-calling capabilities "
...
def is_nous_hagent_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_HAGENT_NON_AGENTIC_RE.search(model_name))
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``hagent config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from hagent_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'hagent model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'hagent doctor' — config issues detected:"
        ...
        if not new_model:
            if pdef.base_url:
                from hagent_cli.runtime_provider import _auto_detect_local_model
    ...
    # Mirrors the same logic in hagent_cli.runtime_provider.resolve_runtime_provider;
    ...
    hagent_warn = _check_hagent_model_warning(new_model)
    if hagent_warn:
        warnings.append(hagent_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from hagent_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from hagent_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by hagent provider ID
    ...
    # --- 1. Check Hagent-mapped providers ---
    for hagent_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(hagent_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (HAGENT_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from hagent_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and hagent_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Hagent release.
        model_ids = curated.get(hagent_id, [])
        if hagent_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(hagent_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Hagent-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hagent_cli.providers import HAGENT_OVERLAYS
    from hagent_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Hagent provider ID.
    # HAGENT_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Hagent IDs ("copilot").
    _mdev_to_hagent = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HAGENT_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Hagent slug — e.g. "github-copilot" → "copilot"
        hagent_slug = _mdev_to_hagent.get(pid, pid)
        if hagent_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(hagent_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, hagent_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or hagent_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(hagent_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", hagent_slug, exc)
        ...
        # configured.
        if not has_creds and hagent_slug == "anthropic":
            try:
                ...
                if (hagent_creds and hagent_creds.get("accessToken")) or \
        ...
        if hagent_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(hagent_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(hagent_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(hagent_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Hagent slug, fall back to overlay key
            model_ids = curated.get(hagent_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if hagent_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(hagent_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": hagent_slug,
            "name": get_label(hagent_slug),
            "is_current": hagent_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "hagent",
        ...
        seen_slugs.add(hagent_slug.lower())
        _record_builtin_endpoint(hagent_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or HAGENT_OVERLAYS (keeps /model in sync
    # with `hagent model`).
    try:
        from hagent_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Hagent's canonical write key (matches
            ...
            # Hagent writes ``models:`` as a dict keyed by model id
            # (see hagent_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Hagent's own writer uses; a hyphen variant is accepted
            ...
            # active model. Hagent's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, hagent_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`hagent_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from hagent_cli.models import fetch_openrouter_models
