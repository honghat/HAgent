...
on Japanese locales, etc.).  Hagent's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``hagent_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Hagent is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``HAGENT_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("HAGENT_DISABLE_WINDOWS_UTF8") in ("1", "true", "True", "yes"):
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Hagent-managed Git install directories so
    ...
    # already-running shells don't see the change — which means hagent
    ...
    # install without Hagent-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Hagent's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Hagent's
    ``hagent config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Hagent (User env var in Windows Settings, or
    export in a PowerShell profile) and Hagent picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Hagent-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\hagent\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``hagent`` from right after install) retain their old PATH.

    Any subprocess Hagent spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Hagent-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "hagent", "git", "cmd"),
        os.path.join(local_appdata, "hagent", "git", "bin"),
        os.path.join(local_appdata, "hagent", "git", "usr", "bin"),
        # Hagent venv Scripts directory — host of the hagent.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes hagent, but harmless to include.
        os.path.join(local_appdata, "hagent", "hagent-agent", "venv", "Scripts"),
