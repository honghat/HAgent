...
_PROJECT_HERMES_HOME = _AGENT_ROOT / "runtime"
...
def get_hermes_home() -> Path:
    ...
    Reads HERMES_HOME when explicitly set, otherwise falls back to
    ``backend/agent/runtime``. HAgent does not use ``~/.hermes`` as its default
    state directory.
    ...
    if val:
        res = Path(val)
        print(f"[DEBUG] get_hermes_home using ENV: {res}")
        return res
    res = _PROJECT_HERMES_HOME
    print(f"[DEBUG] get_hermes_home using PROJECT: {res}")
    return res
...
def get_default_hermes_root() -> Path:
    ...
    """
    native_home = _PROJECT_HERMES_HOME
