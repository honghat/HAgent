...
This module provides the glue:
    - ``HagentTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Hagent Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses HAGENT_HOME so each profile gets its own OAuth tokens.
    Layout: ``HAGENT_HOME/mcp-tokens/``
    """
    try:
        from hagent_constants import get_hagent_home
        base = Path(get_hagent_home())
    except ImportError:
        base = Path(os.environ.get("HAGENT_HOME", str(Path(__file__).resolve().parents[2] / "runtime")))
...
# ---------------------------------------------------------------------------
# HagentTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class HagentTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        HAGENT_HOME/mcp-tokens/<server_name>.json         -- tokens
        HAGENT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        HAGENT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Hagent records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Hagent.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Hagent Agent")
    scope = cfg.get("scope")
