"""disk_cleanup — ephemeral file cleanup for Hagent Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under HAGENT_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly HAGENT_HOME and /tmp/hagent-*
Never touches: ~/.hagent/logs/ or any system directory.
"""
...
try:
    ...
    def get_hagent_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("HAGENT_HOME") or "").strip()
        return Path(val).resolve() if val else (Path(__file__).resolve().parents[3] / "runtime").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$HAGENT_HOME/logs/``."""
    return get_hagent_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$HAGENT_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HAGENT_HOME or ``/tmp/hagent-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    hagent_home = get_hagent_home()
    try:
        path.resolve().relative_to(hagent_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/hagent-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("hagent-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside HAGENT_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under HAGENT_HOME (but leave HAGENT_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "hagent-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(hagent_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == hagent_home:
                continue
            try:
                rel_parts = dirpath.relative_to(hagent_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    hagent_home = get_hagent_home()
    try:
        rel = path.resolve().relative_to(hagent_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "hagent-agent",
        }:
            return None
        ...
        # Path isn't under HAGENT_HOME (e.g. /tmp/hagent-*) — fall through.
        pass
