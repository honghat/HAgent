...
Let the hagent agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where hagent runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `hagent meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `hagent meet` CLI |
...
| `cli.py` | `hagent meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$HAGENT_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `hagent meet node {run,list,approve,remove,status,ping}` |
...
hagent meet install                                      # pip + Chromium
hagent meet setup                                        # preflight
hagent meet auth                                         # optional
hagent meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
hagent meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hagent/.env
hagent meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
hagent meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
hagent meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.hagent/.env
hagent meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, hagent will **not** switch your system audio input automatically — the
...
hagent plugins enable google_meet
hagent meet node run --display-name my-mac --host 0.0.0.0 --port 18789
