...
```bash
hagent memory setup    # select "holographic"
```

Or manually:
```bash
hagent config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.hagent-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$HAGENT_HOME/memory_store.db` | SQLite database path |
