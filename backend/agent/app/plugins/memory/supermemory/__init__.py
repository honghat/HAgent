...
def _load_supermemory_config(hagent_home: str) -> dict:
    config = _default_config()
    config_path = Path(hagent_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, hagent_home: str) -> None:
    config_path = Path(hagent_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `hagent memory setup`.
        # All other options are documented for $HAGENT_HOME/supermemory.json
    ...
    def save_config(self, values, hagent_home):
        ...
        _save_supermemory_config(sanitized, hagent_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from hagent_constants import get_hagent_home
        self._hagent_home = kwargs.get("hagent_home") or str(get_hagent_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._hagent_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "hagent", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "hagent_memory", "target": target, "type": "explicit_memory"},
