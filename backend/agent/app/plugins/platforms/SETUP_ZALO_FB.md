...
```bash
# Xem logs:
tail -f ~/.hagent/logs/gateway-autostart-output.log | grep -i "zalo\|facebook"
...
- `~/.hagent/plugins/platforms/zalo/adapter.py` - Zalo implementation
- `~/.hagent/plugins/platforms/facebook/adapter.py` - FB Messenger implementation
...
# Xem logs:
tail -f ~/.hagent/logs/gateway-autostart-error.log
```
