...
# ``model_tools`` import time, so ``hagent status``, ``hagent chat``, etc.
...
# ``hagent_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Hagent is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a hagent MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${HAGENT_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from hagent_constants import get_hagent_home as _get_hagent_home
            _hagent_home = _get_hagent_home()
        except (ModuleNotFoundError, ImportError):
            _hagent_home = _Path(__file__).resolve().parents[4] / "runtime"
        self._thread_count_store = _ThreadCountStore(
            _hagent_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Hagent is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HAGENT_HOME", str(_Path(__file__).resolve().parents[4] / "runtime"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${HAGENT_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a hagent MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Hagent is thinking…' marker message.
        ...
        "Hagent is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Hagent is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Hagent is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Hagent is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Hagent is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``hagent setup``.

    The setup wizard at ``hagent_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.hagent/.env`` so the next
    gateway restart picks them up.
    """
    from hagent_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. hagent-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.hagent/.env")
    print_info("Restart the gateway: hagent gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``hagent cron`` running as a
    separate process from ``hagent gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Hagent plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'hagent-agent[google_chat]'",
        ...
        # the "Hagent is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Hagent is thinking…' marker message "
