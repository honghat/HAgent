"""
LINE Messaging API platform adapter for Hagent Agent.
...
to Hagent Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``HAGENT_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hagent_home = Path.home().joinpath(".hagent").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``hagent status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``hagent status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hagent setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.hagent/.env`` via ``hagent_cli.config``.
    """
    print()
    ...
    try:
        from hagent_cli.config import get_env_var, set_env_var
    except ImportError:
        print("hagent_cli.config not available; set LINE_* vars manually in ~/.hagent/.env")
        return
