...
~30 Hz (matching xterm key-repeat), summarizes ~/.hagent/perf.log on exit.
...
Environment overrides:
  HAGENT_PERF_LOG     (default ~/.hagent/perf.log)
...
try:
    ...
    def get_hagent_home() -> Path:  # type: ignore[misc]
        ...
        return Path(val) if val else Path(__file__).resolve().parents[1] / "runtime"
...
DEFAULT_STATE_DB = get_hagent_home() / "state.db"
