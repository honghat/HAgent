...
VERSION_FILE = REPO_ROOT / "hagent_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+hagent-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "hagent@nousresearch.com": "NousResearch",
    ...
    "hagent@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "hagent@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/hagent-agent",
    ...
    lines.append(f"# Hagent Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Hagent Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Hagent Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Hagent Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Hagent Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Hagent Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Hagent Agent v{new_version} ({calver_date})' "
