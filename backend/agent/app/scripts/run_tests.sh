...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.hagent/hagent-agent/venv"; do
...
# $HOME/.hagent/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.hagent/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.hagent:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.hagent" ;;
  esac
