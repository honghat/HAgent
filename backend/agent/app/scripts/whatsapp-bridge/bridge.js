 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.hagent/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.hagent', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.hagent', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.hagent', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.hagent', 'audio_cache');
