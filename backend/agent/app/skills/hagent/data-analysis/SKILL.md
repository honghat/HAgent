...
```bash
python backend/agent/app/skills/hagent/data-analysis/scripts/analyze.py \
...
python backend/agent/app/skills/hagent/data-analysis/scripts/analyze.py \
  ...
  --output-file workspace/outputs/monthly-trends.csv
```
...
- On first load, files are parsed and stored in a persistent DuckDB database under `workspace/.data-analysis-cache/`
