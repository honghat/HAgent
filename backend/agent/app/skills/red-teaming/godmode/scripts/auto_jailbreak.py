...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HAGENT_HOME", os.path.expanduser("~/.hagent")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HAGENT_SKILLS_DIR", Path.cwd())) / "red-teaming" / "godmode"
...
HAGENT_HOME = Path(os.getenv("HAGENT_HOME", str(_SKILL_DIR.parents[3] / "runtime")))
CONFIG_PATH = HAGENT_HOME / "config.yaml"
PREFILL_PATH = HAGENT_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Hagent models — already uncensored, just needs clean prompt
    "hagent": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "hagent" in model_lower or "nous" in model_lower:
        return "hagent"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Hagent config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.hagent/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Hagent for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Hagent for changes to take effect.")
