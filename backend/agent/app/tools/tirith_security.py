...
it is automatically downloaded from GitHub releases to $HAGENT_HOME/bin/tirith.
...
def _get_hagent_home() -> str:
    """Return the Hagent home directory, respecting HAGENT_HOME env var."""
    return str(get_hagent_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_hagent_home(), ".tirith-install-failed")
...
def _hagent_bin_dir() -> str:
    """Return $HAGENT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_hagent_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $HAGENT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_hagent_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $HAGENT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $HAGENT_HOME/bin/tirith
    ...
    hagent_bin = os.path.join(_hagent_bin_dir(), "tirith")
    if os.path.isfile(hagent_bin) and os.access(hagent_bin, os.X_OK):
        _resolved_path = hagent_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return hagent_bin
