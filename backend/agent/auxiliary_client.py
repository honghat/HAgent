...
Resolution order for text tasks (auto mode):
  ...
  3.gateway Hat Nguyen Portal (backend/auth.json active provider)
...
from hagent_constants import OPENROUTER_BASE_URL, get_hagent_home, get_token_file_path
...
def _extract_url_query_params(url: str):
    ...
    # Sanitize: strip non-printable chars that could come from corrupted config/env
    url = ''.join(c for c in str(url or '') if c.isprintable() or c in '/:-_.~?&=+#%@!$()*+,;')
    parsed = urlparse(url)
    if parsed.query:
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Hagent triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://hagent-agent.nousresearch.com",
    "X-Title": "Hagent Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``HAGENT_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``HAGENT_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("HAGENT_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("HAGENT_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
#gateway Hat Nguyen Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed bygateway Hat Nguyen Portal.
NOUS_EXTRA_BODY = {"tags": ["product=hagent-agent"]}

# Set at resolve time — True if the auxiliary client points togateway Hat Nguyen Portal
...
_AUTH_JSON_PATH = get_token_file_path("auth.json")
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Hagent Agent)",
...
def _to_openai_base_url(base_url: str) -> str:
    ...
    # Strip non-printable characters that could creep in from corrupted
    # config / env vars (e.g. trailing \n from shell injection)
    url = ''.join(c for c in url if c.isprintable() or c in '/:-_.~?&=+#%@!$()*+,;')
    url = url.rstrip('/')
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``hagent_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate backend/auth.json for an activeNous provider.

    Returns the provider state dict ifNous is active with tokens,
    otherwise None.
    """
    ...
    try:
        ...
        logger.debug("Could not readNous auth: %s", exc)
        return None


def _nous_api_key(provider: dict) -> str:
    """Extract the best API key from aNous provider state dict."""
    return provider.get("agent_key") or provider.get("access_token", "")


def _nous_base_url() -> str:
    """Resolve theNous inference base URL from env or default."""
    return os.getenv("NOUS_INFERENCE_BASE_URL", _NOUS_DEFAULT_BASE_URL)


def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return freshNous runtime credentials when available.
    ...
    try:
        ...
        logger.debug("AuxiliaryNous runtime credential resolution failed: %s", exc)
        return None
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Hagent auth store.
    ...
    try:
        from hagent_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from hagent_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from hagent_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attemptingNous —
    # if another session already recorded a 429, skipNous entirely
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skippinggateway Hat Nguyen Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            return None, None
    except Exception:
    ...
    logger.debug("Auxiliary client:gateway Hat Nguyen Portal")
    ...
    model = _NOUS_MODEL
    try:
        from hagent_cli.models import get_nous_recommended_aux_model
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `hagent setup` or `hagent model` and enter a valid http(s) base URL."
        ) from exc
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second hagent
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``hagent config aux reset``)."""
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `hagent model` but the
    #    old OPENAI_BASE_URL lingers in backend/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: hagent model to reconfigure, or remove "
                "OPENAI_BASE_URL from backend/.env",
...
def resolve_provider_client(
    ...
    # ──gateway Hat Nguyen Portal (OAuth) ──────────────────────────────────────────
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "butgateway Hat Nguyen Portal not configured (run: hagent auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: hagent model)")
                return None, None
    ...
    try:
        ...
        logger.debug("hagent_cli.auth not available for provider %s", provider)
        return None, None
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #   3.gateway Hat Nguyen Portal (vision-capable aggregator fallback)
        #   4. Stop
        main_provider = _read_main_provider()
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includesgateway Hat Nguyen Portal product tags when the auxiliary client is backed
    bygateway Hat Nguyen Portal. Returns empty dict otherwise.
    """
...
def _refresh_nous_auxiliary_client(
    *,
    ...
    """RefreshNous runtime creds, rebuild the client, and replace the cache entry."""
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=hagent-agent"])
    if merged_extra:
    ...
    try:
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshedNous runtime credentials after 401, retrying",
                            task or "call")
    ...
    *,
    ...
    try:
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshedNous runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
