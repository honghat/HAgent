...
Resolution order for text tasks (auto mode):
  ...
  3.gateway Hat Nguyen Portal (~/.hagent/auth.json active provider)
...
_AUTH_JSON_PATH = get_hagent_home() / "auth.json"
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.hagent/auth.json for an activeNous provider.
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    old OPENAI_BASE_URL lingers in ~/.hagent/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "OPENAI_BASE_URL from ~/.hagent/.env",
