"""Unified removal contract for every credential source Hagent reads from.

Hagent seeds its credential pool from many places:

    env:<VAR>     — os.environ / backend/.env
    claude_code   — ~/.claude/.credentials.json
    hagent_pkce   — HAGENT_HOME/tokens/anthropic_oauth.json
    ...
    manual        — user ran `hagent auth add`
...
unify here is **removal**:
    ...
    ``hagent auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), hagent_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in backend/.env  → clear it
    ...
    """
    from hagent_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in backend/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Hagent.",
            f"  The pool entry is now suppressed — Hagent will ignore "
            f"{env_var} until you run `hagent auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Hagent stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `hagent auth add anthropic` to re-enable if needed.",
    ])


def _remove_hagent_pkce(provider: str, removed) -> RemovalResult:
    """Hagent Anthropic OAuth file is ours — delete it outright."""
    from hagent_constants import get_hagent_home, get_token_file_path

    result = RemovalResult()
    oauth_files = {
        get_token_file_path("anthropic_oauth.json", ".anthropic_oauth.json"),
        get_hagent_home() / ".anthropic_oauth.json",
    }
    for oauth_file in oauth_files:
        ...
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Hagent Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `hagent login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `hagent auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Hagent auth store is not enough — _seed_from_singletons() would
    ...
    via ``hagent auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from hagent_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `hagent auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `hagent auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Hagent stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `hagent auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="hagent_pkce",
        remove_fn=_remove_hagent_pkce,
        description="HAGENT_HOME/tokens/anthropic_oauth.json",
    ))
    register(RemovalStep(
