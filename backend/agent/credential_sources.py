...
Hagent seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.hagent/.env
    claude_code   — ~/.claude/.credentials.json
    hagent_pkce   — ~/.hagent/.anthropic_oauth.json
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.hagent/.env  → clear it
...
def _remove_hagent_pkce(provider: str, removed) -> RemovalResult:
    """~/.hagent/.anthropic_oauth.json is ours — delete it outright."""
    from hagent_constants import get_hagent_home

    result = RemovalResult()
    oauth_file = get_hagent_home() / ".anthropic_oauth.json"
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.hagent/.anthropic_oauth.json",
    ))
    register(RemovalStep(
