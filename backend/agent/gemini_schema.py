...
# outside that subset before sending Hagent tool schemas to Google.
...
def sanitize_gemini_schema(schema: Any) -> Dict[str, Any]:
    ...
    Hagent tool schemas are OpenAI-flavored JSON Schema and may contain keys
    ...
    if isinstance(enum_val, list):
        if type_val in {"integer", "number", "boolean"}:
            if any(not isinstance(item, str) for item in enum_val):
                cleaned.pop("enum", None)
        # Gemini also rejects ``enum`` on ``array`` or ``object`` types.
        # Move ``enum`` inside ``items`` (for arrays) or drop it as a last resort.
        if type_val in {"array", "object"}:
            cleaned.pop("enum", None)
