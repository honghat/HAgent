...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - hagent_home (str): The active HAGENT_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``backend``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "hagent").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'hagent memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], hagent_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'hagent memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``hagent_home`` is the active HAGENT_HOME directory path.
