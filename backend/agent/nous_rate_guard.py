"""Cross-session rate limit guard forgateway Hat Nguyen Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whethergateway Hat Nguyen Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 fromNous triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Hagent retries), and every one of those calls counts
...
def _state_path() -> str:
    """Return the path to theNous rate limit state file."""
    try:
        from hagent_constants import get_hagent_home
        base = get_hagent_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".hagent")
...
def record_nous_rate_limit(
    *,
    ...
    """Record thatgateway Hat Nguyen Portal is rate-limited.
    ...
    try:
        ...
        logger.debug("Failed to writeNous rate limit state: %s", exc)


def nous_rate_limit_remaining() -> Optional[float]:
    """Check ifgateway Hat Nguyen Portal is currently rate-limited.
...
def clear_nous_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successfulNous request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clearNous rate limit state: %s", exc)
...
def is_genuine_nous_rate_limit(
    *,
    ...
    """Decide whether a 429 fromgateway Hat Nguyen Portal is a real account rate limit.

  gateway Hat Nguyen Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Hagent, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket onNous is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota onNous.

    Tripping the cross-session breaker on (b) blocks ALLNous requests
    (and all models, sinceNous is one provider key) for minutes even
