...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Hagent starts and finds ``~/.openclaw/``.

    Points users at ``hagent claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``hagent claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Hagent, run "
        "`hagent claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `hagent claw cleanup` (renames it to ~/.openclaw.pre-migration — "
