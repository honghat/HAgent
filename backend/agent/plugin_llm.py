...
Plugins built on Hagent Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Hagent
...
:class:`~hagent_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Hagent' main config
...
handles every provider, fallback chain, and per-task override Hagent
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`hagent_cli.plugins.PluginContext`
