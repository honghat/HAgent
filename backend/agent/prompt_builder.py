...
from hagent_constants import get_hagent_home, get_skills_dir, is_wsl
...
_HAGENT_MD_NAMES = (".hagent.md", "HAGENT.md")


def _find_hagent_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.hagent.md`` or ``HAGENT.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are HAgent, an intelligent AI assistant running inside the user's HAgent app. "
...
HAGENT_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using HAgent itself, "
    "answer for this local HAgent project. Do not direct them to Hagent Agent docs "
    "unless they explicitly ask about the upstream project."
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.hagent/kanban.db`. Your task id is in "
    "`$HAGENT_KANBAN_TASK`; your workspace is `$HAGENT_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $HAGENT_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `hagent kanban <verb>` for board operations. Use "
...
BASH_EXECUTION_GUIDANCE = (
    "# Execution sequence\n"
    "When you run a bash command or call any tool, you MUST follow this strict sequence every single time:\n"
    "1. **Run the command** — execute the bash/tool call\n"
    "2. **Wait for the actual result** — do not guess, assume, or simulate the output. "
    "The tool WILL return output; wait for it.\n"
    "3. **Read and analyze** the result you received\n"
    "4. **Inform the user** about the outcome based on the real result\n"
    "5. **Then, and only then**, decide if more steps are needed\n"
    "\n"
    "Each tool call is one complete cycle: execute → wait → read → inform → continue. "
    "Do NOT batch multiple commands silently and report at the end. "
    "Do NOT run a command, then immediately run another without showing the first result. "
    "The user needs to see what each step produced.\n"
    "\n"
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the HAgent WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Hagent itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Hagent runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Hagent itself is running. The host OS, home, and cwd "
                f"of the Hagent process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Hagent "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_hagent_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.hagent/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Hagent Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `hagent-agent` skill "
            "first. It has the actual commands (e.g. `hagent config set …`, `hagent tools`, "
            "`hagent setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from HAGENT_HOME and return its content, or None.
    ...
    """
    try:
        from hagent_cli.config import ensure_hagent_home
        ensure_hagent_home()
    except Exception as e:
        logger.debug("Could not ensure HAGENT_HOME before loading SOUL.md: %s", e)

    soul_path = get_hagent_home() / "SOUL.md"
...
def _load_hagent_md(cwd_path: Path) -> str:
    """.hagent.md / HAGENT.md — walk to git root."""
    hagent_md_path = _find_hagent_md(cwd_path)
    if not hagent_md_path:
        return ""
    try:
        content = hagent_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = hagent_md_path.name
        try:
            rel = str(hagent_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".hagent.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", hagent_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .hagent.md / HAGENT.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from HAGENT_HOME is independent and always included when present.
    ...
    # SOUL.md from HAGENT_HOME only — skip when already loaded as identity
    if not skip_soul:
