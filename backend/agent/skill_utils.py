...
from hagent_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``HAGENT_PLATFORM`` or
            ``HAGENT_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``hagent`` startup when ~120 skills
...
def get_project_skills_dir() -> Path:
    """Return the in-repo skills directory used by HAgent."""
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``backend/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``hagent`` cold-start time
    when the cache is absent.
    """
    project_skills = get_project_skills_dir().resolve()
    base_dirs = [project_skills] if project_skills.is_dir() else []
    config_path = get_config_path()
    if not config_path.exists():
        return base_dirs
    ...
    if not raw_dirs:
        result: List[Path] = list(base_dirs)
        if cache_key is not None:
    ...
    seen: Set[Path] = set(base_dirs)
    result = list(base_dirs)
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against HAGENT_HOME, not cwd
        if not p.is_absolute():
            p = (hagent_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``backend/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    hagent = metadata.get("hagent") or {}
    if not isinstance(hagent, dict):
        hagent = {}
    return {
        "fallback_for_toolsets": hagent.get("fallback_for_toolsets", []),
        "requires_toolsets": hagent.get("requires_toolsets", []),
        "fallback_for_tools": hagent.get("fallback_for_tools", []),
        "requires_tools": hagent.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    hagent = metadata.get("hagent")
    if not isinstance(hagent, dict):
        return []
    raw = hagent.get("config")
    if not raw:
        return []
