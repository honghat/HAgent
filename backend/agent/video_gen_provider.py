...
as ``kind: backend``) or ``$HAGENT_HOME/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``hagent tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``hagent tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``hagent tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``hagent tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$HAGENT_HOME/cache/videos/``, creating parents as needed."""
    from hagent_constants import get_hagent_home

    path = get_hagent_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$HAGENT_HOME/cache/videos/``.
