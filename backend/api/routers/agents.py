...
DEFAULT_CODER_SOUL = """Bạn là Coder, agent chuyên về lập trình trong HAgent.
- Ưu tiên đọc codebase trước khi sửa.
- Dùng tool file/terminal khi cần kiểm tra, build, test hoặc vá lỗi.
- Với model context nhỏ, làm việc theo bước ngắn, chỉ nạp skill/tài liệu khi thật sự cần.
- Trả lời ngắn gọn, nêu rõ file đã sửa và lệnh đã kiểm tra."""
...
def _ensure_default_coder_agent(conn, uid: str) -> None:
    existing = conn.execute(
        "SELECT id, model FROM agents WHERE LOWER(name) = ? AND user_id = ?",
        ("coder", uid),
    ).fetchone()
    if existing:
        updates = []
        params = []
        if (existing["model"] or "") in {"", "lmstudio", "lmstudio_local"}:
            updates.append("model = ?")
            params.append("default")
        updates.append("tool_groups = ?")
    ...
    conn.execute(
        ...
        (
            ...
            uid,
            "Coder",
            "Agent chuyên đọc codebase, sửa lỗi, build/test và hỗ trợ workflow lập trình.",
            "default",
            DEFAULT_CODER_SOUL,
            json.dumps([]),
            json.dumps([]),
...
def list_agents(request: Request):
    ...
    with get_connection() as conn:
        _ensure_default_coder_agent(conn, uid)
        rows = conn.execute(
...
def create_agent(request: Request, body: dict):
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            """INSERT INTO agents (id, user_id, name, description, model, soul_content, tool_groups, skills, auto_start, interval_seconds)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (aid, uid, name, body.get("description", ""), body.get("model", "default"),
             body.get("soul", ""),
             json.dumps([]),
...
def toggle_agent(agent_id: str, request: Request):
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            "UPDATE agents SET is_active = ?, updated_at = datetime('now') WHERE id = ? AND user_id = ?",
            (new_active, agent_id, uid)
        )
