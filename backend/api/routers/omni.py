...
from fastapi import APIRouter, HTTPException, Request, Query, UploadFile, File, BackgroundTasks
...
def _omni_message_role(message_id: str) -> str:
    if not message_id:
        return ""
    with get_connection() as conn:
        row = conn.execute("SELECT role FROM omni_messages WHERE id = ?", (message_id,)).fetchone()
...
@router.get("/conversations/{id}/messages/search", response_model=list[OmniMessage])
def search_conversation_messages_endpoint(
    id: str,
    request: Request,
    ...
    msgs = search_conversation_messages(conv["id"], q, limit=limit)
    return [
        OmniMessage(
            id=m["id"],
...
async def _bg_sync_facebook_thread(user_id: str, thread_id: str, title: str):
    try:
        await _sync_facebook_exact_thread_live(user_id, thread_id, title, max_messages=50)
    except Exception as exc:
        logging.warning("Background live E2EE sync failed for thread %s: %s", thread_id, exc)
        try:
            await _sync_facebook_exact_thread_cookie(user_id, thread_id, title, max_messages=50)
        except Exception as cookie_exc:
            logging.warning("Background cookie E2EE sync failed for thread %s: %s", thread_id, cookie_exc)
...
async def get_conversation_messages_endpoint(
    ...
    if platform == "facebook" and external_id:
        background_tasks.add_task(
            _bg_sync_facebook_thread,
            uid,
            external_id,
            conv.get("sender") or conv.get("title") or ""
        )
    ...
    if platform == "zalo":
        ...
        try:
            if len(paths_to_send) > 1:
                ...
                # Local file — upload to Zalo CDN directly
                result = _run_zalo_bridge(
                    ZALO_SEND_BRIDGE,
                    {
                        "cookie": cookie,
                        "imei": imei,
                        "action": "send_local_file",
                        "target": external_id,
                        "thread_type": thread_type,
                ...
                # Remote file (public URL) — fallback
                result = _run_zalo_bridge(
                    ZALO_SEND_BRIDGE,
                    {
                        "cookie": cookie,
...
def today_stats(request: Request, period: str = Query("today")):
    uid = _get_user_id(request)
    return OmniStats(**get_today_stats(uid, period))
...
_zalo_qr_locks: dict[str, asyncio.Lock] = {}
...
async def _close_zalo_qr_sessions_for_user(user_id: str) -> None:
    session_ids = [
        session_id
        for session_id, sess in list(_zalo_qr_sessions.items())
...
def _active_zalo_qr_session_for_user(user_id: str) -> tuple[str, dict] | None:
    now = time.monotonic()
    for session_id, sess in list(_zalo_qr_sessions.items()):
        if sess.get("user_id") != user_id:
            continue
        qr_data = str(sess.get("qr") or "")
        expires_at = float(sess.get("expires_at") or 0)
        if qr_data and expires_at > now:
...
async def _try_autofill_facebook_pin(page, pin: str) -> bool:
    try:
        if not await _facebook_page_needs_security_unlock(page):
            return False

        logging.info("Facebook E2EE PIN prompt detected. Attempting autofill...")
        ...
        if len(visible_inputs) == 6:
            for i, char in enumerate(pin[:6]):
                await visible_inputs[i].fill(char)
                await page.wait_for_timeout(100)
            logging.info("Filled 6-digit PIN inputs.")
        elif len(visible_inputs) == 1:
            await visible_inputs[0].fill(pin)
            logging.info("Filled single PIN input.")
        else:
            for inp in visible_inputs:
                attr_type = await inp.get_attribute("type")
                if attr_type in {"password", "text", "tel", "number"}:
                    await inp.fill(pin)
                    logging.info("Filled fallback input.")
                    break
        ...
        logging.warning("Failed to autofill Facebook PIN: %s", e)
        return False
...
async def restore_facebook_live_session(user_id: str) -> dict | None:
    ...
    if await _facebook_page_needs_security_unlock(page):
        pin = os.getenv("FACEBOOK_E2EE_PIN", "111111")
        await _try_autofill_facebook_pin(page, pin)
        await page.wait_for_timeout(2500)
...
def _is_facebook_status_junk(content: str) -> bool:
    content_lower = content.lower().strip()
    
    # 1. Exact status matches
    exact_junk = {
        "đã gửi", "đã chuyển", "đang gửi...", "đã xem", "seen", "delivered", "sending...",
        "tùy chỉnh đoạn chat", "file phương tiện và file", "thành viên trong đoạn chat",
        "tắt thông báo", "thay đổi biểu tượng cảm xúc", "tên biệt danh", "xem trang cá nhân",
        "tìm kiếm trong cuộc trò chuyện", "xem ảnh & file", "thay đổi chủ đề",
        "quyền riêng tư & hỗ trợ", "quyền riêng tư và hỗ trợ", "chặn", "vừa xong"
    }
    if content_lower in exact_junk:
        return True
        
    # 2. Regex checks for relative time
    junk_patterns = [
        r"^đã\s+gửi\s+\d+\s+(phút|giờ|ngày)\s+trước$",
        r"^đã\s+nhận\s+\d+\s+(phút|giờ|ngày)\s+trước$",
        r"^đã\s+xem\s+lúc\s+\d{1,2}:\d{2}$",
        r"^đã\s+gửi\s+lúc\s+\d{1,2}:\d{2}$",
        r"^sent\s+\d+\s+(min|hr|hour|day)s?\s+ago$",
        r"^received\s+\d+\s+(min|hr|hour|day)s?\s+ago$",
        r"^active\s+\d+\s+(min|hr|hour|day)s?\s+ago$",
        r"^seen\s+at\s+\d{1,2}:\d{2}$",
        r"^\d{1,2}:\d{2}$",  # Matches standalone times like "11:55" or "20:48"
        r"^\d{1,2}:\d{2}\s+(am|pm)$",
        r"^\d{1,2}:\d{2}\s+(thứ\s+[2-7]|chủ\s+nhật)$",
    ]
...
async def _sync_facebook_exact_thread_cookie(
    ...
    # No longer using Playwright for Facebook sync
...
async def _sync_facebook_for_user(user_id: str, max_threads: int, max_messages: int = 1) -> dict:
    # No longer using Playwright for Facebook sync
    return {"ok": True, "threads": [], "thread_messages": {}}
...
def _apply_zalo_reaction_to_message(
    user_id: str,
    emoji: str,
    msg_id: str,
    cli_msg_id: str,
    ...
    if not emoji or (not msg_id and not cli_msg_id):
        return False, ""
    with get_connection() as conn:
        row = conn.execute(
            """SELECT id, conversation_id, reactions_json FROM omni_messages
               WHERE user_id = ?
                 AND platform = 'zalo'
...
def _apply_zalo_reaction_event(user_id: str, message_object: dict, author_id: str) -> bool:
    ...
    if not msg_id and not cli_msg_id:
        return False
    ok, conversation_id = _apply_zalo_reaction_to_message(user_id, emoji, msg_id, cli_msg_id, author_id)
...
def _insert_zalo_message_once(user_id: str, conversation_id: str, msg: dict, own_id: str = "") -> str:
    ...
    if not external_id:
        return ""
    if str(msg.get("msg_type") or "").lower() == "chat.reaction":
        try:
        ...
        ok, reaction_conversation_id = _apply_zalo_reaction_to_message(
            user_id,
            emoji,
            str(target.get("gMsgID") or ""),
            str(target.get("cMsgID") or ""),
            str(msg.get("author_id") or ""),
        )
        if ok and reaction_conversation_id:
            _broadcast({
                "type": "reaction",
                "platform": "zalo",
                "conversationId": reaction_conversation_id,
            })
        return ""
    with get_connection() as conn:
        exists = conn.execute(
...
async def start_zalo_qr(request: Request):
    user_id = _get_user_id(request)
    lock = _zalo_qr_locks.setdefault(user_id, asyncio.Lock())
    async with lock:
        active = _active_zalo_qr_session_for_user(user_id)
        if active:
            ...
            return {
                ...
                "qr": sess["qr"],
                "status": "pending",
                "detail": "Đang dùng mã QR Zalo vừa tạo. Quét mã này bằng Zalo trên điện thoại.",
                "expires_in": max(1, int(float(sess.get("expires_at") or 0) - time.monotonic())),
            }

        session_id = str(uuid.uuid4())
        await _close_zalo_qr_sessions_for_user(user_id)
        try:
            from playwright.async_api import async_playwright
...
async def check_zalo_qr_status(session: str, request: Request):
    ...
    if expires_at and now > expires_at:
        ...
        return OmniQRStatusResponse(
            session=session,
            status="expired",
            detail="QR đã hết hạn. Hãy tạo mã mới.",
        )
...
def sync_zalo_messages(payload: OmniSyncMessagesRequest, request: Request):
    ...
    for thread in (data.get("threads") or [])[: payload.maxThreads]:
        ...
        for msg in (thread.get("messages") or [])[-payload.maxMessages:]:
            inserted_message_id = _insert_zalo_message_once(user_id, conv_id, msg, own_id=own_id)
            if inserted_message_id:
                role = _omni_message_role(inserted_message_id) or "assistant"
                synced_messages += 1
                _broadcast({
                    "type": "message",
                ...
                if role == "assistant":
                    _start_agent_auto_reply_for_incoming(user_id, conv_id, inserted_message_id)
...
async def facebook_browser_login_status(session_id: str, request: Request):
    ...
    if await _facebook_page_needs_security_unlock(page):
        pin = os.getenv("FACEBOOK_E2EE_PIN", "111111")
        autofilled = await _try_autofill_facebook_pin(page, pin)
        if autofilled:
            await page.wait_for_timeout(2500)
            if await _facebook_page_is_connected(page, sess["context"]):
                cookie = await _get_facebook_cookie_header(sess["context"])
                ...
                _facebook_browser_sessions.pop(session_id, None)
                _start_facebook_mqtt_listener(user_id, cookie)
                return {"session_id": session_id, "status": "connected", "detail": f"Facebook đã kết nối sau khi tự động điền PIN."}
        return {
            "session_id": session_id,
            "status": "pending",
            "detail": f"Facebook đang yêu cầu mã bảo mật/PIN. Đã thử điền tự động PIN nhưng thất bại. Vui lòng nhập trực tiếp trong trình duyệt.",
        }
    return {
        "session_id": session_id,
        "status": "pending",
...
async def sync_facebook_messages(request: Request):
    user_id = _get_user_id(request)
    try:
    ...
    if isinstance(payload, dict) and "messages" in payload:
        ...
        if thread_id:
            ...
            return {
                ...
                "status": f"Đã nhận và lưu {synced} tin nhắn Facebook từ extension.",
            }

    return {
        "synced_conversations": 0,
        "synced_messages": 0,
        "status": "Không đồng bộ bằng Playwright; chờ tin nhắn từ Extension gửi qua.",
    }
