...
DATA_DIR = Path(__file__).resolve().parents[3] / "data" / "video"
...
async def upload_video(
    ...
    upload_path = DATA_DIR / f"{prefix}_{filename}"
    if video:
        upload_path.write_bytes(await video.read())
    task_id = _create_task_in_db(
        title=filename,
        source_type="upload",
        source_ref=upload_path.name,
        voice=voice,
        user_id=uid,
...
async def publish_video(task_id: str, request: Request):
    ...
    video_path = Path(__file__).resolve().parents[3] / "data" / "uploads" / video_file
...
async def youtube_callback(code: str = Query(""), state: str = Query(""), request: Request = None):
    ...
    redirect_uri = (os.environ.get("APP_BASE_URL") or str(request.base_url).rstrip("/")) + "/api/video/auth/youtube/callback"
    client_id = os.environ.get("YOUTUBE_CLIENT_ID", "")
    client_secret = os.environ.get("YOUTUBE_CLIENT_SECRET", "")
