...
def toggle_workflow_schedule(workflow_id: str, request: Request):
    user_id = _user_id(request)
    item = get_workflow(workflow_id, user_id)
    if not item:
        raise HTTPException(status_code=404, detail="Workflow not found")
    schedule = item.get("schedule")
    enabled = not (schedule and schedule.get("enabled"))
    from api.services.workflow_store import upsert_workflow_schedule
    upsert_workflow_schedule(workflow_id, user_id, enabled=enabled)
    return {
        "id": workflow_id,
