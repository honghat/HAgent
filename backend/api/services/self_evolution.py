    ...
    with get_connection() as conn:
        ...
        if duplicate:
            conn.execute(
                ...
                SET evidence = ?, lesson = ?, action = ?, confidence = MAX(confidence, ?),
        ...
        conn.execute(
            "INSERT INTO self_evolution (key, content, metadata_json, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)",
            (
                normalized["event_type"],
                normalized["lesson"],
...
def record_feedback(
    ...
    rating = rating if rating in {"positive", "negative", "needs_memory", "wrong_tool"} else "negative"
    feedback_id = str(uuid4())
    comment = _clip(comment, 1200)
    with get_connection() as conn:
        conn.execute(
            """
            INSERT INTO message_feedback (id, user_id, session_id, message_id, rating, comment)
            VALUES (?, ?, ?, ?, ?, ?)
            ...
            """,
            (feedback_id, user_id, session_id, message_id, rating, comment),
        )
        row = conn.execute("SELECT content FROM messages WHERE id = ?", (message_id,)).fetchone()
    message_excerpt = _clip(row["content"] if row else "", 700)
    if rating == "positive":
        event_type = "successful_workflow"
        title = "Người dùng đánh giá phản hồi tốt"
        lesson = "Phản hồi này phù hợp với kỳ vọng người dùng; có thể dùng làm ví dụ phong cách/cách xử lý."
    elif rating == "needs_memory":
        event_type = "user_preference"
        title = "Người dùng yêu cầu ghi nhớ từ phản hồi"
        lesson = comment or "Người dùng muốn lưu một thông tin hoặc quy tắc từ phản hồi này."
    elif rating == "wrong_tool":
        event_type = "tool_issue"
        title = "Người dùng báo chọn tool hoặc cách thực thi chưa đúng"
        lesson = "Khi nhiệm vụ cần bằng chứng thực thi, agent phải dùng tool phù hợp và chỉ báo kết quả dựa trên output thật."
    else:
        event_type = "agent_failure"
        title = "Người dùng đánh giá phản hồi chưa đạt"
        lesson = "Phản hồi bị người dùng đánh giá tiêu cực; cần xem lại lập luận, tool use, độ chính xác và mức độ hoàn thành."
    event = record_event(
        user_id=user_id,
        event_type=event_type,
        title=title,
        evidence=comment or message_excerpt,
        lesson=lesson,
        action="Xem lại phản hồi, cập nhật memory/skill/test nếu lỗi có thể lặp lại.",
        confidence=0.95 if rating != "positive" else 0.7,
        source="feedback",
        source_session_id=session_id,
        related_message_id=message_id,
        metadata={"rating": rating, "comment": comment},
    )
    return {"ok": True, "rating": rating, "event": event}
