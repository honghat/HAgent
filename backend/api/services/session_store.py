...
def update_message_content(
    ...
    with get_connection() as conn:
        conn.execute(
            "UPDATE messages SET content = ?, usage_json = ? WHERE id = ?",
            (content, json.dumps(usage) if usage else None, message_id),
        )
    ...
    for row in rows:
        ...
        if row["usage_json"]:
            try:
                ...
                usage = None
        msg_id = str(row["id"])
        journal = list_journal(session_id, msg_id)
        items.append({
            "id": msg_id,
            "role": row["role"],
            "content": row["content"],
    ...
    with get_connection() as conn:
        cursor = conn.execute(
            ...
            INSERT INTO run_journals
              (message_id, session_id, type, content, event_name, status, count, input, output)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (message_id, session_id, type_, content, event_name, status, count, input, output),
        )
...
def list_journal(session_id: str, message_id: str | None = None) -> list[dict]:
    query = (
        "SELECT type, content, event_name, status, count, created_at, message_id, input, output "
