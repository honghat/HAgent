...
def _translate_llm(texts: list[str], provider: str = "groq", model: str = "llama-3.3-70b-versatile") -> list[str]:
    """Translate via any OpenAI-compatible provider using provider_config."""
    ...
    try:
        ...
        raise RuntimeError(f"Translate HTTP {e.code}: {e.read().decode(errors='replace')[:200]}") from e


async def translate_batch(texts: list[str], provider: str = "groq", model: str = "llama-3.3-70b-versatile") -> list[str]:
    """Translate in batches of 20 using configurable provider/model."""
    BATCH = 20
    out: list[str] = []
    loop = asyncio.get_event_loop()
    fn = lambda batch: _translate_llm(batch, provider, model)
    for i in range(0, len(texts), BATCH):
        batch = texts[i : i + BATCH]
        try:
            results = await loop.run_in_executor(None, fn, batch)
            out.extend(results)
        except Exception:
            for t in batch:
                try:
                    r = await loop.run_in_executor(None, fn, [t])
...
def _download_video(url: str, output_path: str, send: Callable) -> None:
    """Download video using yt-dlp into a temp dir to avoid stale-path collisions."""
    import tempfile as _tf
    with _tf.TemporaryDirectory(prefix="ytdlp-", dir=UPLOAD_DIR) as _td:
        opts = {
            "format": "bv*[height<=1080]+ba/b[height<=1080]/bv*+ba/b/best",
            "outtmpl": os.path.join(_td, "%(id)s.%(ext)s"),
            "quiet": True,
            "no_warnings": True,
            "retries": 5,
        ...
        with yt_dlp.YoutubeDL(opts) as ydl:
            try:
                ...
                err_msg = str(e1)
                if not any(kw in err_msg for kw in ("Sign in", "age", "private", "members", "No video formats found")):
                    raise
                # yt-dlp Python API's cookies_from_browser has macOS decryption issues.
                # Fallback: use yt-dlp CLI to extract decrypted cookies via --cookies-from-browser.
                import subprocess, os as _os, sys as _sys
                send("Thử lại với cookie Chrome (yt-dlp CLI decrypt)...")
                clean_url = url.split("?")[0]
                result = subprocess.run(
                    [_sys.executable, "-m", "yt_dlp", "--cookies-from-browser", "chrome",
                     "--format", "bv*[height<=1080]+ba/b[height<=1080]/bv*+ba/b/best",
                     "-o", os.path.join(_td, "%(id)s.%(ext)s"),
                ...
                send("Tải với cookies thành công.")

        _move_downloaded_video(_td, output_path)

        if not os.path.exists(output_path) or os.path.getsize(output_path) < 1024:
            raise RuntimeError("yt-dlp không tải được video")


def _move_downloaded_video(temp_dir: str, output_path: str) -> None:
    """Find yt-dlp output and normalize it to an MP4 path for downstream ffmpeg."""
    media_exts = {".mp4", ".m4v", ".webm", ".mkv", ".mov"}
    ignore_suffixes = (".part", ".ytdl", ".json", ".description", ".jpg", ".png", ".webp")
    paths = []
    for root, _, files in os.walk(temp_dir):
        for name in files:
            ...
            if ext in media_exts and os.path.getsize(path) > 1024:
                paths.append(path)
    ...
    candidate = paths[0]
    if os.path.splitext(candidate)[1].lower() in {".mp4", ".m4v", ".mov"}:
...
def make_sender(task_id: int) -> Callable[[str], None]:
    ...
    Each call opens a fresh DB connection to avoid stale-handle errors after retries.
    """
    # Read existing progress on creation
    conn = get_connection()
    ...
    def send(msg: str) -> None:
        ...
        try:
            ...
            pass  # best-effort persist; don't crash pipeline if DB write fails
...
async def run_pipeline(task_id: int) -> None:
    ...
    try:
        ...
        if segments and not segments[0].get("vi"):
            t_provider = task["translate_provider"] if task["translate_provider"] else "groq"
            t_model = task["translate_model"] if task["translate_model"] else "llama-3.3-70b-versatile"
            send(f"🌐 Đang dịch ({t_provider}/{t_model})...")
            vi_texts = await translate_batch([s["text"] for s in segments], provider=t_provider, model=t_model)
