...
Jobs are stored in backend/cron/jobs.json
Output is saved to backend/cron/output/{job_id}/{timestamp}.md
"""
...
HAGENT_DIR = get_hagent_home().resolve()
CRON_DIR = HAGENT_DIR / "cron"
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    try:
        ...
        run_at = _hagent_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Hagent configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Hagent timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _hagent_now().tzinfo
    if dt.tzinfo is None:
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall hagent-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _hagent_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                scripts/; ``.sh`` / ``.bash`` files run via bash,
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _hagent_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _hagent_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _hagent_now().strftime("%Y-%m-%d_%H-%M-%S")
...
def run_job_now(job: Union[str, dict]) -> Dict[str, Any]:
    """Run a cron job immediately (synchronously) and return result.

    Args:
        job: Either a job_id string or a full job dict (with provider/model pre-filled).
    ...
    # Save output
    now_str = _hagent_now().strftime("%Y%m%d_%H%M%S")
    output_dir = OUTPUT_DIR / job_id
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"{now_str}.md"
    try:
        output_file.write_text(doc, encoding="utf-8")
    except Exception as e:
        logger.warning("Job '%s': failed to write output file: %s", job_id, e)

    # Update job state
    deliver_content = final_response or doc
    update_job(job_id, {
        "state": "completed" if success else "failed",
        "last_run_at": _hagent_now().isoformat(),
        "last_status": "success" if success else "error",
        "last_error": error,
        "last_output": deliver_content,
    })

    return {
        "success": success,
        "output": deliver_content,
        "error": error,
    }
