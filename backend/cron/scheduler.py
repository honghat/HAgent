...
Uses a file-based lock (backend/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `hagent update` reloads
# the module) fail with ModuleNotFoundError for hagent_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from hagent_constants import get_hagent_home
from hagent_time import now as _hagent_now

# Lazy-load helper: _expand_env_vars lives in hagent_cli.config
def _expand_env_vars(obj):
    try:
        from hagent_cli.config import _expand_env_vars as _fn
        return _fn(obj)
    except ImportError:
        return obj

# ContextVars for per-job session/delivery state — must be module-level
# so parallel jobs don't clobber each other's state.
_VAR_MAP: dict[str, contextvars.ContextVar] = {
    "HAGENT_SESSION_PLATFORM": contextvars.ContextVar("HAGENT_SESSION_PLATFORM", default=""),
    "HAGENT_SESSION_CHAT_ID": contextvars.ContextVar("HAGENT_SESSION_CHAT_ID", default=""),
    "HAGENT_SESSION_CHAT_NAME": contextvars.ContextVar("HAGENT_SESSION_CHAT_NAME", default=""),
    "HAGENT_CRON_AUTO_DELIVER_PLATFORM": contextvars.ContextVar("HAGENT_CRON_AUTO_DELIVER_PLATFORM", default=""),
    "HAGENT_CRON_AUTO_DELIVER_CHAT_ID": contextvars.ContextVar("HAGENT_CRON_AUTO_DELIVER_CHAT_ID", default=""),
    "HAGENT_CRON_AUTO_DELIVER_THREAD_ID": contextvars.ContextVar("HAGENT_CRON_AUTO_DELIVER_THREAD_ID", default=""),
}
...
def _get_frontend_provider() -> tuple[str, str]:
    '''Lay provider va model ma user da chon tren frontend Chat.jsx.

    Goi GET /api/auth/provider (port 8010) - giong cach Chat.jsx doc
    provider tu localStorage + PUT /api/auth/provider khi user chon tren UI.
    Fallback ve ('', '') neu API khong available.

    Returns:
        (provider_name, model_name) - ca hai co the rong.
    '''
    try:
    ...
    if provider:
        try:
            ...
            with urllib.request.urlopen(req2, timeout=5) as resp2:
                providers_list = _json.loads(resp2.read().decode())
                if isinstance(providers_list, list):
                    for p in providers_list:
                        if p.get('name') == provider:
                            fe_model = str(p.get('model') or p.get('modelStr') or '').strip()
                            return provider, fe_model
        except Exception:
            pass
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict):
    """Resolve the toolset list for a cron job.

    Precedence:
    1. Per-job ``enabled_toolsets`` (set via ``cronjob`` tool on create/update).
       Keeps the agent's job-scoped toolset override intact — #6130.
    2. Per-platform ``hagent tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from hagent_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _get_hagent_home() -> Path:
    """Resolve Hagent home dynamically while preserving test monkeypatch hooks."""
    return _hagent_home or get_hagent_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    hagent_home = _get_hagent_home()
    lock_dir = hagent_home / "cron"
...
_MEDIA_TAG_RE = re.compile(r"^MEDIA:\s*(.+?)\s*$", re.MULTILINE)


def _send_telegram_direct(chat_id: str, text: str, thread_id=None) -> Optional[str]:
    """Send a plain text message to Telegram via Bot API. Returns error or None."""
    ...
    for chunk in chunks:
        payload = {"chat_id": str(chat_id), "text": chunk, "disable_web_page_preview": True}
        if thread_id:
            try:
        ...
        except (_uerr.HTTPError, OSError, ValueError) as exc:
            return f"telegram send failed: {exc}"
        if not data.get("ok"):
            return f"telegram api error: {data.get('description', data)}"
    return None


def _extract_media(content: str) -> tuple[list, str]:
    """Pull ``MEDIA: /path/to/file`` lines out of ``content``.

    Returns ``(media_files, cleaned_content)`` where ``media_files`` is a list
    of ``(path, is_voice)`` tuples and ``cleaned_content`` has the MEDIA lines
    stripped. ``is_voice`` is reserved for future voice-note routing — always
    ``False`` here since cron deliveries don't tag voice explicitly.
    """
    ...
    cleaned = _MEDIA_TAG_RE.sub("", content).strip()
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    media_files, cleaned_delivery_content = _extract_media(delivery_content)

    delivery_errors = []
    for target in targets:
        platform_name = (target.get("platform") or "").lower()
        ...
        if platform_name == "telegram":
            err = _send_telegram_direct(chat_id, text, thread_id)
            if err:
                msg = f"delivery to telegram:{chat_id} failed: {err}"
                logger.error("Job '%s': %s", job["id"], msg)
                delivery_errors.append(msg)
            else:
                logger.info("Job '%s': delivered to telegram:%s", job["id"], chat_id)
        else:
            msg = f"platform '{platform_name}' delivery not supported"
            logger.warning("Job '%s': %s", job["id"], msg)
            delivery_errors.append(msg)
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("HAGENT_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid HAGENT_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within the repo-root scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against repo-root scripts/.  Absolute and ~-prefixed paths
    ...
    try:
        tokens = shlex.split(script_path)
    ...
    # escape — scripts MUST reside within repo-root scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _hagent_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    # ---------------------------------------------------------------
    # Default (LLM) path — run the prompt through the FastAPI chat
    # endpoint (same path as Chat.jsx) instead of creating AIAgent
    # directly.  This ensures the agent sees the same tool definitions,
    # platform hints, and system prompt as the web frontend.
    # ---------------------------------------------------------------
    import requests as _cron_requests
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_hagent_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_hagent_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["HAGENT_CRON_SESSION"] = "1"
    ...
    #     (notify_on_complete / watch_patterns) reads HAGENT_SESSION_PLATFORM
    #     and HAGENT_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via HAGENT_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     HAGENT_SESSION_PLATFORM == "telegram".
    ...
    #     HAGENT_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read HAGENT_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the HAGENT_CRON_AUTO_DELIVER_* vars set
    # below, so clearing HAGENT_SESSION_* here does not affect delivery.
    _VAR_MAP["HAGENT_SESSION_PLATFORM"].set("")
    _VAR_MAP["HAGENT_SESSION_CHAT_ID"].set("")
    _VAR_MAP["HAGENT_SESSION_CHAT_NAME"].set("")
    # Store old values as a tuple for later restore
    _ctx_tokens = ("", "", "")
    _cron_delivery_vars = (
        "HAGENT_CRON_AUTO_DELIVER_PLATFORM",
        "HAGENT_CRON_AUTO_DELIVER_CHAT_ID",
        "HAGENT_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_hagent_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_hagent_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["HAGENT_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["HAGENT_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["HAGENT_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Lấy provider+model từ frontend (giống Chat.jsx) nếu job không pin riêng
        ...
        model = job.get("model") or _fe_model or os.getenv("HAGENT_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_hagent_home() / "config.yaml")
            if os.path.exists(_cfg_path):
                with open(_cfg_path, encoding="utf-8") as _f:
                    _cfg = yaml.safe_load(_f) or {}
                _cfg = _expand_env_vars(_cfg)
                _model_cfg = _cfg.get("model", {})
                if not job.get("model") and not _fe_model:
        ...
        prefill_file = os.getenv("HAGENT_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_hagent_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject HAGENT_INFERENCE_PROVIDER here. resolve_runtime_provider()
            # already prefers persisted config over stale shell/env overrides when
            # no explicit provider is requested. Passing the env var here short-
            # circuits that precedence and can resurrect old providers (for
            # example DeepSeek) for cron jobs that do not pin provider/model.
            runtime_kwargs = {
                "requested": _enforced_provider or None,
            }
            if job.get("base_url"):
        ...
        # Run the prompt through the FastAPI chat endpoint, which uses the
        # same agent creation path as Chat.jsx (run_source_agent).  This
        # ensures the agent sees the correct tool definitions, platform
        # hints ("webui"), and system prompt — unlike the old cron path
        # which created AIAgent with quiet_mode=True and platform="cron".
        #
        # Total timeout: default 600s, overridable via HAGENT_CRON_TIMEOUT.
        _raw_cron_timeout = os.getenv("HAGENT_CRON_TIMEOUT", "").strip()
        _cron_timeout = 600.0
        if _raw_cron_timeout:
            try:
                ...
                logger.warning(
                    "Invalid HAGENT_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
        _api_base = os.environ.get("HAGENT_API_BASE", "http://localhost:8010")
        _resp = _cron_requests.post(
            f"{_api_base}/api/cron/run-chat",
            json={
                "prompt": prompt,
        ...
        except _cron_requests.HTTPError as http_exc:
            detail = (_resp.text or "").strip()
            if detail:
                raise _cron_requests.HTTPError(f"{http_exc}; response={detail[:1000]}") from http_exc
            raise
        _result = _resp.json()
        final_response = str(_result.get("reply") or "")
...
**Job ID:** {job_id}
**Run Time:** {_hagent_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _hagent_now().strftime('%H:%M:%S'))
            return 0

        # Filter jobs in quiet hours (23:00–07:00) — skip news/tts jobs during
        # sleeping hours so the user isn't woken up by spoken news.
        _quiet_start = 23  # 11 PM
        _quiet_end = 7     # 7 AM
        _current_hour = _hagent_now().hour
        _in_quiet = _quiet_start <= _current_hour or _current_hour < _quiet_end
        if _in_quiet:
            _quiet_jobs = []
            for j in due_jobs:
                _skills = j.get("skills") or []
                if isinstance(_skills, str):
                    _skills = [_skills]
                if "tin-tuc" in _skills:
                    logger.info(
                        "Job '%s' skipped — quiet hours (%d:00–%d:00)",
                        j.get("name", j["id"]), _quiet_start, _quiet_end,
                    )
                else:
                    _quiet_jobs.append(j)
            due_jobs = _quiet_jobs

        if verbose:
            logger.info("%s - %s job(s) due", _hagent_now().strftime('%H:%M:%S'), len(due_jobs))

        # Advance next_run_at for all recurring jobs FIRST, under the file lock,
        # before any execution begins.  This preserves at-most-once semantics.
        # Jobs skipped by quiet-hours filter are NOT advanced — they'll be
        # re-evaluated on the next tick and run once quiet hours end.
        for job in due_jobs:
            advance_next_run(job["id"])

        # Resolve max parallel workers: env var > config.yaml > unbounded.
        # Set HAGENT_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("HAGENT_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid HAGENT_CRON_MAX_PARALLEL value; defaulting to unbounded")
