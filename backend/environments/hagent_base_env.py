"""
HagentAgentBaseEnv -- Abstract Base Environment for Hagent-Agent + Atropos

Provides the Atropos integration plumbing that all hagent-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via HagentAgentLoop
...
# Ensure the hagent-agent repo root is on sys.path so that imports like
...
# Load API keys from hagent-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, HagentAgentLoop
...
# Import hagent-agent toolset infrastructure
...
class HagentAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for hagent-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of hagent toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: hagent, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/hagent-results/ "
...
class HagentAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for hagent-agent Atropos integration.
    ...
    name: Optional[str] = "hagent-agent"
    env_config_cls = HagentAgentEnvConfig

    def __init__(
        self,
        config: HagentAgentEnvConfig,
        ...
        # Set terminal environment variables so hagent tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY hagent-agent tool (terminal, file, web,
