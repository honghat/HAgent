"""
HagentSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser hagent
    run-api
    python environments/hagent_swe_env.py serve \\
...
class HagentSweEnv(HagentAgentBaseEnv):
    ...
    name = "hagent-swe"
    env_config_cls = HagentSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[HagentSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        env_config = HagentSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/DeepHagent-3-Llama-3-3B-Preview",
            tool_call_parser="hagent",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="hagent-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/DeepHagent-3-Llama-3-3B-Preview",
