"""Windows UTF-8 bootstrap for Hagent entry points.
...
should be imported at the very top of every Hagent entry point
(``hagent``, ``hagent-agent``, ``hagent-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import hagent_bootstrap``
# (or ``from hagent_bootstrap import apply_windows_utf8_bootstrap``) at
