...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_hagent_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:hagent_pkce",
    ...
    if provider == "nous":
        # Codex-style auto-import: if a sharedNous credential lives at
        # <hagent-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `hagent --profile <name>
        ...
        if shared:
            ...
            print()
            if path:
                print(f"Found existingNous OAuth credentials at {path}")
            else:
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `hagent auth
    ...
    raise SystemExit(f"`hagent auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Hagent reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `hagent auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `hagent auth` is called bare."""
