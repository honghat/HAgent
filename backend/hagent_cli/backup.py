"""
Backup and import commands for hagent CLI.

`hagent backup` creates a zip archive of the entire backend/ directory
(excluding the hagent-agent repo and transient files).

`hagent import` restores from a backup zip, overlaying onto the current
HAGENT_HOME root.
"""
...
from hagent_constants import get_default_hagent_root, get_hagent_home, display_hagent_home
...
_EXCLUDED_DIRS = {
    "hagent-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to hagent root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Hagent home directory."""
    hagent_root = get_default_hagent_root()

    if not hagent_root.is_dir():
        print(f"Error: Hagent home directory not found at {hagent_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"hagent-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"hagent-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_hagent_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(hagent_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(hagent_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(hagent_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside hagent root
            try:
    ...
    print(f"\nRestore with: hagent import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Hagent backup.
    ...
    # Look for telltale files that a hagent home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Hagent backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip with a directory prefix (e.g. ``backend/config.yaml``).
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a backend dir name
        if prefix in ("backend", "hagent"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_hagent_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (hagent_root / "config.yaml").exists()
        has_env = (hagent_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Hagent configuration.")
        ...
        hagent_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = hagent_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(hagent_root.resolve())
            except ValueError:
        ...
        profiles_dir = hagent_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # hagent_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: hagent profile list  (after installing hagent)")

        # Guidance
        print()
        if not (hagent_root / "hagent-agent").is_dir():
            print("Note: The hagent-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: hagent update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  hagent -p {pname} gateway install")

        print("Done. Your Hagent configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and hagent backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to HAGENT_HOME).
...
# — `hagent update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(hagent_home: Optional[Path] = None) -> Path:
    home = hagent_home or get_hagent_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    hagent_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(hagent_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(hagent_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for hagent backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_hagent_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, hagent_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``hagent_root`` to ``out_path``.
...
def _pre_update_backup_dir(hagent_home: Optional[Path] = None) -> Path:
    home = hagent_home or get_hagent_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of HAGENT_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<HAGENT_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``hagent update``) should continue even if the backup fails.
    """
    hagent_root = hagent_home or get_default_hagent_root()
    if not hagent_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(hagent_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, hagent_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `hagent claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of HAGENT_HOME under ``backups/`` before a
    ``hagent claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``hagent import <archive>``.  Writes to
    ``<HAGENT_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `hagent import` and the
