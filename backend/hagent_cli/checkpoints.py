"""`hagent checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``backend/checkpoints/``.  Actions:

    hagent checkpoints               # same as `status`
    hagent checkpoints status        # total size, project count, breakdown
    hagent checkpoints list          # per-project checkpoint counts + workdir
    hagent checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    hagent checkpoints clear [-f]    # nuke the entire base (asks first)
    hagent checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    hagent checkpoints
    hagent checkpoints prune --retention-days 3 --max-size-mb 200
    hagent checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: hagent checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``hagent checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `hagent checkpoints` → status
