"""
Configuration management for Hagent Agent.

Config files are stored in backend/ for easy access:
- backend/config.yaml  - All settings (model, toolsets, terminal, etc.)
- backend/.env         - API keys and secrets

This module provides:
- hagent config          - Show current configuration
- hagent config edit     - Open config in editor
- hagent config set      - Set a specific value
- hagent config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``backend/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``hagent logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  hagent config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `hagent plugins enable
    # observability/langfuse` or `hagent tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "HAGENT_LANGFUSE_ENV",
    "HAGENT_LANGFUSE_RELEASE",
    "HAGENT_LANGFUSE_SAMPLE_RATE",
    "HAGENT_LANGFUSE_MAX_CHARS",
    "HAGENT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from hagent_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("HAGENT_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_hagent_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Hagent is running in package-manager-managed mode.

    Two signals: the HAGENT_MANAGED env var (set by the systemd service),
    or a .managed marker file in HAGENT_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade hagent-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "hagent update"


def format_managed_message(action: str = "modify this Hagent installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("HAGENT_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Hagent installation is managed by NixOS "
            f"(HAGENT_MANAGED={env_hint}).\n"
            "Edit services.hagent-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Hagent installation is managed by Homebrew "
            f"(HAGENT_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade hagent-agent"
        )

    return (
        f"Cannot {action}: this Hagent installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Hagent."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from HAGENT_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hagent_bin
    or None if container mode is not active, we're already inside the
    container, or HAGENT_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("HAGENT_DEV") == "1":
        return None
    ...
    container_mode_file = get_hagent_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "hagent-agent")
    exec_user = info.get("exec_user", "hagent")
    hagent_bin = info.get("hagent_bin", "/data/current-package/bin/hagent")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "hagent_bin": hagent_bin,
    }
...
# Re-export from hagent_constants — canonical definition lives there.
from hagent_constants import get_hagent_home, get_logs_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_hagent_home() / "config.yaml"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the hagent group can
    share state with the gateway service.

    The mode can be overridden via the HAGENT_HOME_MODE environment variable
    (e.g. HAGENT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse HAGENT_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("HAGENT_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Hagent runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("HAGENT_CONTAINER") or os.environ.get("HAGENT_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set HAGENT_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into HAGENT_HOME if the user doesn't have one yet."""
...
def ensure_hagent_home():
    """Ensure backend directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_hagent_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_hagent_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"HAGENT_HOME {home} does not exist. "
    ...
    get_logs_home().joinpath("curator").mkdir(parents=True, exist_ok=True)
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Hagent-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Hagent auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Hagent sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Hagent
        ...
        #  "$HAGENT_HOME/cache/documents:/output"]
        ...
        # bundled Hagent image, which drops to the `hagent` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Hagent sends a stable profile-scoped userId to Camofox
    ...
    # Opt in via ``hagent chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``backend/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: hagent sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Hagent truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``hagent kanban specify`` (single id or --all). Set a
        ...
        # model; override via `hagent model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `hagent --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `hagent -c` muscle memory.  Default off so existing
        # users aren't surprised.  HAGENT_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/hagent`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = backend/cache/piper-voices/
    ...
    "stt": {
        "enabled": True,
        "provider": "groq",  # "local" (free, faster-whisper) | "groq" | "openai" (Whisper API) | "mistral" (Voxtral Transcribe)
        "local": {
    ...
    # a plugin in plugins/context_engine/<name>/ or backend/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        ...
        "memory_char_limit": 12000,  # ~4K tokens at 2.75 chars/token
        "user_char_limit": 8000,     # ~3K tokens at 2.75 chars/token
    ...
    # response. If not, Hagent feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Hagent auto-pauses the goal and
    ...
    # always goes to backend/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${HAGENT_SKILL_DIR} and ${HAGENT_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `hagent curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot backend/skills/ into
        # backend/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `hagent curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for hagent-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Hagent Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (HAGENT_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from backend/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or HAGENT_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via HAGENT_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `hagent -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     hagent-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to root logs/.
    ...
    # curated model lists for OpenRouter andgateway Hat Nguyen Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a hagent-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hagent-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `hagent model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of data/state.db.
    ...
    "sessions": {
        ...
        # default of ``hagent sessions prune``.
        "retention_days": 90,
    ...
    # ``hagent update`` behaviour.
    "updates": {
        # Run a full ``hagent backup``-style zip of HAGENT_HOME before every
        # ``hagent update``.  Backups land in ``<HAGENT_HOME>/backups/`` and
        # can be restored with ``hagent import <path>``.  Off by default —
        # on large HAGENT_HOME directories the zip can add minutes to every
...
# selection step (gateway Hat Nguyen Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "gateway Hat Nguyen Portal base URL override",
        "prompt": "gateway Hat Nguyen Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'hagent model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override forHat Nguyen Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix forHat Nguyen Subscribers only, used to derive vendor hosts, e.g. nousresearch.com -> firecrawl-gateway.nousresearch.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme forHat Nguyen Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "ExplicitHat Nguyen Subscriber access token for tool-gateway requests (optional; otherwise read from the Hagent auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @hagent:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. HAGENT_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #hagent)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: hagent-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'hagent-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Hagent API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Hagent API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Hagent API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # HAGENT_TOOL_PROGRESS and HAGENT_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "HAGENT_TOOL_PROGRESS": {
    ...
    "HAGENT_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.hagent.config`` entries, then checks
    ...
    try:
        ...
        # should never break `hagent update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Hagent versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``hagent_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'hagent setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Hagent won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'hagent doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HAGENT_HOME", "backend")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("HAGENT_TOOL_PROGRESS")
            old_mode = get_env_value("HAGENT_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from HAGENT_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from HAGENT_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("HAGENT_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from HAGENT_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$HAGENT_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_hagent_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `hagent plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `hagent curator status` has no
    ...
    #   3. Creates `logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hagent_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_logs_home() / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: hagent config set <key> <value>")
    ...
    # metadata.hagent.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _hagent_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read backend/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``HAGENT_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_hagent_home()
...
#   openai-codex (OAuth — hagent auth) — OpenAI Codex
#   nous         (OAuth — hagent auth) —gateway Hat Nguyen Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to backend/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Hagent env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite backend/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in backend/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from backend/.env and os.environ.
...
def reload_env() -> int:
    """Re-read backend/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Hagent — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Hagent vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from backend/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Hagent Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  hagent config edit     # Edit config file", Colors.DIM))
    print(color("  hagent config set <key> <value>", Colors.DIM))
    print(color("  hagent setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: hagent config set <key> <value>")
            print()
            print("Examples:")
            print("  hagent config set model anthropic/claude-sonnet-4")
            print("  hagent config set terminal.backend docker")
            print("  hagent config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'hagent config migrate' to add them")
        ...
        print("  hagent config           Show current configuration")
        print("  hagent config edit      Open config in editor")
        print("  hagent config set <key> <value>   Set a config value")
        print("  hagent config check     Check for missing/outdated config")
        print("  hagent config migrate   Update config with new options")
        print("  hagent config path      Show config file path")
        print("  hagent config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``hagent config`` UI so users
