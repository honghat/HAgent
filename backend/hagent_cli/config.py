...
Config files are stored in ~/.hagent/ for easy access:
- ~/.hagent/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.hagent/.env         - API keys and secrets
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.hagent/config.yaml`` causes ``load_config()``
...
from hagent_constants import get_hagent_home  # noqa: F811,E402
...
def ensure_hagent_home():
    """Ensure ~/.hagent directory structure exists with secure permissions.
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        #  "/home/user/.hagent/cache/documents:/output"]
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.hagent/checkpoints/`` size (MB).  When
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.hagent/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.hagent/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.hagent/skills/ into
        # ~/.hagent/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    # stored approval from ~/.hagent/shell-hooks-allowlist.json.
    ...
    # Logging — controls file logging to ~/.hagent/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.hagent/state.db.
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HAGENT_HOME", "~/.hagent")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    #   3. Creates `~/.hagent/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hagent_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_hagent_home() / "logs" / "curator"
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.hagent/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.hagent/config.yaml.
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.hagent/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.hagent/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.hagent/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.hagent/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.hagent/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.hagent/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.hagent/.env or environment."""
