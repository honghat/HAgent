...
_DHH = display_hagent_home()  # user-facing display path (e.g. ~/.hagent or ~/.hagent/profiles/coder)

# Load environment variables from ~/.hagent/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.hagent/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # Check ~/.hagent/.env (primary location for user config)
    ...
    # Check ~/.hagent/config.yaml (primary) or project cli-config.yaml (fallback)
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.hagent/.env or run 'hagent setup')",
                            )
                            issues.append(
    ...
    state_db_path = hagent_home / "state.db"
    ...
    wal_path = hagent_home / "state.db-wal"
    if wal_path.exists():
        try:
