"""
Doctor command for hagent CLI.

Diagnoses issues with Hagent Agent setup.
"""
...
from hagent_cli.config import get_project_root, get_hagent_home, get_env_path
from hagent_cli.env_loader import load_hagent_dotenv
from hagent_constants import display_hagent_home, get_data_home

PROJECT_ROOT = get_project_root()
HAGENT_HOME = get_hagent_home()
_DHH = display_hagent_home()  # user-facing display path (e.g. backend or backend/profiles/coder)

# Load environment variables from backend/.env so API key checks work
_env_path = get_env_path()
load_hagent_dotenv(hagent_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from hagent_cli.colors import Colors, color
from hagent_cli.models import _HAGENT_USER_AGENT
from hagent_cli.vercel_auth import describe_vercel_auth
from hagent_constants import OPENROUTER_MODELS_URL
...
from hagent_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when backend/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("HAGENT_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("HAGENT_KANBAN_TASK"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from hagent_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `hagent`.
    os.environ.setdefault("HAGENT_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Hagent Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check backend/.env (primary location for user config)
    env_path = HAGENT_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'hagent setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'hagent setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'hagent setup' to create one")
                issues.append("Run 'hagent setup' to create .env")
    
    # Check backend/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from hagent_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from hagent_cli.providers import (
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'hagent config set model.provider <valid_provider>'"
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom", "openrouter"):
                try:
                    from hagent_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check backend/.env or run 'hagent setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'hagent setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'hagent config set model.provider <name>'"
                            )
                except Exception:
                    pass
    ...
    if config_path.exists():
        try:
            from hagent_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'hagent setup' to migrate config")
                else:
                    issues.append("Run 'hagent doctor --fix' or 'hagent setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'hagent doctor --fix'")
        except Exception:
            pass
    ...
    try:
        ...
        if nous_status.get("logged_in"):
            check_ok("gateway Hat Nguyen Portal auth", "(logged in)")
        else:
            check_warn("gateway Hat Nguyen Portal auth", "(not logged in)")
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses Hagent' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `hagent auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    hagent_home = HAGENT_HOME
    if hagent_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            hagent_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = hagent_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Hagent a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Hagent Agent Persona\n\n"
                "<!-- Edit this file to customize how Hagent communicates. -->\n\n"
                "You are Hagent, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = hagent_home / "memories"
    ...
    state_db_path = get_data_home() / "state.db"
    ...
    wal_path = get_data_home() / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'hagent doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (hagent bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "hagent"
        ...
        _cmd_link = _cmd_link_dir / "hagent"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(hagent not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/hagent → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/hagent points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/hagent → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/hagent — run 'hagent doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/hagent exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/hagent not found",
                    "(hagent command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/hagent → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/hagent symlink — run 'hagent doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'hagent-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'hagent-agent[vercel]'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `hagent doctor` invocation.
    ...
    if _npm_bin:
        npm_dirs = [
            ...
            (PROJECT_ROOT.parent / "scripts" / "whatsapp-bridge", "WhatsApp bridge"),
        ]
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'hagent config set model.provider <provider>' "
    ...
    # never the legitimate source for `hagent doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'hagent setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = HAGENT_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: hagent skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = HAGENT_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: hagent memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: hagent memory setup")
                issues.append("No Honcho API key — run 'hagent memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run hagent memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: hagent memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: hagent memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from hagent_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "hagent -p" in content:
                            _m = _re.search(r"hagent -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'hagent doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
