"""
Dump command for hagent CLI.

Outputs a compact, plain-text summary of the user's Hagent setup
...
from hagent_cli.config import get_hagent_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``hagent dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from hagent_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(hagent_home: Path) -> int:
...
def _cron_summary(hagent_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = hagent_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"hagent_home:      {display_hagent_home()}")
    ...
    toolsets = config.get("toolsets", ["hagent-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(hagent_home)}")
    lines.append(f"  skills:             {_count_skills(hagent_home)}")
