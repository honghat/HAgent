"""Persistent session goals — the Ralph loop for Hagent.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Hagent feeds a
...
- This module has zero hard dependency on ``cli.HagentCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current HAGENT_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``hagent_home`` path so profile switches still pick up the right DB.
...
def _goals_dump_dir() -> Optional[Path]:
    """Return ``<HAGENT_HOME>/goals`` (created on first use), or None on error."""
    try:
        from hagent_constants import get_hagent_home

        home = Path(get_hagent_home())
    except Exception as exc:
        logger.debug("goals dump dir: get_hagent_home failed: %s", exc)
        return None
    try:
        path = home / "goals"
...
class GoalManager:
    ...
    def evaluate_after_turn(
        ...
        *,
        ...
        When provided, it's dumped to ``<HAGENT_HOME>/goals/<sid>.json`` so
