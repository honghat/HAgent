"""hagent hooks — inspect and manage shell-script hooks.

Usage::

    hagent hooks list
    hagent hooks test <event> [--for-tool X] [--payload-file F]
    hagent hooks revoke <command>
    hagent hooks doctor

Consent records live under ``backend/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``backend/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``hagent hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: hagent hooks {list|test|revoke|doctor}")
        print("Run 'hagent hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in backend/config.yaml.")
        print("See `hagent hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `hagent hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `hagent hooks test` and `hagent hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Hagent wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `hagent hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `hagent hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `hagent hooks doctor`.")
