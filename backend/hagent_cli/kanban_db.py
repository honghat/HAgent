...
``<root>`` is the **shared Hagent root** (the parent of any active
...
``hagent -p <profile>`` joins the same board as the dispatcher that
...
the profile's Hagent home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``HAGENT_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``HAGENT_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``hagent kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.hagent``. In Docker / custom
deployments where ``HAGENT_HOME`` points outside ``~/.hagent`` (e.g.
``/opt/hagent``), ``<root>`` is ``HAGENT_HOME``. Legacy env-var
overrides still work:

* ``HAGENT_KANBAN_DB`` — pin the database file path directly.
* ``HAGENT_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``HAGENT_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``HAGENT_KANBAN_DB``,
``HAGENT_KANBAN_WORKSPACES_ROOT``, and ``HAGENT_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/hagent-kanban-v1-spec.pdf`` for the full
...
# enough that kebab-case names like ``atm10-server`` or ``hagent-agent``
...
def kanban_home() -> Path:
    """Return the shared Hagent root that anchors the kanban board.

    Resolution order:

    1. ``HAGENT_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hagent_root()``, which already returns ``<root>``
       when ``HAGENT_HOME`` is ``<root>/profiles/<name>``, and returns
       ``HAGENT_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``HAGENT_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("HAGENT_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``hagent kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``HAGENT_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``hagent kanban boards
    ...
    """
    env = os.environ.get("HAGENT_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``hagent kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``HAGENT_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("HAGENT_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``HAGENT_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("HAGENT_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``hagent kanban log`` unambiguous even when multiple
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``HAGENT_KANBAN_DB`` env → ``HAGENT_KANBAN_BOARD`` env →
    ...
    # See hagent_state._WAL_INCOMPAT_MARKERS for detection logic.
    from hagent_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``hagent kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from hagent_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``hagent --skills ...`` alongside the built-in
    ...
    # `hagent --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `hagent kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``hagent kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``hagent kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``hagent kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Hagent
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``hagent kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Hagent profile.
    ...
    if not rows:
        return False
    try:
        from hagent_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Hagent profile.
        # `_default_spawn` invokes ``hagent -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Hagent
...
def _resolve_hagent_argv() -> list[str]:
    """Resolve the ``hagent`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``shutil.which("hagent")`` — the console-script shim, the same form
       that shows up in ``ps`` output and existing logs. Preferred so live
       systems' diagnostics stay familiar.
    2. ``sys.executable -m hagent_cli.main`` — fallback for setups where
       Hagent is launched from a venv and the ``hagent`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hagent_bin`` for the same reason. Kept
    local (not imported from gateway) because ``hagent_cli`` sits below
    ...
    if hagent_bin:
        return [hagent_bin]
    # Fallback to the module form. ``hagent_cli.main`` is the actual
    # console-script target declared in pyproject.toml, NOT a top-level
    # ``hagent`` package — there is no ``hagent`` package to import.
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``hagent -p <profile> chat -q ...`` subprocess.
    ...
    ``HAGENT_KANBAN_DB`` / ``HAGENT_KANBAN_BOARD`` / workspaces_root env
    ...
    env = dict(os.environ)
    if task.tenant:
        env["HAGENT_TENANT"] = task.tenant
    env["HAGENT_KANBAN_TASK"] = task.id
    env["HAGENT_KANBAN_WORKSPACE"] = workspace
    if task.current_run_id is not None:
        env["HAGENT_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["HAGENT_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Pin the shared board + workspaces root the dispatcher resolved, so
    # that even when the worker activates a profile (`hagent -p <name>`
    # rewrites HAGENT_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hagent_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["HAGENT_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["HAGENT_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["HAGENT_KANBAN_BOARD"] = resolved_board
    # HAGENT_PROFILE is the author the kanban_comment tool defaults to.
    # `hagent -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["HAGENT_PROFILE"] = profile_arg

    cmd = [
        *_resolve_hagent_argv(),
        "-p", profile_arg,
    ...
    # `hagent kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`hagent` executable not found on PATH. "
            "Install Hagent Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``hagent kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HAGENT_PROFILE values like "hagent-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Hagent root exists

    Reads profile paths directly so this module has no import dependency on
    ``hagent_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from hagent_constants import get_default_hagent_root
        default_root = get_default_hagent_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``hagent kanban assignees`` for the terminal.
