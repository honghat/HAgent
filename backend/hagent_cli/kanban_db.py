...
``<root>`` is the **shared data root** (``data/`` at the project root).
Profiles intentionally collapse onto a shared board: it IS
...
the same data root but are otherwise isolated: a worker spawned
...
In standard installs ``<root>`` is ``backend``. In Docker / custom
deployments where ``HAGENT_HOME`` points outside ``backend`` (e.g.
...
def kanban_home() -> Path:
    """Return the root directory for kanban data (``<root>/kanban.db``,
    ``<root>/kanban/boards/``, ``<root>/kanban/workspaces/``, etc.).
    All kanban data lives under ``data/`` at the project root.
    ...
    from hagent_constants import get_data_home
