#!/usr/bin/env python3
"""
Hagent CLI - Main entry point.

Usage:
    hagent                     # Interactive chat (default)
    hagent chat                # Interactive chat
    hagent gateway             # Run gateway in foreground
    hagent gateway start       # Start gateway as service
    hagent gateway stop        # Stop gateway service
    hagent gateway status      # Show gateway status
    hagent gateway install     # Install gateway service
    hagent gateway uninstall   # Uninstall gateway service
    hagent setup               # Interactive setup wizard
    hagent logout              # Clear stored authentication
    hagent status              # Show status of all components
    hagent cron                # Manage cron jobs
    hagent cron list           # List cron jobs
    hagent cron status         # Check if cron scheduler is running
    hagent doctor              # Check configuration and dependencies
    hagent honcho setup                    # Configure Honcho AI memory integration
    hagent honcho status                   # Show Honcho config and connection status
    hagent honcho sessions                 # List directory → session name mappings
    hagent honcho map <name>               # Map current directory to a session name
    hagent honcho peer                     # Show peer names and dialectic settings
    hagent honcho peer --user NAME         # Set user peer name
    hagent honcho peer --ai NAME           # Set AI peer name
    hagent honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    hagent honcho mode                     # Show current memory mode
    hagent honcho mode [hybrid|honcho|local]  # Set memory mode
    hagent honcho tokens                   # Show token budget settings
    hagent honcho tokens --context N       # Set session.context() token cap
    hagent honcho tokens --dialectic N     # Set dialectic result char cap
    hagent honcho identity                 # Show AI peer identity representation
    hagent honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    hagent honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hagent + Honcho
    hagent version             Show version
    hagent update              Update to latest version
    hagent uninstall           Uninstall Hagent Agent
    hagent acp                 Run as an ACP server for editor integration
    hagent sessions browse     Interactive session picker with search

    hagent claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: hagent_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``hagent_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``hagent update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``hagent_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, hagent crashes on import and the user can't run
# ``hagent update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import hagent_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to HAGENT_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (hagent tools, hagent setup, hagent model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'hagent {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any hagent module import.
#
# Many modules cache HAGENT_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HAGENT_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to backend/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set HAGENT_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors hagent_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If HAGENT_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. backend/profiles/coder or
    # /opt/data/profiles/coder).  If HAGENT_HOME points to the hagent root
    # instead (e.g. systemd hardcodes HAGENT_HOME=/backend), we must
    # still read active_profile — the user may have switched profiles via
    # `hagent profile use` and the gateway should honour that choice.
    # See issue #22502.
    hagent_home_env = os.environ.get("HAGENT_HOME", "")
    if profile_name is None and hagent_home_env:
        if Path(hagent_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the hagent root
    if profile_name is None:
        try:
            from hagent_constants import get_default_hagent_root

            active_path = get_default_hagent_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set HAGENT_HOME
    if profile_name is not None:
        try:
            from hagent_cli.profiles import resolve_profile_env

            hagent_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent hagent from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["HAGENT_HOME"] = hagent_home
...
# Load .env from project root (single source of truth — /Users/.../HAgent/.env).
# User-managed env files should override stale shell exports on restart.
from hagent_cli.config import get_hagent_home
from hagent_cli.env_loader import load_hagent_dotenv

load_hagent_dotenv(project_env=PROJECT_ROOT.parent / ".env")

# Bridge security.redact_secrets from config.yaml → HAGENT_REDACT_SECRETS env
# var BEFORE hagent_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "HAGENT_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_hagent_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["HAGENT_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `hagent` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from hagent_logging import setup_logging as _setup_logging
...
try:
    from hagent_cli.config import load_config as _load_config_early
    from hagent_constants import apply_ipv4_preference as _apply_ipv4
...
from hagent_cli import __version__, __release_date__
from hagent_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from hagent_cli.config import get_env_path, get_hagent_home, load_config
    from hagent_cli.auth import get_auth_status

    # Determine whether Hagent itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from hagent_cli.config import DEFAULT_CONFIG

    _DEFAULT_MODEL = DEFAULT_CONFIG.get("model", "")
    _DEFAULT_MODEL_CFG = _DEFAULT_MODEL if isinstance(_DEFAULT_MODEL, dict) else {}
    cfg = load_config()
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_hagent_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from hagent_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_token_file_path("auth.json")
    if auth_file.exists():
        try:
            import json
    ...
    if isinstance(model_cfg, dict):
        ...
        default_provider = str(_DEFAULT_MODEL_CFG.get("provider") or "").strip()
        default_base_url = str(_DEFAULT_MODEL_CFG.get("base_url") or "").strip()
        if (
            cfg_api_key
            or _model_name
            or (cfg_provider and cfg_provider != default_provider)
            or (cfg_base_url and cfg_base_url != default_base_url)
        ):
            return True

    # Check for Claude Code OAuth credentials (~/.claude/.credentials.json)
    # Only count these if Hagent has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Hagent to use their tokens.
    if _has_hagent_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, hagent_bin
        cli_args: the original CLI arguments (everything after 'hagent')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    hagent_bin = container_info["hagent_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo hagent {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo hagent {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  hagent --tui --resume {target}")
    if title:
        print(f'  hagent --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @hagent/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@hagent" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: HAGENT_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("HAGENT_TUI_DIR")
    if env:
        p = Path(env)
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _hagent_ink_bundle_stale(tui_dir):
        return True
...
def _hagent_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "hagent-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``HAGENT_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("HAGENT_SKIP_NODE_BOOTSTRAP"):
        return

    helper = PROJECT_ROOT.parent / "scripts" / "lib" / "node-bootstrap.sh"
    if not helper.is_file():
        return

    from hagent_constants import get_hagent_home
    hagent_home = os.environ.get("HAGENT_HOME") or str(get_hagent_home())
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HAGENT_HOME": hagent_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(hagent_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (HAGENT_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("HAGENT_NODE")
    ...
    # pre-built dist + node_modules (nix / full HAGENT_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("HAGENT_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("HAGENT_QUIET"):
    ...
    if tui_dev:
        if _hagent_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/hagent-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from hagent_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env_tui_dir = os.environ.get("HAGENT_TUI_DIR")
    if not tui_dir.exists() and not (env_tui_dir and Path(env_tui_dir).exists()):
        if not quiet and not os.environ.get("HAGENT_QUIET"):
            print(
                "Modern TUI bundle not found; falling back to the classic CLI.",
                file=sys.stderr,
            )
        return False
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="hagent-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["HAGENT_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["HAGENT_PYTHON_SRC_ROOT"] = os.environ.get(
        "HAGENT_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("HAGENT_PYTHON", sys.executable)
    env.setdefault("HAGENT_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["HAGENT_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["HAGENT_MODEL"] = model
        env["HAGENT_INFERENCE_MODEL"] = model
    if provider:
        env["HAGENT_TUI_PROVIDER"] = provider
        env["HAGENT_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["HAGENT_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["HAGENT_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["HAGENT_TUI_SKILLS"] = value
    if query:
        env["HAGENT_TUI_QUERY"] = query
    if image:
        env["HAGENT_TUI_IMAGE"] = image
    if checkpoints:
        env["HAGENT_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["HAGENT_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["HAGENT_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["HAGENT_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["HAGENT_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["HAGENT_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["HAGENT_TUI_RESUME"] = resume_session_id
    ...
    try:
        ...
        if not quiet and not os.environ.get("HAGENT_QUIET"):
            print(
                f"Modern TUI path missing ({exc.filename or exc}); falling back to the classic CLI.",
                file=sys.stderr,
            )
        try:
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``HAGENT_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``hagent kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``hagent kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("HAGENT_KANBAN_BOARD"):
        return
    try:
        from hagent_cli.kanban_db import get_current_board

        os.environ["HAGENT_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("HAGENT_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'hagent sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Hagent isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  hagent setup")
        print()

        from hagent_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'hagent setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from hagent_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["HAGENT_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's backend/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["HAGENT_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["HAGENT_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["HAGENT_SESSION_SOURCE"] = args.source
    ...
    # Import and run the classic CLI entrypoint.
    # Ensure we import backend/cli.py (local compatibility shim), not a
    # third-party "cli" package that may exist in site-packages.
    ...
    # Treat empty toolset payloads as "not provided" so classic CLI can
    # resolve the default platform toolsets instead of narrowing to nothing.
    if kwargs.get("toolsets") in ([], "", (), set()):
...
def cmd_gateway(args):
    ...
    from hagent_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from hagent_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Hagent?")
        print()
    ...
    session_dir = get_hagent_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: hagent gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  hagent gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Hagent Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hagent gateway")
        ...
        print()
        print("  Or install as a service: hagent gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'hagent whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from hagent_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``hagent model``) and the setup wizard
    ...
    from hagent_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("HAGENT_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'hagent model' for "
                "available providers, or run 'hagent doctor' to diagnose config "
                "issues."
            )
    ...
    from hagent_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in backend/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from backend/.env if the active provider is not 'custom'.
    ...
    """
    from hagent_cli.config import get_env_value, save_env_value, load_config
...
#
# Hagent uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `hagent model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `hagent model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from hagent_cli.config import load_config, save_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``hagent model`` flow, then route aux tasks to them here.
    """
    from hagent_cli.config import load_config
    from hagent_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from hagent_cli.auth import _prompt_model_selection
    from hagent_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from hagent_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from hagent_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from hagent_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from hagent_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from hagent_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from hagent_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `hagent auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from hagent_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from hagent_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from hagent_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from hagent_cli.auth import _save_model_choice, deactivate_provider
    from hagent_cli.config import get_env_value, load_config, save_config
    ...
    from hagent_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `hagent model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from hagent_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from hagent_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from hagent_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Hagent will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from hagent_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from hagent_cli.config import save_env_value, load_config, save_config
    from hagent_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from hagent_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from hagent_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Hagent turns to `copilot --acp`.")
    print("  Hagent currently starts its own ACP subprocess for each request.")
    print("  Hagent uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set HAGENT_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``hagent setup`` / ``hagent model``.
    ...
    recover from a malformed paste without editing ``backend/.env`` by hand.
    ...
    from hagent_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from hagent_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `hagent setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so hagent knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Hagent typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Hagent, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from hagent_cli.auth import AuthError
        from hagent_cli.models import fetch_lmstudio_models
        ...
        from hagent_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from hagent_constants import display_hagent_home as _dhh_fn

            print(
                f"    Hagent will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from hagent_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'hagent model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Hagent CLI with a provider."""
    from hagent_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from hagent_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from hagent_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from hagent_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from hagent_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from hagent_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``hagent slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: hagent slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `hagent slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from hagent_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from hagent_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from hagent_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from hagent_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from hagent_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from hagent_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from hagent_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Hagent home directory to a zip file."""
    if getattr(args, "quick", False):
        from hagent_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from hagent_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Hagent backup from a zip file."""
    from hagent_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Hagent Agent v{__version__} ({__release_date__})")
    ...
    try:
        from hagent_cli.banner import check_for_updates
        from hagent_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Hagent Agent."""
    _require_tty("uninstall")
    from hagent_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``hagent update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``hagent_cli/web_dist/`` (per vite.config.ts
    outDir: "../hagent_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "hagent_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``hagent update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``hagent web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (hagent web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``hagent dashboard`` processes other than ourselves.

    ``hagent dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``hagent update`` replaces files on disk, the running
    ...
    patterns = [
        "hagent dashboard",
        "hagent_cli.main dashboard",
        "hagent_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "hagent.*dashboard"`) keeps us consistent
            # with `hagent_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `hagent update`.
    ...
    print("  Preview now:  hagent curator run --dry-run")
    print("  Pause it:     hagent curator pause")
    print(
        "  Docs:         https://hagent-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``hagent update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``hagent update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: hagent curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``hagent dashboard`` processes.

    Called at the end of ``hagent update`` (default ``reason``) and also
    from ``hagent dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    hagent dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Hagent Agent by downloading a ZIP archive.
    ...
    try:
        tmp_dir = tempfile.mkdtemp(prefix="hagent-update-")
        zip_path = os.path.join(tmp_dir, f"hagent-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to hagent-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"hagent-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "hagent-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Hagent behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes hagent completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Hagent couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Hagent couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Hagent behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `hagent update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_hagent_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_hagent_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``hagent update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from hagent_constants import get_default_hagent_root

    default_home = get_default_hagent_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``hagent update`` is still progressing even if pip/uv itself is silent.
    """
...
def _hagent_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "hagent.exe",
        scripts_dir / "hagent-gateway.exe",
    ]


def _quarantine_running_hagent_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``hagent.exe``.
    ...
    when ``hagent update`` runs, ``hagent.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``hagent.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next hagent invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _hagent_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_hagent_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``hagent.exe.old.*`` left by prior updates.

    Called early on every hagent invocation. The .old files are unlocked once
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``hagent.exe`` / ``hagent-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_hagent_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_hagent_exe(scripts_dir)
        try:
...
class _UpdateOutputStream:
    """Stream wrapper used during ``hagent update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``hagent update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``hagent update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``hagent update --gateway``) the update is already
    ...
    try:
        # Late-bound import so tests can monkeypatch logging path setup.
        from hagent_constants import get_logs_home as _get_logs_home

        logs_dir = _get_logs_home()
...
def _cmd_update_check():
    """Implement ``hagent update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``hagent update`` without requiring a reinstall.  The
    ...
    ``hagent`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/hagent.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v hagent'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/hagent, code at /usr/local/lib/hagent-agent).
    fhs_link = Path("/usr/local/bin/hagent")
    ...
    path_comment = (
        "# Hagent Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of HAGENT_HOME before running the update.
    ...
    HAGENT_HOME directories.  The ``--backup`` flag on ``hagent update``
    ...
    try:
        from hagent_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_hagent_home so the user sees backend/...
    try:
        from hagent_constants import get_hagent_home, display_hagent_home

        home = get_hagent_home()
        try:
            display_path = f"{display_hagent_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  hagent import {out_path}")
...
def cmd_update(args):
    """Update Hagent Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from hagent_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Hagent Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_hagent_home added to hagent_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload hagent_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_hagent_home() added since the last release.
        try:
            import importlib
            import hagent_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit HAGENT_HOME so
        # it is not affected by sync_skills()'s module-level HAGENT_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's HAGENT_HOME env var points at the default or a named profile.
        try:
            from hagent_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in ("", "y", "yes", "auto"):
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: hagent config migrate")
            else:
                print()
                print("Skipped. Run 'hagent config migrate' later to configure.")
        else:
        ...
        # check `hagent curator status`. Self-stamps after printing so it
        ...
        # When running as ``hagent update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_hagent_home() / ".update_exit_code"
            try:
        ...
        try:
            from hagent_cli.gateway import (
                is_macos,
            ...
            # Discover all hagent-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. hagent-gateway.service or hagent-gateway-coder.service
                            ...
                            # path in `hagent gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: hagent gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: hagent -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Hagent gateway unit files are still installed.
        # When both hagent.service (from a pre-rename install) and the
        # current hagent-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `hagent update` surfaces the issue until the user migrates.
        try:
            from hagent_cli.gateway import (
                has_legacy_hagent_units,
                _find_legacy_hagent_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_hagent_units():
                print()
                print("⚠ Legacy Hagent gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hagent_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (hagent.service) fight the current")
                print("  hagent-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    hagent gateway migrate-legacy")
                print()
        ...
        print("  hagent model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``hagent -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `hagent profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_hagent_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → hagent -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (backend)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  hagent profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     hagent -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `hagent profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from hagent_cli.profiles import profile_exists
        ...
        from hagent_cli.profiles import rename_profile
        ...
        from hagent_cli.profiles import export_profile
        ...
        from hagent_cli.profiles import import_profile
        ...
        import tempfile
        from hagent_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="hagent_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  hagent -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      hagent -p {plan.manifest.name} chat")
        ...
        from hagent_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `hagent profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"hagent -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from hagent_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("hagent_requires"):
            print(f"Requires:     Hagent {data['hagent_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from hagent_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.hagent_requires:
        print(f"  Requires: Hagent {mf.hagent_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``hagent dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``hagent dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No hagent dashboard processes running.")
        return 0

    print(f"{len(pids)} hagent dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No hagent dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `hagent update`.
    ...
    if "HAGENT_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from hagent_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("HAGENT_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from hagent_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Hagent log files."""
    from hagent_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from hagent_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``hagent -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``hagent_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``hagent -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``hagent`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for hagent CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from hagent_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``hagent.exe.old.*`` quarantine files left by previous
    # ``hagent update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_hagent_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from hagent_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from hagent_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://hagent-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `hagent model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy hagent.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Hagent gateway unit files "
            "(e.g. hagent.service) left over from older installs. Profile "
            "units (hagent-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Hagent Agent with an interactive wizard. "
        "Run a specific section: hagent setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'hagent setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Hagent.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $HAGENT_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Hagent")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Hagent CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: hagent-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Hagent with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set HAGENT_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Hagent Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from hagent_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in backend/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at backend/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Hagent Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Hagent setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Hagent Agent. Use 'hagent debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    hagent debug share              Upload debug report and print URL
    hagent debug share --lines 500  Include more log lines
    hagent debug share --expire 30  Keep paste for 30 days
    hagent debug share --local      Print report locally (no upload)
    hagent debug share --no-redact  Disable upload-time secret redaction
    hagent debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'hagent debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Hagent home directory to a zip file",
        description="Create a zip archive of your entire Hagent configuration, "
        "skills, sessions, and data (excludes the hagent-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/hagent-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear backend/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo hagent uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from hagent_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Hagent backup from a zip file",
        description="Extract a previously created Hagent backup into your "
        "Hagent home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Hagent Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from hagent_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (backend/skills/.bundled_manifest) "
            "so future 'hagent update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from hagent_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from hagent_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/hagent-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `hagent plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from hagent_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``hagent --help``, ``hagent version``, ``hagent logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from hagent_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from hagent_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_hagent_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_hagent_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_hagent_home()}/memories/\n")
        else:
            from hagent_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'hagent tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # hagent tools list [--platform cli]
    ...
    # hagent tools disable <name...> [--platform cli]
    ...
    # hagent tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from hagent_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from hagent_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `hagent computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `hagent tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from hagent_cli.tools_config import _run_post_setup
        ...
        if action == "status":
            ...
            print("  Run: hagent computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Hagent as an MCP server",
        description=(
            "Manage MCP server connections and run Hagent as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'hagent mcp add' to connect to a new server, or\n"
            "'hagent mcp serve' to expose Hagent conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Hagent as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `hagent mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from hagent_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_hagent_home() / "sessions"
            ...
            # Launch hagent --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from hagent_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Hagent",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Hagent",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of backend/ (by default a "
        "single restore-point archive is written to backend/backups/ "
        "before apply; restorable with 'hagent import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from hagent_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Hagent Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'hagent config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Hagent Agent",
        description="Remove Hagent Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Hagent Agent as an ACP (Agent Client Protocol) server",
        description="Start Hagent Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Hagent Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Hagent instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `hagent update` skill sync)",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Hagent profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Hagent Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `hagent --tui` via PTY/WebSocket). "
            "Alternatively set HAGENT_DASHBOARD_TUI=1."
        ),
    )
    ...
    # for `hagent dashboard` cmdlines and SIGTERM them directly — the same
    # path `hagent update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running hagent dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running hagent dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Hagent log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    hagent logs                    Show last 50 lines of agent.log
    hagent logs -f                 Follow agent.log in real time
    hagent logs errors             Show last 50 lines of errors.log
    hagent logs gateway -n 100     Show last 100 lines of gateway.log
    hagent logs --level WARNING    Only show WARNING and above
    hagent logs --session abc123   Filter by session ID
    hagent logs --component tools  Only show tool-related lines
    hagent logs --since 1h         Lines from the last hour
    hagent logs --since 30m -f     Follow, starting from 30 min ago
    hagent logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``hagent -c Pokemon Agent Dev`` → ``hagent -c 'Pokemon Agent Dev'``
    ...
    from hagent_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'hagent -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (hagent hooks list, cron
    ...
    if getattr(args, "oneshot", None):
        from hagent_cli.oneshot import run_oneshot
