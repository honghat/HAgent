...
# Falls back to ~/.hagent/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # parent directory name (e.g. ~/.hagent/profiles/coder or
    # /opt/data/profiles/coder).  If HAGENT_HOME points to the hagent root
    # instead (e.g. systemd hardcodes HAGENT_HOME=/root/.hagent), we must
...
load_hagent_dotenv(project_env=PROJECT_ROOT / ".env")
...
def cmd_chat(args):
    ...
    # user's ~/.hagent/config.yaml and return built-in defaults. Set BEFORE
...
def select_provider_and_model(args=None):
    ...
    # a leftover OPENAI_BASE_URL in ~/.hagent/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.hagent/.env if the active provider is not 'custom'.
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.hagent/.env`` by hand.
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.hagent/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.hagent/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # hagent_cli.config.get_hagent_home to simulate setup failure.
        from hagent_cli.config import get_hagent_home as _get_hagent_home

        logs_dir = _get_hagent_home() / "logs"
...
def _run_pre_update_backup(args) -> None:
    ...
    # Render path using display_hagent_home so the user sees ~/.hagent/...
    try:
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.hagent)")
            else:
...
def main():
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.hagent/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.hagent/scripts/. Pass empty string to clear. "
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.hagent/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.hagent/shell-hooks-allowlist.json."
        ),
    )
...
""",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.hagent/checkpoints/",
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.hagent/skills/.bundled_manifest) "
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.hagent/ (by default a "
        "single restore-point archive is written to ~/.hagent/backups/ "
