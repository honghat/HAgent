...
The Hagent docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
gateway Hat Nguyen Portal). This module fetches, validates, and caches that manifest,
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``backend/cache/model_catalog.json``.
...
DEFAULT_CATALOG_URL = (
    "https://hagent-agent.nousresearch.com/docs/api/model-catalog.json"
)
...
def _cache_path() -> Path:
    ...
    return get_hagent_home() / "cache" / "model_catalog.json"
...
def get_curated_nous_models() -> list[str] | None:
    """Returngateway Hat Nguyen Portal's curated list of model ids from the manifest.
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``hagent model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
