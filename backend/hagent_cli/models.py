...
Add, remove, or reorder entries here — both `hagent setup` and
`hagent` provider-selection will pick up the change automatically.
"""
...
_HAGENT_USER_AGENT = f"hagent-cli/{_HAGENT_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `hagent model`
    flow without maintaining a separate static list.
    """
    from hagent_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $HAGENT_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $HAGENT_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Hagent refreshes the cache from
...
# ---------------------------------------------------------------------------
#gateway Hat Nguyen Portal free-model helper
# ---------------------------------------------------------------------------
# Thegateway Hat Nguyen Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
#gateway Hat Nguyen Portal account tier detection
# ---------------------------------------------------------------------------

def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    """Fetch the user'sgateway Hat Nguyen Portal account/subscription info.
...
def partition_nous_models_by_tier(
    ...
    """SplitNous models into (selectable, unavailable) based on user tier.
...
def check_nous_free_tier() -> bool:
    """Check if the currentgateway Hat Nguyen Portal user is on a free (unpaid) tier.
    ...
    try:
        from hagent_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# ---------------------------------------------------------------------------
#gateway Hat Nguyen Portal recommended models
#
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch thegateway Hat Nguyen Portal's curated recommended-models payload.
...
# this list:  hagent model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `hagent model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `hagent model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "gateway Hat Nguyen Portal",              "gateway Hat Nguyen Portal (Nous Research subscription)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``hagent model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``hagent auth add openai-codex`` without
    ``hagent model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    hagent-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (gateway Hat Nguyen Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from hagent_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — hagent-agent
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curatedgateway Hat Nguyen Portal model-id list.
    ...
    """
    try:
        from hagent_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from hagent_constants import AI_GATEWAY_BASE_URL
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter,gateway Hat Nguyen Portal).
    """
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return hagent-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; hagent's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` forgateway Hat Nguyen Portal pricing, or empty strings."""
    try:
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``hagent model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from hagent_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from hagent_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Hagent' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``hagent_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Hagent should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``hagent auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``backend/.env``.
    ...
    """
    try:
        from hagent_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from hagent_cli.auth import read_credential_pool
        from hagent_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Hagent release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Hagent release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        # Try livegateway Hat Nguyen Portal /models endpoint
        try:
            from hagent_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _HAGENT_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Hagent' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _HAGENT_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_hagent_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Hagent will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Hagent cannot verify the model name."
