...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with hagent-agent;
   ...
   own discovery paths)
2. **User plugins**   – ``backend/plugins/<name>/``
3. **Project plugins** – ``./backend/plugins/<name>/`` (opt-in via
...
# stderr in addition to logs/agent.log. Aimed at plugin authors
...
class PluginManifest:
    ...
    #              in backend/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def register_skill(
        ...
        ``backend/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        # 2. User plugins (backend/plugins/)
        ...
        # 3. Project plugins (./backend/plugins/)
        if _env_enabled("HAGENT_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / "backend" / "plugins"
