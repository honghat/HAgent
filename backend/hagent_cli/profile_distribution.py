"""Profile distributions — shareable, packaged Hagent profiles via git.

A distribution is a Hagent profile published as a git repository (or
...
* ``hagent profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``hagent skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``hagent profile``, not a parallel tree):

    hagent profile install <source> [--name N] [--alias] [--force] [--yes]
    hagent profile update  <name>  [--force-config] [--yes]
    hagent profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "hagent-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            hagent_requires=str(data.get("hagent_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.hagent_requires:
            out["hagent_requires"] = self.hagent_requires
        if self.author:
...
def check_hagent_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Hagent {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Hagent distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``hagent profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Hagent profile distribution."
            )
...
def plan_install(
    ...
    from hagent_cli import __version__ as hagent_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Hagent distribution."
        )

    # Version check up-front so we fail fast
    check_hagent_requires(manifest.hagent_requires, hagent_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (backend).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="hagent_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `hagent profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`hagent profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="hagent_dist_update_") as tmp:
        plan = plan_install(
