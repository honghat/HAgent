...
Profiles live under ``backend/profiles/<name>/`` by default.

The "default" profile is ``backend`` itself — backward compatible,
...
# default profile (``backend``).  Named profiles never contain these
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``backend``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (backend) profile.
...
def _get_profiles_root() -> Path:
    ...
    ``backend``, profiles live under ``HAGENT_HOME/profiles/`` so
...
def _get_default_hagent_home() -> Path:
    ...
    In Docker/custom deployments where HAGENT_HOME is outside ``backend``
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (backend)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling backend/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (backend).\n"
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``backend/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    ...
    Returns ``"default"`` if HAGENT_HOME is not set or points to ``backend``.
    Returns the profile name if HAGENT_HOME points into ``backend/profiles/<name>``.
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS backend itself — its parent is ~/ and its
        # directory name is "backend", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target backend itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (backend). "
...
_hagent_profiles() {
    local profiles_dir="${HAGENT_HOME:-backend}/profiles"
