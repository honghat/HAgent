"""
Profile management for multiple isolated Hagent instances.

Each profile is a fully independent HAGENT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.hagent/profiles/<name>/`` by default.

The "default" profile is ``~/.hagent`` itself — backward compatible,
zero migration needed.

Usage::

    hagent profile create coder          # fresh profile + bundled skills
    hagent profile create coder --clone  # also copy config, .env, SOUL.md, skills
    hagent profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    hagent -p coder chat                 # or via flag
    hagent profile use coder             # set as sticky default
    hagent profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See hagent_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.hagent``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   hagent-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `hagent profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `hagent
...
# `hagent skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Hagent infrastructure directories that only the default profile
         (``~/.hagent``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_hagent_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.hagent) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "hagent-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "hagent", "default", "test", "tmp", "root", "sudo",
})

# Hagent subcommands that cannot be used as profile names/aliases
_HAGENT_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hagent root, NOT to the current HAGENT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where HAGENT_HOME points outside
    ``~/.hagent``, profiles live under ``HAGENT_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_hagent_home() / "profiles"


def _get_default_hagent_home() -> Path:
    """Return the default (pre-profile) HAGENT_HOME path.

    In standard deployments this is ``~/.hagent``.
    In Docker/custom deployments where HAGENT_HOME is outside ``~/.hagent``
    (e.g. ``/opt/data``), returns HAGENT_HOME directly.
    """
    from hagent_constants import get_default_hagent_root
    return get_default_hagent_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_hagent_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Hagent installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its HAGENT_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_hagent_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, hagent subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _HAGENT_SUBCOMMANDS:
        return f"'{canon}' conflicts with a hagent subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``hagent profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_hagent_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``hagent update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.hagent)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.hagent/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `hagent update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`hagent profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `hagent update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches HAGENT_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``hagent profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HAGENT_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.hagent).\n"
            "To remove everything, use: hagent uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set HAGENT_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("HAGENT_HOME")
    try:
        os.environ["HAGENT_HOME"] = str(profile_dir)
        from hagent_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["HAGENT_HOME"] = old_home
        elif "HAGENT_HOME" in os.environ:
            del os.environ["HAGENT_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.hagent/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: hagent profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from HAGENT_HOME.

    Returns ``"default"`` if HAGENT_HOME is not set or points to ``~/.hagent``.
    Returns the profile name if HAGENT_HOME points into ``~/.hagent/profiles/<name>``.
    Returns ``"custom"`` if HAGENT_HOME is set to an unrecognized path.
    """
    from hagent_constants import get_hagent_home
    hagent_home = get_hagent_home()
    resolved = hagent_home.resolve()

    default_resolved = _get_default_hagent_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.hagent itself — its parent is ~/ and its
        # directory name is ".hagent", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: hagent profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.hagent itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.hagent). "
            "Specify a different name: hagent profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="hagent_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"hagent.{old_name}"
    new_host = f"hagent.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_hagent_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for hagent profile names."""
    return '''# Hagent Agent profile completion
# Add to ~/.bashrc: eval "$(hagent completion bash)"

_hagent_profiles() {
    local profiles_dir="$HOME/.hagent/profiles"
...
_hagent_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_hagent_profiles)" -- "$cur"))
        return
    fi
...
complete -F _hagent_completion hagent
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for hagent profile names."""
    return '''#compdef hagent
# Hagent Agent profile completion
# Add to ~/.zshrc: eval "$(hagent completion zsh)"

_hagent() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.hagent/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.hagent/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a HAGENT_HOME path string.

    Called early in the CLI entry point, before any hagent modules
    are imported, to set the HAGENT_HOME environment variable.
    """
