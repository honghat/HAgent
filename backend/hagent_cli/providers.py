"""
Single source of truth for provider identity in Hagent Agent.
...
2. **Hagent overlays** — transport type, auth patterns, aggregator flags,
...
# -- Hagent overlay ----------------------------------------------------------
# Hagent-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HagentOverlay:
    """Hagent-specific provider metadata layered on top of models.dev."""
...
HAGENT_OVERLAYS: Dict[str, HagentOverlay] = {
    "openrouter": HagentOverlay(
    ...
    "openai-codex": HagentOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": HagentOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="HAGENT_QWEN_BASE_URL",
    ),
    "google-gemini-cli": HagentOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": HagentOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": HagentOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": HagentOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": HagentOverlay(
    ...
    "kimi-for-coding": HagentOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": HagentOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": HagentOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": HagentOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": HagentOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": HagentOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": HagentOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": HagentOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": HagentOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": HagentOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": HagentOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": HagentOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": HagentOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": HagentOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "hagent", "user-config"
...
_LABEL_OVERRIDES: Dict[str, str] = {
    "nous": "gateway Hat Nguyen Portal",
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Hagent overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Hagent overlay
    ...
    overlay = HAGENT_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + hagent extra
    ...
    if overlay is not None:
        # Hagent-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in HAGENT_OVERLAYS
    if provider == "bedrock":
