...
from hagent_cli import auth as auth_mod
...
from hagent_cli.config import get_compatible_custom_providers, load_config
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from hagent_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from hagent_cli.models import azure_foundry_model_api_mode
        ...
        if provider in ("opencode-zen", "opencode-go"):
            ...
            # Refs #16878.
            from hagent_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("HAGENT_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            if (canonical or "").strip().lower() == requested_norm:
                # Don't shadow if the user has explicitly configured this provider
                # in the ``providers:`` dict with an API key.  An explicit entry
                # signals intent to override the built-in.
                _cfg = load_config()
                ...
                if isinstance(_providers, dict):
                    for _ep_name, _entry in _providers.items():
                        if not isinstance(_entry, dict):
                            continue
                        _name_norm = _normalize_custom_provider_name(_ep_name)
                        if requested_norm in {_ep_name, _name_norm}:
                            if str(_entry.get("api_key", "") or "").strip():
                                break
                    else:
                        return None
                else:
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'hagent model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "$HAGENT_HOME/.env or run 'hagent model' to configure."
        )
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # ForNous, the pool entry's runtime_api_key is the agent_key — a
        # short-lived inference credential (~30 min TTL).  The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `hagent auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_nous_runtime_credentials() which handles refresh + mint.
        if provider == "nous" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("HAGENT_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
            nous_state = {
    ...
    if provider == "nous":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detectedNous but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detectedNous provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "hagent-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from hagent_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Hagent canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Hagent-compatible importers).
