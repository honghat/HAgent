"""
Interactive setup wizard for Hagent Agent.
...
_DOCS_BASE = "https://hagent-agent.nousresearch.com/docs"
...
# display_hagent_home imported lazily at call sites (stale-module safety during hagent update)
...
from hagent_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Hagent Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Hagent using environment variables or config commands:")
    print_info("  hagent config set model.provider custom")
    print_info("  hagent config set model.base_url http://localhost:8080/v1")
    print_info("  hagent config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'hagent setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from hagent_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from hagent_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'hagent setup')")


def _print_setup_summary(config: dict, hagent_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'hagent setup' to configure"))
    ...
    # Image generation — FAL (direct or viaNous), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from hagent_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI viaNous subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'hagent setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'hagent setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'hagent setup terminal'"))
    elif managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional viaNous subscription)", True, None))
    ...
    # Spotify (OAuth via hagent auth spotify — check auth.json, not env vars)
    try:
        from hagent_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'hagent setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {hagent_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('hagent setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('hagent setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('hagent setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('hagent setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('hagent setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('hagent config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('hagent config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('hagent config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('hagent', Colors.GREEN)}              Start chatting")
    print(f"   {color('hagent gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('hagent doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `hagent tools` and `hagent setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``hagent model``)
    ...
    provider added to ``hagent model`` is automatically available here.
    ...
    # Delegate to the shared hagent model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from hagent_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: hagent model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from hagent_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Hagent can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        _prov_names = {
            "nous-api": "gateway Hat Nguyen Portal API key",
        ...
        print_info("doesn't currently provide one Hagent can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'hagent setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("gateway Hat Nguyen Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    if selected == "nous-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managedNous gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from backend/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"hagent setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'hagent setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Hagent runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managedNous gateway and bill to your subscription.")
        ...
        print_info("Requires the optional SDK: pip install 'hagent-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'hagent-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into HAGENT_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("HAGENT_MAX_ITERATIONS")
    ...
    print_info("  Run `hagent setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # HAGENT_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Hagent delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `hagent update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://hagent-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under HAGENT_HOME, and print
    ...
    """
    try:
        from hagent_cli.slack_cli import _build_full_manifest
        from hagent_constants import get_hagent_home

        manifest = _build_full_manifest(
            bot_name="Hagent",
            bot_description="Your Hagent agent on Slack",
        )
        target = Path(get_hagent_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `hagent slack manifest --write` anytime to refresh after "
            "Hagent adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Hagent delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Hagent delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  hagent config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Hagent to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from hagent_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://hagent-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://hagent-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  hagent config edit")
    print_info("   Open config in your editor:  hagent config edit")
...
def setup_gateway(config: dict):
    ...
    from hagent_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Hagent from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'hagent setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     hagent config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_hagent_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  hagent gateway")
        else:
            from hagent_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   hagent gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   hagent gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `hagent setup tools` and `hagent tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``hagent_cli.auth`` — lists every supported
    ...
    """
    try:
        from hagent_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from hagent_cli.config
    ...
    if section_key == "model":
        ...
        from hagent_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_hagent.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_hagent migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_hagent", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Hagent.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Hagent to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Hagent at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Hagent at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Hagent at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Hagent at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Hagent equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Hagent config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Hagent.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Hagent's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(hagent_home: Path) -> bool:
    ...
    print_info("Hagent can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: hagent claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=hagent_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: hagent claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Hagent configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Hagent config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hagent (use hagent claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      hagent setup           — full or quick (auto-detected)
      hagent setup model     — just model/provider
      hagent setup tts       — just text-to-speech
      hagent setup terminal  — just terminal backend
      hagent setup gateway   — just messaging platforms
      hagent setup tools     — just tool configuration
      hagent setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Hagent Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Hagent Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Hagent Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, hagent_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Hagent configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'hagent setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(hagent_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Hagent?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, hagent_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {hagent_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'hagent config edit'")
    ...
    _print_setup_summary(config, hagent_home)
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch hagent chat now?", True):
        return
...
def _run_first_time_quick_setup(config: dict, hagent_home, is_existing: bool):
    ...
    the user can customize later via ``hagent setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'hagent setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    hagent setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  hagent setup gateway")
    print()
...
def _run_quick_setup(config: dict, hagent_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'hagent setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Hagent to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'hagent setup gateway'.")
