"""
Status command for hagent CLI.

Shows the status of all Hagent Agent components.
"""
...
from hagent_cli.auth import AuthError, resolve_provider
from hagent_cli.colors import Colors, color
from hagent_cli.config import get_env_path, get_env_value, get_hagent_home, load_config
from hagent_cli.models import provider_label
from hagent_cli.nous_subscription import get_nous_subscription_features
from hagent_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``hagent config``'s
...
def show_status(args):
    """Show status of all Hagent Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Hagent Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: hagent model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: hagent auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: hagent model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from hagent_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'hagent-agent[vercel]')"
    ...
    try:
        from hagent_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   hagent gateway")
    ...
    jobs_file = get_hagent_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_hagent_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'hagent doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'hagent setup' to configure", Colors.DIM))
    print()
