...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or hagent -c.",
    ...
    "Ctrl+Z suspends Hagent to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "hagent -c resumes your most recent CLI session. hagent -c \"project name\" resumes by title.",
    "hagent -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "hagent -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "hagent chat -t web,terminal enables only specific toolsets for a focused session.",
    "hagent chat -s github-pr-workflow preloads a skill at launch.",
    "hagent chat -q \"query\" runs a single non-interactive query and exits.",
    "hagent chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "hagent chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hagent --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hagent chat --source telegram tags the session for filtering in hagent sessions list.",
    "hagent -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "hagent doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hagent dump outputs a compact setup summary — great for bug reports.",
    "hagent config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "hagent config edit opens config.yaml in your default editor.",
    "hagent config check scans for missing or stale configuration options.",
    "hagent sessions browse opens an interactive session picker with search.",
    "hagent sessions stats shows session counts by platform and database size.",
    "hagent sessions prune --older-than 30 cleans up old sessions.",
    "hagent skills search react --source skills-sh searches the skills.sh public directory.",
    "hagent skills check scans installed hub skills for upstream updates.",
    "hagent skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "hagent skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hagent mcp add github --command npx adds MCP servers from the command line.",
    "hagent mcp serve runs Hagent itself as an MCP server for other agents.",
    "hagent auth add lets you add multiple API keys for credential pool rotation.",
    "hagent completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "hagent logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "hagent backup creates a zip backup of your entire Hagent home directory.",
    "hagent profile create coder creates an isolated profile that becomes its own command.",
    "hagent profile create work --clone copies your current config and keys to a new profile.",
    "hagent update syncs new bundled skills to ALL profiles automatically.",
    "hagent gateway install sets up Hagent as a system service (systemd/launchd).",
    "hagent memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hagent webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: hagent tools disables unused tools, hagent skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "hagent models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as hagent -w).",
    ...
    "HAGENT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Hagent tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'hagent profile create coder' creates the 'coder' command.",
    "hagent profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Hagent prints a resume command with session ID and stats.",
    "hagent sessions export backup.jsonl exports all sessions for backup or analysis.",
    "hagent -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "hagent skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under backend/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: hagent cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Hagent runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "hagent gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Hagent via a local macOS server.",
    ...
    "Context files (.hagent.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at $HAGENT_HOME/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Hagent loads project context from .hagent.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — hagent tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in $HAGENT_HOME/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "hagent chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "hagent plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "hagent acp runs Hagent as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "Cron scripts live in repo-root scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Hagent your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit $HAGENT_HOME/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Hagent auto-flushes important facts to memory in the background.",
    ...
    "agent.api_max_retries (default 5) controls how many times the agent retries a failed API call before surfacing the error — lower it for fast fallback.",
    ...
    "The skills audit log at $HAGENT_HOME/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at HAGENT_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "HAGENT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in HAGENT_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to $HAGENT_HOME/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'hagent profile create ls' would be rejected.",
    "hagent profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "hagent claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from $HAGENT_HOME/.env.",
    "Skills from trusted repos (HatNguyen) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at $HAGENT_HOME/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Hagent auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Hagent config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Hagent is working.',
    ...
    '/reload reloads $HAGENT_HOME/.env into the running session — pick up new API keys without restarting.',
    ...
    "HAGENT_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under $HAGENT_HOME/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a $HAGENT_HOME/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'hagent curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "hagent curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'hagent curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    ...
    # --- TUI & Dashboard ---
    'HAGENT_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "HAGENT_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'hagent dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'hagent dashboard --tui embeds the full Hagent TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in $HAGENT_HOME/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in $HAGENT_HOME/dashboard-plugins/ — no npm build required.',
    ...
    'HAGENT_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'HAGENT_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'HAGENT_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'HAGENT_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to $HAGENT_HOME/cache/piper-voices/.',
    ...
    '/reload-skills re-scans $HAGENT_HOME/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hagent -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'hagent chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'hagent chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'hagent chat --ignore-user-config skips $HAGENT_HOME/config.yaml — reproducible bug reports and CI runs.',
    "hagent chat --source tool tags programmatic chats so they don't clutter hagent sessions list.",
    'hagent dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'hagent sessions rename <ID> "new title" renames any past session; hagent sessions delete <ID> removes one.',
    'hagent import restores a session export or profile archive produced by sessions export or profile export.',
    'hagent fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'hagent pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'hagent setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'hagent status --deep runs the full health sweep across every component; plain hagent status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'HAGENT_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'HAGENT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./backend/plugins/ — trust-gated by design.',
    "HAGENT_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'HAGENT_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'HAGENT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'HAGENT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'HAGENT_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'HAGENT_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'HAGENT_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'HAGENT_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'HAGENT_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'HAGENT_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'HAGENT_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Hagent block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from hagent sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into $HAGENT_HOME/dashboard-plugins/.',
]
