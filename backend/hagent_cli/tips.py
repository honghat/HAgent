...
TIPS = [
    ...
    "The plan skill saves markdown plans under .hagent/plans/ in the active workspace.",
    ...
    "SOUL.md at ~/.hagent/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    ...
    "Checkpoints use shadow repos in ~/.hagent/checkpoints/ — your project's .git is never touched.",
    ...
    "Cron scripts live in ~/.hagent/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.hagent/SOUL.md to customize.",
    ...
    "The skills audit log at ~/.hagent/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.hagent/interrupt_debug.log with timestamps.",
    ...
    '/reload reloads ~/.hagent/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.hagent/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.hagent/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    'Drop a YAML in ~/.hagent/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.hagent/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.hagent/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.hagent/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hagent chat --ignore-user-config skips ~/.hagent/config.yaml — reproducible bug reports and CI runs.',
    ...
    'HAGENT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hagent/plugins/ — trust-gated by design.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.hagent/dashboard-plugins/.',
]
