"""
Unified tool configuration for Hagent Agent.

`hagent tools` and `hagent setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to $HAGENT_HOME/config.yaml under
...
from hagent_cli.nous_subscription import (
...
from hagent_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _HAGENT_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl", "spotify", "discord", "discord_admin", "video"}

# Platform-scoped toolsets: only appear in the `hagent tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``hagent tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from hagent_cli.plugins import discover_plugins, get_plugin_toolsets
...
from hagent_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. HAGENT_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "HAGENT_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "HAGENT_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "HAGENT_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "HAGENT_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "HAGENT_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hagent_home()}/hagent-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in hagent-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/hagent-agent:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0 and shutil.which("cua-driver"):
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Hagent process.")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in $HAGENT_HOME/config.yaml")
        ...
        # Run the full `hagent auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to $HAGENT_HOME/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from hagent_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: hagent auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `hagent auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: hagent auth spotify")
        except Exception as exc:
        ...
        try:
            from hagent_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: hagent plugins enable observability/langfuse")
        _print_info("    Restart Hagent for tracing to take effect.")
        _print_info("    Verify: hagent plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "hagent-cli" (which include all _HAGENT_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[hagent-cli, spotify]`` after enabling Spotify via ``hagent
        ...
        # (e.g. "hagent-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
    ...
    # otherwise saving via `hagent tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"hagent-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("hagent-")}
    ...
    # shipped as a bundled plugin but user must opt in via `hagent tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `hagent tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by hagent tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (hagent-cli, hagent-telegram, etc.)
    ...
    # Opening `hagent tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `hagent tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `hagent tools` and `hagent setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Hagent Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://hagent-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from hagent_constants import display_hagent_home
    print(color(f"  Tool configuration saved to {display_hagent_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'hagent' or gateway restart.", Colors.DIM))
    print()
