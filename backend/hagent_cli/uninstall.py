"""
Hagent Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep $HAGENT_HOME/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Hagent PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing hagent-agent or hagent PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Hagent Agent" comment and following line
                if '# Hagent Agent' in line or '# hagent-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('hagent' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hagent
                if 'hagent' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the hagent wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "hagent",
        Path("/usr/local/bin/hagent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains hagent_cli reference)
                content = wrapper.read_text()
                if 'hagent_cli' in content or 'hagent-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``hagent gateway run`` processes
    ...
    try:
        from hagent_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from hagent_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``HAGENT_HOME`` and ``HAGENT_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\hagent\git\cmd``, ``%LOCALAPPDATA%\hagent\git\bin``,
#      ``%LOCALAPPDATA%\hagent\git\usr\bin``, ``%LOCALAPPDATA%\hagent\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\hagent\git\`` and Node to
#      ``%LOCALAPPDATA%\hagent\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``hagent dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\hagent\gateway-service\`` and sometimes
...
def _hagent_path_markers(hagent_home: Path) -> list[str]:
    """Path-entry substrings that identify Hagent-owned User-PATH entries."""
    root = str(hagent_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare hagent-agent install dir.
    markers = [root + "\\hagent-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if HAGENT_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "hagent".  We don't want to catch
    # unrelated entries like "chagent-foo" or "ephermeral", so we look for
    # backslash-hagent as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(hagent_home: Path) -> list[str]:
    """Strip Hagent-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hagent_path_markers(hagent_home)
            kept: list[str] = []
...
def remove_hagent_env_vars_windows() -> list[str]:
    """Delete HAGENT_HOME and HAGENT_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("HAGENT_HOME", "HAGENT_GIT_BASH_PATH"):
                try:
...
def _is_default_hagent_home(hagent_home: Path) -> bool:
    """Return True when ``hagent_home`` points at the default (non-profile) root."""
    try:
        from hagent_constants import get_default_hagent_root
        return hagent_home.resolve() == get_default_hagent_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its HAGENT_HOME directory.

    We shell out to ``hagent -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current HAGENT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m hagent_cli.main` so we don't depend on a `hagent`
    #    wrapper that may be half-removed mid-uninstall.
    hagent_invocation = [_sys.executable, "-m", "hagent_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                hagent_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's HAGENT_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + $HAGENT_HOME/ (configs, data, logs)
    - Keep data: removes code but keeps $HAGENT_HOME/ for future reinstall
    """
    project_root = get_project_root()
    hagent_home = get_hagent_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie HAGENT_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_hagent_home(hagent_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Hagent Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {hagent_home / 'config.yaml'}")
    print(f"  Secrets: {hagent_home / '.env'}")
    print(f"  Data:    {hagent_home / 'cron/'}, {hagent_home / 'sessions/'}, {hagent_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their HAGENT_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Hagent data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Hagent code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in hagent_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\hagent\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(hagent_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Hagent-owned PATH entries in User environment")

        log_info("Removing HAGENT_HOME / HAGENT_GIT_BASH_PATH User env vars...")
        removed_env = remove_hagent_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Hagent-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing hagent command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside $HAGENT_HOME/, just remove the hagent-agent subdir
            if hagent_home in project_root.parents or project_root.parent == hagent_home:
    ...
    #     under HAGENT_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(hagent_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(hagent_home)
    ...
    # 5. Optionally remove $HAGENT_HOME/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile HAGENT_HOME dirs live under
        ...
        try:
            if hagent_home.exists():
                shutil.rmtree(hagent_home)
                log_success(f"Removed {hagent_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {hagent_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {hagent_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/HatNguyen/hagent-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/HatNguyen/hagent-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Hagent Agent! ⚕")
    print()
