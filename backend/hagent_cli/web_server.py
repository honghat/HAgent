...
# streamed to a per-action log file under ``~/.hagent/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_hagent_home() / "logs"
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.hagent/config.yaml`` — applies to **new** sessions only.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hagent/.anthropic_oauth.json`` — Hagent-managed PKCE flow
...
#          → persists to ~/.hagent/.anthropic_oauth.json AND credential pool
...
async def get_logs(
    ...
    log_path = get_hagent_home() / "logs" / log_name
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.hagent/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.hagent/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.hagent/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.hagent/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.hagent/plugins/  (only if HAGENT_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    if os.environ.get("HAGENT_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".hagent" / "plugins", "project"))
