"""hagent webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    hagent webhook subscribe <name> [options]
    hagent webhook list
    hagent webhook remove <name>
    hagent webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to backend/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _hagent_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: hagent gateway run
"""
...
def webhook_command(args):
    """Entry point for 'hagent webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: hagent webhook {subscribe|list|remove|test}")
        print("Run 'hagent webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (hagent gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: hagent webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from hagent webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (hagent gateway run)")
