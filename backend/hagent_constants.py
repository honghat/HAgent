...
def get_hagent_home() -> Path:
    ...
    ``~/.hagent`` — because raising here would brick 30+ module-level
...
def get_default_hagent_root() -> Path:
    ...
    ``~/.hagent`` (e.g. ``/opt/data``), returns ``HAGENT_HOME`` directly
    — that IS the root.

    In profile mode where ``HAGENT_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.hagent/profiles/coder``) and Docker
    ...
    try:
        ...
        # HAGENT_HOME is under ~/.hagent (normal or profile mode)
        return native_home
    except ValueError:
        pass
...
def display_hagent_home() -> str:
    ...
    ``~/.hagent``.  For code that needs a real ``Path``, use
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_hagent_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, hagent_logging.py, hagent_time.py, etc.).
    """
...
def _guard_hermes_path(path: Union[str, Path]) -> None:
    ...
    try:
        ...
        pass  # path is not under ~/.hermes — OK


def _patched_makedirs(name, mode=0o777, exist_ok=False):
    _guard_hermes_path(name)
