...
Configuration is read from backend/config.yaml under the ``mcp_servers`` key.
...
# centralized log file (logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            try:
                ...
                hagent_home = os.getenv("HAGENT_HOME", "").strip()
            candidates = [
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under backend/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``backend/.env`` loaded at startup).
    """
    try:
