...
from tools.registry import discover_builtin_tools, get_agent_mode, registry
...
#   - cli.py, hagent_cli/*      -> inline on startup (no event loop)
...
def _resolve_compatible_toolset(toolset_name: str) -> tuple[list[str], str] | tuple[None, None]:
    """Resolve toolset names across legacy/new aliases.

    Supports:
    - exact registry key
    - hyphen->underscore normalized key
    - "<name>_tools" fallback for short UI keys (web, browser, file, ...)
    - legacy hardcoded map
    """
    raw = str(toolset_name or "").strip()
    if not raw:
        return None, None
    ...
    if not normalized.endswith("_tools"):
        candidates.append(f"{normalized}_tools")

    for candidate in candidates:
        if validate_toolset(candidate):
            return resolve_toolset(candidate), candidate
        if candidate in _LEGACY_TOOLSET_MAP:
            return list(_LEGACY_TOOLSET_MAP[candidate]), candidate
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from hagent_cli.config import get_config_path
...
def _compute_tool_definitions(
    ...
    # Treat an explicit empty list the same as "not provided" so callers
    # that accidentally pass [] (e.g. pre-discovery platform mapping) don't
    # end up with zero tools.
    if enabled_toolsets is not None and len(enabled_toolsets) == 0:
    ...
    if enabled_toolsets is not None:
        for toolset_name in enabled_toolsets:
            resolved, resolved_name = _resolve_compatible_toolset(toolset_name)
            if resolved is not None:
                tools_to_include.update(resolved)
                if not quiet_mode:
                    shown = toolset_name if resolved_name == toolset_name else f"{toolset_name} -> {resolved_name}"
                    print(f"✅ Enabled toolset '{shown}': {', '.join(resolved) if resolved else 'no tools'}")
            else:
                if not quiet_mode:
        ...
        if all_toolsets:
            ...
            # Safety fallback: some environments may have registered tools
            # without a populated toolset map yet. In that case, include all
            # registered tool names instead of returning an empty tool list.
            tools_to_include.update(registry.get_all_tool_names())

    # Always apply disabled toolsets as a subtraction step at the end.
    # This ensures that even if a composite toolset (like hagent-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            ...
            if resolved is not None:
                ...
                if not quiet_mode:
                    shown = toolset_name if resolved_name == toolset_name else f"{toolset_name} -> {resolved_name}"
                    print(f"🚫 Disabled toolset '{shown}': {', '.join(resolved) if resolved else 'no tools'}")
            else:
                if not quiet_mode:
                    print(f"⚠️  Unknown toolset: {toolset_name}")
    ...
    # Ensure mandatory tools are always included unless they are explicitly disabled
    mandatory_tools = {
        "get_gold_price",
        "get_silver_price",
        "vietcombank_rate",
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            resolved, _ = _resolve_compatible_toolset(toolset_name)
            if resolved is not None:
                disabled_tools.update(resolved)

    final_tools = (tools_to_include | mandatory_tools) - disabled_tools
    filtered_tools = registry.get_definitions(final_tools, quiet=quiet_mode)
