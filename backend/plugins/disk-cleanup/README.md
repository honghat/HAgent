...
Auto-tracks and cleans up ephemeral files created during Hagent Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$HAGENT_HOME` and `/tmp/hagent-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HAGENT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under HAGENT_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `HAGENT_HOME` or `/tmp/hagent-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$HAGENT_HOME/disk-cleanup/` is itself excluded
- `$HAGENT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
