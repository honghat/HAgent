...
session-scoped state file under ``$HAGENT_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $HAGENT_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_hagent_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this hagent install, leave it first —
    ...
    env = os.environ.copy()
    env["HAGENT_MEET_URL"] = url
    env["HAGENT_MEET_OUT_DIR"] = str(out)
    env["HAGENT_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["HAGENT_MEET_HEADED"] = "1"
    if auth_state:
        env["HAGENT_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["HAGENT_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if HAGENT_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["HAGENT_MEET_MODE"] = mode
    if realtime_model:
        env["HAGENT_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["HAGENT_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["HAGENT_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["HAGENT_MEET_REALTIME_KEY"] = realtime_api_key
