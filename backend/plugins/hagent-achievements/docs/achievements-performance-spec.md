# Hagent Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: hagent-achievements plugin
...
### `GET /api/plugins/hagent-achievements/achievements`
...
### `POST /api/plugins/hagent-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/hagent-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/hagent-achievements/overview`
...
- Persist plugin data under:
  - `backend/plugins/hagent-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `backend/plugins/hagent-achievements/scan_checkpoint.json`
...
- `backend/plugins/hagent-achievements/`
