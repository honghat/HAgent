...
```bash
hagent memory setup    # select "byterover"
```

Or manually:
```bash
hagent config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> backend/.env
```
...
Working directory: `$HAGENT_HOME/byterover/` (profile-scoped).
