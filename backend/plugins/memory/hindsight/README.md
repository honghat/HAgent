...
```bash
hagent memory setup    # select "hindsight"
```
...
```bash
hagent config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> backend/.env
```
...
Hagent spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `backend/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p hagent ui start
```
...
Config file: `backend/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `hagent` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `hagent-{profile}` isolates memory per active Hagent profile. Empty placeholders collapse cleanly (e.g. `hagent-{user}` with no user becomes `hagent`). |
...
| `retain_context` | `conversation between Hagent Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `backend/.env` as `HINDSIGHT_LLM_API_KEY`.
