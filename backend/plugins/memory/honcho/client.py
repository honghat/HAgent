...
Resolution order for config file:
  1. $HAGENT_HOME/honcho.json  (instance-local, enables isolated Hagent instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Hagent profile.

    Resolution order:
      1. HAGENT_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``hagent.<profile>``
      3. Fallback: ``"hagent"`` (default profile)
    """
    explicit = os.environ.get("HAGENT_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $HAGENT_HOME/honcho.json      (profile-local, if it exists)
      2. backend/honcho.json            (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_hagent_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Hagent system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.hagent
    ...
    def from_global_config(
        ...
        Resolution: $HAGENT_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Hagent profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.hagent block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Hagent session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Hagent session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Hagent session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'hagent honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            hagent_cfg = load_config()
            honcho_cfg = hagent_cfg.get("honcho", {})
