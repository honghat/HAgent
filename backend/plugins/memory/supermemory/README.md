...
```bash
hagent memory setup    # select "supermemory"
```

Or manually:

```bash
hagent config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> backend/.env
```

## Config

Config file: `$HAGENT_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `hagent` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `hagent-{identity}` → `hagent-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Hagent profile:

```json
{
  "container_tag": "hagent-{identity}"
}
```

For a profile named `coder`, this resolves to `hagent-coder`. The default profile resolves to `hagent-default`. Without `{identity}`, all profiles share the same container.
