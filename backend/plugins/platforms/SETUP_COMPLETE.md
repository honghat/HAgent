...
```
backend/plugins/platforms/zalo/
...
```
backend/plugins/platforms/facebook/
...
└── quick-check.md          # Quick platform status guide

backend/plugins/platforms/
...
```
/Users/nguyenhat/HAgent/backend/config.yaml
...
python hagent_cli/main.py gateway run --replace \
  --platform zalo_qr \
  --qr-path backend/qrs/zalo_qr.png
```
