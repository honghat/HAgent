"""
IRC Platform Adapter for Hagent Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Hagent agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "hagent-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Hagent Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Hagent Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: hagent_, hagent_1, hagent_2...
...
def check_requirements() -> bool:
    ...
    # hagent setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `hagent gateway setup` flow for the IRC platform.

    Lazy-imports ``hagent_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Hagent to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #hagent — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.hagent/.env")
    print_info("Restart the gateway for changes to take effect: hagent gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "hagent-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "hagent-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Hagent Agent (cron)")
...
def register(ctx):
    """Plugin entry point: called by the Hagent plugin system."""
