...
class Config:
    ...
    API_PORT = int(_env_value("OMNICHANNEL_API_PORT", "8080"))
    HOST = _env_value("OMNICHANNEL_HOST", "127.0.0.1")
    ENABLED = _env_value("OMNICHANNEL_ENABLED", "true").lower() == "true"
    API_TOKEN = _env_value("OMNICHANNEL_API_TOKEN", "")
    
    # Zalo configuration
    ZALO_COOKIE_STRING = _env_value("ZALO_COOKIE_STRING", "")
    ZALO_QR_ENABLED = _env_value("ZALO_QR_ENABLED", "true").lower() == "true"
    ZALO_BOT_UID = _env_value("ZALO_BOT_UID", "")
    ZALO_IMEI = _env_value("ZALO_IMEI", "")
    
    # Facebook configuration  
    FACEBOOK_COOKIE_STRING = _env_value("FACEBOOK_COOKIE_STRING", "")
    FACEBOOK_HEADLESS = _env_value("FACEBOOK_HEADLESS", "false").lower() == "true"
    
    # Session storage
    SESSION_FILE = Path(_env_value(
        "OMNICHANNEL_SESSION_FILE",
        str(_hagent_home() / "omnichannel_sessions.json"),
    )).expanduser()
    SECRET_FILE = Path(_env_value(
        "OMNICHANNEL_SECRET_FILE",
        str(_hagent_home() / "plugins" / "platforms" / "omnichannel" / "config.env"),
    )).expanduser()
...
def _is_authorized(request: Request) -> bool:
    ...
    return bool(Config.API_TOKEN) and hmac.compare_digest(auth.encode(), expected.encode())
...
async def require_api_token(request: Request, call_next):
    ...
    if not Config.API_TOKEN:
        return JSONResponse(
            status_code=503,
            content={"detail": "OMNICHANNEL_API_TOKEN is required for protected endpoints"},
        )
...
def _persist_runtime_secret(key: str, value: str) -> None:
    import re

    Config.SECRET_FILE.parent.mkdir(parents=True, exist_ok=True)
    env_content = Config.SECRET_FILE.read_text() if Config.SECRET_FILE.exists() else ""
    line = f"{key}={_quote_env_value(value)}"
    if re.search(rf"^{re.escape(key)}=.*$", env_content, flags=re.MULTILINE):
        env_content = re.sub(rf"^{re.escape(key)}=.*$", line, env_content, flags=re.MULTILINE)
    else:
        env_content = (env_content.rstrip() + "\n" + line).lstrip()
    Config.SECRET_FILE.write_text(env_content.rstrip() + "\n")
    try:
        Config.SECRET_FILE.chmod(0o600)
    except OSError:
        logger.warning("Could not chmod runtime secret file: %s", Config.SECRET_FILE)
...
async def poll_qr_scan_status(session_id: str):
    ...
    try:
        ...
        _persist_runtime_secret("ZALO_COOKIE_STRING", cookie_str)
        _persist_runtime_secret("ZALO_IMEI", imei)
