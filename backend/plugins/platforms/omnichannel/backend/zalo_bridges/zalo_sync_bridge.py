...
def reaction_content_from_message(msg):
    if not isinstance(msg, dict):
        return ""
    content = pick(msg, "content", "text", "message", "body")
    if isinstance(content, str) and content.strip().startswith("{"):
        return content.strip()
    if isinstance(content, dict):
        return json.dumps(content, ensure_ascii=False)
    emoji = pick(msg, "rIcon", "emoji", "reaction", "icon")
    targets = pick(msg, "rMsg", "reactionMsgs", "messages")
    if emoji and isinstance(targets, list):
        return json.dumps({"rIcon": emoji, "rMsg": targets}, ensure_ascii=False)
    return ""
...
def normalize_message(msg, fallback_thread_id="", own_id=""):
    if not isinstance(msg, dict):
        return None
    msg_type = str(pick(msg, "msgType", "type") or "").lower()
    if msg_type == "chat.undo":
        return None
    content = reaction_content_from_message(msg) if msg_type == "chat.reaction" else text_from_message(msg).strip()
    if not content:
        return None
