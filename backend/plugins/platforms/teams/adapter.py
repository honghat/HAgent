"""
Microsoft Teams platform adapter for Hagent Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Hagent"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        hagent_action = data.get("hagent_action", "")
        session_key = data.get("session_key", "")

        if not hagent_action or not session_key:
        ...
        choice = choice_map.get(hagent_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="hagent_approve",
                    data={**btn_data_base, "hagent_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="hagent_approve",
                    data={**btn_data_base, "hagent_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="hagent_approve",
                    data={**btn_data_base, "hagent_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="hagent_approve",
                    data={**btn_data_base, "hagent_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Hagent\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.hagent/.env")
