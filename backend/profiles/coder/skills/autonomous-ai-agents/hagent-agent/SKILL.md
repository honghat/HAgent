...
cp ~/.hagent/gateway-autostart.plist ~/Library/LaunchAgents/com.nguyenhat.hagent-gateway.plist
launchctl load ~/Library/LaunchAgents/com.nguyenhat.hagent-gateway.plist
```

**LaunchDaemons (with sudo):**
```bash
...
Sessions are the persistent state of an agent's work. They're stored in SQLite (`~/.hagent/agent.db`) and can be resumed across invocations or platform restarts.
...
```
hagent auth add             Interactive credential wizard
...
/reload-skills       Re-scan ~/.hagent/skills/ for added/removed skills
...
```
~/.hagent/config.yaml       Main configuration
~/.hagent/.env              API keys and secrets
$HAGENT_HOME/skills/        Installed skills
~/.hagent/sessions/         Session transcripts
~/.hagent/logs/             Gateway and error logs
~/.hagent/auth.json         OAuth tokens and credential pools
~/.hagent/hagent-agent/     Source code (if git-installed)
```

Profiles use `~/.hagent/profiles/<name>/` with the same layout.
...
Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.hagent/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.
...
- **Telemetry:** sidecar at `~/.hagent/skills/.usage.json` holds
...
Check logs first:
```bash
grep -i "failed to send\|error" ~/.hagent/logs/gateway.log | tail -20
```
...
Config: `~/.hagent/config.yaml` (settings), `~/.hagent/.env` (API keys).
...
All handlers must return JSON strings. Use `get_hagent_home()` for paths, never hardcode `~/.hagent`.
...
- Tests auto-redirect `HAGENT_HOME` to temp dirs — never touch real `~/.hagent/`
...
### When upgrading a Hagent-like chat app or agent UI

These patterns came from real agent-console upgrade work and should be reused whenever extending a chat product toward Codex/Hagent behavior.

- **Implement stop end-to-end, not just in the browser.** A frontend `AbortController` only stops the HTTP request from the browser side. For real stop behavior, add a run registry keyed by `sessionId` (or run id), keep an `AbortController`/cancel token on the backend, expose a stop endpoint such as `/api/sessions/:id/stop`, and thread the abort signal through the stream loop and tool/orchestrator loop so work does not continue in the background.
- **Session-scoped state should be explicit.** If the app has tool loops, plans, or workspace panels, pass `sessionId` through orchestrator/tool APIs deliberately instead of relying on global state. This makes persistent todos, workspace introspection, and cancellation practical.
- **Avoid carrying source branding into the target project.** When porting logic from a reference agent into a user's own codebase, keep the source paths for audit/reference only; do not use the source product name in new filenames, namespaces, UI labels, roadmap docs, or user-facing copy unless the user explicitly asks for that branding. Use neutral names such as `source_port`, `agent_core`, `session_state`, or `reference_agent` instead.
- **Use a live roadmap file for long migrations.** For multi-phase agent upgrades or ports, create a single `.md` checklist in the target repo, group work by phases, and update `[ ]` -> `[x]` immediately when a milestone lands. This keeps long-running migrations coherent across sessions and gives the user a durable status board inside the project.
- **Roll out backend changes to the frontend incrementally.** Do not wait for a total backend rewrite to finish before exposing progress. Add the new backend behind narrow endpoints first (`sessions`, `workspace`, `stop`, then `messages`), wire the frontend to each stable endpoint gradually, and keep the legacy backend as fallback until the new path is proven.
- **Keep provider names in strict parity with the frontend.** If the UI exposes provider keys like `gemini`, `deepseek`, `cx`, `openai`, `anthropic`, `ollama`, `lmstudio`, `llamacpp`, and `lmstudio_local`, the new backend must accept exactly those names. Do not silently remap unknown providers to a default backend; return an explicit error for unknown keys so frontend/backend drift is obvious during rollout.
- **When a dev backend is under active porting, run it with auto-reload.** For a local FastAPI sidecar used during migration, prefer `uvicorn ... --reload` so route/schema edits land immediately and you do not waste time chasing stale live code after patching files.
- **If the new backend runs on a different origin or port, enable CORS before frontend testing.** A Vite UI on `127.0.0.1:3004` calling a FastAPI service on `127.0.0.1:8010` will trigger browser preflight as soon as requests include headers like `Authorization` or `Content-Type`. Add `CORSMiddleware` early and verify `OPTIONS` returns the expected `access-control-allow-origin` and headers before debugging the UI state machine.
- **Use a dedicated virtualenv for an auxiliary FastAPI backend.** When prototyping or porting a Python backend inside another agent project, do not install its requirements into a shared agent/runtime venv. Create a local `.venv` under the backend folder, install there, and document the exact launch command. This avoids downgrading shared packages like `fastapi`, `pydantic`, or `uvicorn` for the source agent/runtime.
- **When copying a large reference-agent core, preserve it under a neutral namespace first, then bridge it.** A practical migration pattern is to copy `run_agent.py`, `model_tools.py`, and supporting `agent/` + `tools/` trees into a target-local namespace such as `source_core/` or `reference_core/`, confirm imports in isolation, and only then add a thin adapter service that calls the copied runtime from the new web backend. This minimizes accidental rewrites during the first pass and keeps the diff auditable.
- **When the target product must not inherit source branding, scrub it everywhere.** During ports from a reference agent, avoid the source product name not only in code namespaces but also in roadmap files, generated docs, filenames, UI copy, and feature labels. If a migration doc or scaffold already contains the old branding, rename it immediately instead of leaving cleanup for later.
- **For long-running upgrades, report progress continuously and keep the repo status board current.** Some users want execution to continue without repeated confirmations. In those cases, keep moving, emit short progress updates as milestones land, and update the in-repo roadmap/checklist immediately so the project itself remains the source of truth.
- **If the user wants an "AI company" architecture, model it explicitly.** Treat the design as a coordinator/worker system: a CEO orchestrator, a worker registry with role-specialized agents, a task board/work queue, escalation rules, and per-agent memory/KPI. Keep this as a distinct orchestration layer over the chat/runtime core rather than blending it ad hoc into a single-agent loop.
- **Use LangChain/LangGraph selectively at the orchestration layer.** In Hagent-like or derivative systems, they fit best around CEO/worker routing graphs, task boards, and multi-step coordination. Do not spread them through every service by default; keep core session state, transport, and simple CRUD endpoints framework-light unless the graph actually adds value.
- **For Vietnamese-first users, localize the UI completely.** Do not leave mixed English labels or Vietnamese without diacritics in chat surfaces. Translate visible labels, status text, placeholders, error text, and side-panel headings into proper Vietnamese with dấu for a cohesive product feel.
- **After large chat UI rewrites, do a string pass.** Agent/chat rewrites often regress into placeholder English or unaccented text. Before finishing, scan the UI for visible literals (`New Chat`, `Loading`, `Request failed`, unaccented Vietnamese, etc.) and normalize them to the user's preferred language.
