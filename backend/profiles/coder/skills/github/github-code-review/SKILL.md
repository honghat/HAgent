...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.hagent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.hagent/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.hagent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${HAGENT_HOME:-$HOME/.hagent}/skills/github/github-auth/scripts/gh-env.sh"
