...
4. Store the token in `~/.hagent/.env` (or via `hagent setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.hagent/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
