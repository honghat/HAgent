...
# Ensure sibling modules (_hagent_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _hagent_home import display_hagent_home, get_hagent_home

HAGENT_HOME = get_hagent_home()
TOKEN_PATH = HAGENT_HOME / "google_token.json"
CLIENT_SECRET_PATH = HAGENT_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = HAGENT_HOME / "google_oauth_pending.json"
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_hagent_home()}/google_token.json")
