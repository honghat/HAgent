...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with hagent-agent)
2. User plugins: ``$HAGENT_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$HAGENT_HOME/plugins/model-providers/`` can override
...
def get_provider_profile(name: str) -> ProviderProfile | None:
    ...
    try:
        ...
        db_path = get_hagent_home().parent / "data" / "hagent.db"
        if db_path.exists():
...
def _user_plugins_dir() -> Path | None:
    """Return ``$HAGENT_HOME/plugins/model-providers/`` if it exists."""
    try:
        from hagent_constants import get_hagent_home

        d = get_hagent_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple HAGENT_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_hagent_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$HAGENT_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with hagent-agent.
    ...
    # 2. User plugins — under $HAGENT_HOME/plugins/model-providers/<name>/.
