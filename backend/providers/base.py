...
_FETCH_MODELS_TTL_SECONDS = 300.0
_FETCH_MODELS_CACHE: dict[tuple[str, str, str], tuple[float, list[str] | None]] = {}
_FETCH_MODELS_LOCK = threading.Lock()
...
class ProviderProfile:
    ...
    def fetch_models(
        ...
        *,
        ...
        cache_key = (self.name, url, key_hash)
        now = time.monotonic()
        with _FETCH_MODELS_LOCK:
            cached = _FETCH_MODELS_CACHE.get(cache_key)
            if cached and now - cached[0] < _FETCH_MODELS_TTL_SECONDS:
        ...
        try:
            ...
            with _FETCH_MODELS_LOCK:
                _FETCH_MODELS_CACHE[cache_key] = (time.monotonic(), models)
            return models
        except Exception as exc:
            logger.debug("fetch_models(%s): %s", self.name, exc)
            with _FETCH_MODELS_LOCK:
                _FETCH_MODELS_CACHE[cache_key] = (time.monotonic(), None)
            return None
