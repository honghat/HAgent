...
class AIAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            load_soul_identity (bool): If True, still use ~/.hagent/SOUL.md as the primary
        ...
        if not self.api_key or not self.base_url:
            try:
                import sqlite3
                # DB is at <repo>/data/hagent.db. Current file is at <repo>/backend/run_agent.py
                db_path = Path(__file__).resolve().parent.parent / "data" / "hagent.db"
                if db_path.exists():
        ...
        # both live under ~/.hagent/logs/.  Idempotent, so gateway mode
        ...
        # Session logs go into ~/.hagent/sessions/ alongside gateway sessions
