...
def load_hagent_dotenv(hagent_home=None, project_env=None):
    """Load .env files with priority: hagent_home > project_env (project root)"""
...
class _SafeWriter:
    ...
    When hagent-agent runs as a systemd service, Docker container, or headless
...
def _sanitize_message_surrogates(msg: dict) -> bool:
    """Sanitize surrogate characters from a single message dict (in-place).
        ...
        for tc in tool_calls:
            ...
            if isinstance(fn, dict):
                ...
                if isinstance(fn_args, str) and _SURROGATE_RE.search(fn_args):
                    ...
                    found = True
    # Walk any additional string / nested structured fields
...
def _sanitize_messages_surrogates(messages: list) -> bool:
    """Sanitize surrogate characters from all messages (in-place).
    ...
    found = False
    for msg in messages:
        if _sanitize_message_surrogates(msg):
            found = True
    return found
...
def _routermint_headers() -> dict:
    ...
    from hagent_cli import __version__ as _HAGENT_VERSION

    return {
        "User-Agent": f"HagentAgent/{_HAGENT_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[hagent-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        Args:
            ...
            load_soul_identity (bool): If True, still use backend/SOUL.md as the primary
        ...
        try:
            from hagent_cli.model_normalize import (
        ...
        # Background persist lock — prevents concurrent background saves
        self._persist_background_lock = threading.Lock()
        ...
        self.allow_small_context = bool(self.request_overrides.pop("allow_small_context", False))
        self._request_context_length_override = self.request_overrides.pop("context_length", None)
        ...
        self._cache_ttl = "5m"
        try:
            from hagent_cli.config import load_config as _load_pc_cfg
        ...
        # both live under root logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from hagent_logging import setup_logging, setup_verbose_logging
        setup_logging(hagent_home=_hagent_home)
        ...
        if self.verbose_logging:
            ...
            if self.quiet_mode:
                ...
                # on the fact that hagent_logging.setup_logging() does not
                ...
                # handler level inside hagent_logging.py, not here.
                pass
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from hagent_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    ...
                    from hagent_cli.models import copilot_default_headers
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        if not _fb_resolved:
                            raise RuntimeError(
                                ...
                                f"variable, or switch to a different provider with `hagent model`."
                            )
                    if not getattr(self, "_fallback_activated", False):
                        # No provider configured — reject with a clear message.
                        raise RuntimeError(
                            "No LLM provider configured. Run `hagent model` to "
                            "select a provider, or run `hagent setup` for first-time "
                            "configuration."
                        )
            ...
            try:
                ...
                if not self.quiet_mode:
                    ...
                    # Show API key info (masked) for debugging auth issues.
                    # Local endpoints (LM Studio, Ollama, vLLM, Tailscale Ollama…)
                    # do not validate the bearer token, so suppress the
                    # "invalid key" warning for them and emit a friendly note.
                    key_used = client_kwargs.get("api_key", "none")
                    if is_local_endpoint(self.base_url or ""):
                        print("🏠 Local endpoint — API key not required")
                    elif key_used and key_used != "dummy-key" and len(key_used) > 12:
        ...
        # Session logs go into backend/sessions/ alongside gateway sessions
        hagent_home = get_hagent_home()
        self.logs_dir = hagent_home / "sessions"
        ...
        try:
            from hagent_cli.config import load_config as _load_agent_config
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "hagent"
                        except Exception:
                            pass
        ...
        _request_context_length = self._request_context_length_override
        if _request_context_length is not None:
            try:
                if isinstance(_request_context_length, bool):
                    raise ValueError
                _parsed_request_context = int(_request_context_length)
                if _parsed_request_context <= 0:
                    raise ValueError
                _config_context_length = _parsed_request_context
            except (TypeError, ValueError):
                logger.warning(
                    "Invalid request_overrides.context_length for model %r: %r",
                    self.model,
                    _request_context_length,
                )
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from hagent_cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from hagent_cli.plugins import get_plugin_context_engine
        ...
        if self.allow_small_context and _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            # Small local models are useful for lightweight chat/tool flows even
            # when they cannot support the full default HAgent tool surface.
            # Keep compression thresholds inside the real window instead of the
            # normal 64K floor.
            self.context_compressor.threshold_tokens = max(1, int(_ctx * compression_threshold))
            target_tokens = int(self.context_compressor.threshold_tokens * compression_target_ratio)
            self.context_compressor.tail_token_budget = max(1, target_tokens)
            self.context_compressor.max_summary_tokens = max(
                256,
                min(int(_ctx * 0.05), getattr(self.context_compressor, "max_summary_tokens", 2048)),
            )
        elif _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                f"Model {self.model} has a context window of {_ctx:,} tokens, "
                f"which is below the minimum {MINIMUM_CONTEXT_LENGTH:,} required "
                f"by Hagent Agent.  Choose a model with at least "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                session_id=self.session_id,
                source=self.platform or os.environ.get("HAGENT_SESSION_SOURCE", "cli"),
                model=self.model,
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Hagent' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from hagent_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        turn-scoped).
        """
        from hagent_cli.providers import determine_api_mode
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            try:
                from hagent_cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``hagent chat -q``. In that mode,
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        ``hagent logs --level WARNING | grep "Stream drop"`` to inspect.
        """
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `hagent setup` or set OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Hagent "
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``HAGENT_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``HAGENT_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("HAGENT_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``HAGENT_API_CALL_STALE_TIMEOUT`` env var
          4. 300.0s default
        ...
        env_timeout = os.getenv("HAGENT_API_CALL_STALE_TIMEOUT")
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from hagent_cli.models import _should_use_copilot_responses_api
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            "platform": self.platform or os.environ.get("HAGENT_SESSION_SOURCE", "cli"),
            "tool_name": "memory",
        }
        if task_id:
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        The actual I/O (JSON file write + SQLite INSERT) runs in a daemon
        background thread so the calling thread returns immediately.
        """
        _t_ps = time.time()
        ...
        _t_sync = time.time()
        # Snapshot messages for the background thread (the live list may be
        # mutated by subsequent API call iterations before the thread runs).
        _msg_snapshot = copy.deepcopy(messages)
        _conv_snapshot = copy.deepcopy(conversation_history) if conversation_history else None
        _t_copy = time.time()
        # Spawn a daemon thread for the actual I/O. Uses a Lock to prevent
        # more than one background save from stacking up.
        if self._persist_background_lock.acquire(blocking=False):
            threading.Thread(
                target=self._persist_io_background,
                args=(_msg_snapshot, _conv_snapshot),
                daemon=True,
                name="persist-io",
            ).start()
        else:
            # Another background save is in-flight — skip this one rather
            # than block the hot path. The latest state will be persisted
            # on the next exit path anyway.
            if self.verbose_logging:
                logging.debug("Background persist skipped (previous save still in-flight)")
        _t_done = time.time()
        _logger = getattr(self, "logger", None) or logging.getLogger(__name__)
        _logger.debug(
            "Persist timing: sync=%s copy=%s total=%s",
            f"{(_t_sync - _t_ps) * 1000:.1f}ms",
            f"{(_t_copy - _t_sync) * 1000:.1f}ms",
            f"{(_t_done - _t_ps) * 1000:.1f}ms",
        )

    def _persist_io_background(self, messages: List[Dict], conversation_history: List[Dict] = None):
        """Run the actual I/O operations in a background thread."""
        try:
            self._save_session_log(messages)
            # Save and restore _last_flushed_db_idx — the foreground may have
            # already flushed newer messages (and updated the index) before
            # this background thread runs.  Restoring the old index would
            # cause the next foreground flush to double-write.
            _saved_idx = getattr(self, "_last_flushed_db_idx", 0)
            self._flush_messages_to_session_db(messages, conversation_history)
            self._last_flushed_db_idx = _saved_idx
        except Exception as _e:
            _logger = getattr(self, "logger", None) or logging.getLogger(__name__)
            _logger.warning("Background persist I/O failed: %s", _e)
        finally:
            self._persist_background_lock.release()
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("HAGENT_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Some execution modes (cron) still want HAGENT_HOME persona while keeping
        ...
        # Pointer to the hagent-agent skill + docs for user questions about Hagent itself.
        prompt_parts.append(HAGENT_AGENT_HELP_GUIDANCE)
        ...
        # HAGENT_KANBAN_TASK env var). Normal chat sessions never see
        # this block.
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the hagent-agent install
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from hagent_cli.copilot_auth import copilot_request_headers
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from hagent_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("HAGENT_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("HAGENT_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from hagent_cli.copilot_auth import resolve_copilot_token
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # wins over the HAGENT_API_TIMEOUT env default if the user set it.
            _provider_timeout_cfg = get_provider_request_timeout(self.provider, self.model)
            _base_timeout = (
                _provider_timeout_cfg
                if _provider_timeout_cfg is not None
                else float(os.getenv("HAGENT_API_TIMEOUT", 1800.0))
            )
            # Read timeout: config wins here too.  Otherwise use
            # HAGENT_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
            if _provider_timeout_cfg is not None:
                _stream_read_timeout = _provider_timeout_cfg
            else:
                _stream_read_timeout = float(os.getenv("HAGENT_STREAM_READ_TIMEOUT", 120.0))
                # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
                # prefill on large contexts before producing the first token.
                # Auto-increase the httpx read timeout unless the user explicitly
                # overrode HAGENT_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("HAGENT_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("HAGENT_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set HAGENT_STREAM_STALE_TIMEOUT.
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        try:
            ...
            if not fb_api_key_hint:
                ...
                # _normalize_custom_provider_entry in hagent_cli/config.py).
            ...
            try:
                from hagent_cli.model_normalize import normalize_model_for_provider
        ...
        def _shrink_data_url(url: str) -> Optional[str]:
            ...
            try:
                ...
                tmp = tempfile.NamedTemporaryFile(
                    prefix="hagent_shrink_", suffix=suffix, delete=False,
                )
                try:
                    tmp.write(raw)
                    tmp.close()
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "hagent",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from hagent_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from hagent_cli.models import lmstudio_model_reasoning_options
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        if self._session_db:
            try:
                ...
                # state_meta under "goal:<sid>" by hagent_cli.goals.
                try:
        ...
        # compression (not a fresh /new). Plugin engines (e.g. hagent-lcm) use
        # boundary_reason="compression" to preserve DAG lineage across the
        # rollover instead of re-initializing fresh per-session state.
        # See hagent-lcm#68. Built-in ContextCompressor ignores kwargs.
        try:
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        if not pre_tool_block_checked:
            try:
                from hagent_cli.plugins import get_pre_tool_call_block_message
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=hagent-agent"]
    ...
    def run_conversation(
        ...
        # ``hagent logs --session <id>`` can filter a single conversation.
        from hagent_logging import set_session_context
        ...
        # are reused.  The system prompt is Hagent's territory; plugins
        ...
        # Private HAgent memory/wiki context is loaded per turn and injected
        # into the current user message only. This keeps the cached system
        # prompt stable while still making fresh Wiki DB entries visible before
        # the model answers.
        _personal_user_context = ""
        try:
            from agent.personal_context import build_personal_context

            _personal_user_context = build_personal_context(
                original_user_message if isinstance(original_user_message, str) else "",
                user_id=getattr(self, "_user_id", None) or None,
            )
        except Exception as exc:
            logger.debug("personal context load failed: %s", exc)
        ...
        try:
            length_continue_max = max(3, int(os.getenv("HAGENT_LENGTH_CONTINUATION_MAX", "8") or "8"))
        except ValueError:
            length_continue_max = 8
        try:
            truncated_tool_call_max = max(1, int(os.getenv("HAGENT_TRUNCATED_TOOL_CALL_RETRIES", "3") or "3"))
        except ValueError:
            truncated_tool_call_max = 3
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            for idx, msg in enumerate(messages):
                ...
                if idx == current_turn_user_idx and msg.get("role") == "user":
                    ...
                    if _personal_user_context:
                        _injections.append(_personal_user_context)
            ...
            # cache prefix.  The system prompt is reserved for Hagent internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            # Combined single-pass: normalize tool-call JSON + strip
            # surrogate characters + estimate token count.  Previously
            # three separate walks of api_messages — merged into one
            ...
            for am in api_messages:
                ...
                # Strip surrogate characters in-place (single-pass)
                _sanitize_message_surrogates(am)

                total_chars += len(str(am))

            approx_tokens = estimate_messages_tokens_rough(api_messages)
            _t_nm = time.time()

            # Pipeline timing (debug)
            _pi = lambda _s, _e: f"{(_e - _s) * 1000:.1f}ms"  # noqa: E731
            request_logger.debug(
                "Pipeline timing: sanctc=%s repair=%s build_msgs=%s cachectl=%s "
                "sanimsgs=%s dropthink=%s normalize=%s total=%s",
                _pi(_t_pipe, _t_sc), _pi(_t_sc, _t_rm), _pi(_t_rm, _t_am),
                _pi(_t_am, _t_pc), _pi(_t_pc, _t_sa), _pi(_t_sa, _t_dt),
                _pi(_t_dt, _t_nm), _pi(_t_pipe, _t_nm),
            )
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if env_var_enabled("HAGENT_DUMP_REQUESTS"):
                    ...
                    if finish_reason == "length":
                        ...
                        if self.api_mode in ("chat_completions", "bedrock_converse", "anthropic_messages"):
                            ...
                            if assistant_message is not None and not _trunc_has_tool_calls:
                                ...
                                if length_continue_retries < length_continue_max:
                                    self._vprint(
                                        f"{self.log_prefix}↻ Requesting continuation "
                                        f"({length_continue_retries}/{length_continue_max})..."
                                    )
                                    continue_msg = {
                                        "role": "user",
                                        "content": (
                                ...
                                if partial_response:
                                    partial_response = (
                                        f"{partial_response}\n\n"
                                        "⚠️ Phản hồi vẫn chạm giới hạn độ dài sau nhiều lần nối tiếp. "
                                        "Mình đã giữ lại phần đã tạo thay vì báo lỗi; hãy yêu cầu tiếp tục nếu cần phần còn lại."
                                    )
                                return {
                        ...
                        if self.api_mode in ("chat_completions", "bedrock_converse", "anthropic_messages"):
                            ...
                            if assistant_message is not None and _trunc_has_tool_calls:
                                if truncated_tool_call_retries < truncated_tool_call_max:
                                    truncated_tool_call_retries += 1
                                    _boost_base = self.max_tokens if self.max_tokens else 4096
                                    self._ephemeral_max_output_tokens = min(
                                        _boost_base * (truncated_tool_call_retries + 1),
                                        32768,
                                    )
                                    self._vprint(
                                        f"{self.log_prefix}⚠️  Truncated tool call detected — retrying API call "
                                        f"({truncated_tool_call_retries}/{truncated_tool_call_max})...",
                                        force=True,
                                    )
                                ...
                                return {
                                    "final_response": (
                                        "⚠️ Provider đã cắt ngắn phần tham số tool nhiều lần, "
                                        "nên HAgent không thực thi tool với JSON chưa hoàn chỉnh để tránh làm sai tác vụ. "
                                        "Hãy gửi lại yêu cầu ngắn hơn hoặc chia nhỏ tác vụ."
                                    ),
                                    "messages": messages,
                                    "api_calls": api_call_count,
                                    "completed": False,
                        ...
                        if len(messages) > 1:
                            ...
                            rollback_response = self._strip_think_blocks(
                                truncated_response_prefix or (_trunc_content or "")
                            ).strip()
                            if rollback_response:
                                rollback_response = (
                                    f"{rollback_response}\n\n"
                                    "⚠️ Phản hồi bị cắt bởi giới hạn độ dài. "
                                    "Mình đã giữ phần đã tạo thay vì báo lỗi; hãy yêu cầu tiếp tục nếu cần."
                                )
                            ...
                            return {
                                "final_response": rollback_response or None,
                                "messages": rolled_back_messages,
                                "api_calls": api_call_count,
                                "completed": False,
                                "partial": True,
                                **({} if rollback_response else {"error": "Response truncated due to output length limit"}),
                            }
                        else:
                            # First message was truncated - mark as failed
                            first_response = self._strip_think_blocks(
                                truncated_response_prefix or (_trunc_content or "")
                            ).strip()
                            self._vprint(
                                f"{self.log_prefix}⚠️  First response truncated"
                                + (" — returning partial response" if first_response else " - cannot recover"),
                                force=True,
                            )
                            if first_response:
                                first_response = (
                            ...
                            self._persist_session(messages, conversation_history)
                            return {
                                "final_response": first_response or None,
                                "messages": messages,
                                "api_calls": api_call_count,
                                "completed": False,
                                "failed": not bool(first_response),
                                **({} if first_response else {"error": "First response truncated due to output length limit"}),
                            }
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: hagent login --provider nous")
                    ...
                    if (
                        ...
                        from hagent_constants import display_hagent_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Hagent-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: hagent config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: hagent config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `hagent auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: hagent setup", force=True)
                ...
                if assistant_message.tool_calls:
                    ...
                    if invalid_json_args:
                        ...
                        if _truncated:
                            ...
                            if self._invalid_json_retries < truncated_tool_call_max:
                                _boost_base = self.max_tokens if self.max_tokens else 4096
                                self._ephemeral_max_output_tokens = min(
                                    _boost_base * (self._invalid_json_retries + 1),
                                    32768,
                                )
                                self._vprint(
                                    f"{self.log_prefix}🔄 Retrying truncated tool arguments "
                                    f"({self._invalid_json_retries}/{truncated_tool_call_max})...",
                                    force=True,
                                )
                                continue
                            self._invalid_json_retries = 0
