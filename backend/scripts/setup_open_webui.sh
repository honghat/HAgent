...
# - ensures ~/.hagent/.env has API server settings
...
HAGENT_ENV_FILE="${HAGENT_ENV_FILE:-$HOME/.hagent/.env}"
...
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.hagent/logs/openwebui.log
StandardError=append:%h/.hagent/logs/openwebui.error.log
