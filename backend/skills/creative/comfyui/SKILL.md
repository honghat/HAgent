...
- `references/remote-ssh-setup.md` — ComfyUI on remote Linux servers (no comfy-cli, no GUI): detection, startup via screen/tmux/systemd, manual model download via wget, SSH tunnel setup, common errors, **GGUF model setup with SDXL-Lightning**
- `references/image-gen-provider-comfyui.md` — implementing ImageGenProvider for ComfyUI (SSH tunnel + GGUF + no API key pattern, as used in the Photo tab): full workflow JSON, DualCLIPLoaderGGUF clip name quirks (`clip_vitg` vs `clip_g`), LoadImage path requirements, queue monitoring, known errors and fixes
...
## Photo Tab / ImageGenProvider Integration

ComfyUI can be registered as an `ImageGenProvider` plugin so the **Photo tab** and the **`image_generate` tool** use it for image generation. This is the primary integration path since the ChatGPT2API bridge was replaced.

### Provider Plugin Location

`backend/plugins/image_gen/comfyui/__init__.py` + `plugin.yaml`

### Key Differences from Standard Providers

Unlike OpenAI/Cloud-based providers, the ComfyUI provider:

- **Has no API key** — requires an SSH tunnel to a remote GPU server instead
- **Uses stick-figure style prompt** (`_build_prompt()` wraps user input in "simple stick figure, black line art, white background...")
- **Does NOT use `save_b64_image`** — downloads the image from ComfyUI's `/view` endpoint and writes directly to `$HAGENT_HOME/cache/images/`
- **SDXL-Lightning is 1024×1024 square only** — aspect ratio on the frontend is a suggestion (ComfyUI output is always square)
- **Always returns absolute file path** (URL conversion handled by frontend)

### SSH Tunnel Pattern

```python
# Must be called before every generation attempt
def _ensure_ssh_tunnel(base_url: str) -> bool:
    try:
        ...
        pass
    # Start tunnel
    subprocess.run(["ssh", "-N", "-f", "-L", "8188:127.0.0.1:8188", "hatnguyen@100.69.50.64"], timeout=15)
    time.sleep(2)
    ...
```

### Workflow JSON (api format) — SDXL-Lightning 4step GGUF

The workflow uses these `class_types` (GGUF-specific, not standard ComfyUI):
- `UnetLoaderGGUF` — loads `sdxl_lightning_4step.q5_1.gguf`
- `DualCLIPLoaderGGUF` — loads `clip_vitl.safetensors` + `clip_vitg.safetensors`
- `VAELoader` — loads `sdxl-vae-fp16-fix.safetensors`
- `CLIPTextEncodeSDXL` — SDXL-specific encoder (not the standard `CLIPTextEncode`)
- `KSampler` — 4 steps, cfg=1.0, sgm_uniform scheduler
...
image_gen:
  provider: comfyui
  comfyui:
    host: "http://127.0.0.1:8188"     # default
    model: "sdxl_lightning_4step.q5_1.gguf"  # default
```

### Backend Caching

Generated images are saved under `$HAGENT_HOME/cache/images/` with prefix `comfyui_`. The history endpoint in `photo.py` scans this directory. If the SSH tunnel drops mid-generation, the provider returns an error response — no placeholder fallback.

### Frontend

`PhotoTab.jsx` has two sub-views (generate + history). No Account/Token UI since ComfyUI needs no authentication. The info badge warns users about the SSH tunnel requirement.

See `backend/plugins/image_gen/comfyui/__init__.py` for the full implementation.
...
11. **GGUF models use different class_type names.** Workflows designed for `.safetensors`
    checkpoints will **silently fail** if the class_types aren't updated for GGUF:
    - `CheckpointLoaderSimple` → `UnetLoaderGGUF` + `DualCLIPLoaderGGUF`
    - `UNETLoader` → `UnetLoaderGGUF` (or `UnetLoaderGGUFAdvanced`)
    - `DualCLIPLoader` → `DualCLIPLoaderGGUF`
    - Using the wrong name produces `"Node 'X' not found"` errors and the standard
      PyTorch loader will fail with `invalid load key` on GGUF binary files.
    See `references/remote-ssh-setup.md` for the full GGUF setup guide.

12. **Cache filename ≠ ComfyUI output filename** — When used as an `ImageGenProvider`
    plugin (Photo tab), the downloaded copy at `$HAGENT_HOME/cache/images/` gets a
    new timestamp+uuid name. The original ComfyUI output filename (from `SaveImage`)
    is NOT preserved. The `DELETE /api/photo/delete/{filename}` endpoint in
    `api/routers/photo.py` handles this with a blanket SSH `rm -f ~/ComfyUI/output/*.png`
    — every delete removes ALL remote PNGs. Move any files you want to keep out of
    `~/ComfyUI/output/` first. See `references/image-gen-provider-comfyui.md` for details.

15. **ComfyUI output directory accumulation** — Each generation leaves a file in
    `~/ComfyUI/output/` on the server. Since the HAgent provider downloads via HTTP
    and saves locally, the remote output files are orphaned. Periodically clean them
    via SSH: `ssh hatnguyen@100.69.50.64 "rm -f ~/ComfyUI/output/comfyui_gen_*.png"`.

14. **ADE_AnimateDiffCombine + SaveImage return_type_mismatch** — The `ADE_AnimateDiffCombine` node outputs `GIF` type, not `IMAGE` type. If you connect it to a `SaveImage` node, ComfyUI returns `"return_type_mismatch"`. Fix: set `"save_image": true` on `ADE_AnimateDiffCombine` itself (writes GIF to output dir), or use `SaveAnimatedWEBP` which accepts GIF type input. Never include a `SaveImage` node downstream of `ADE_AnimateDiffCombine`.
