...
```bash
# Local — TEMPORARY FILE WORKAROUND: piping inline JSON often breaks shell escaping.
# Write payload to a file and use -d @file instead:
cat > /tmp/payload.json << 'PAYLOAD'
{"prompt": {"3": {"class_type": "KSampler", "inputs": {"seed": 42, "steps": 20}}}}
PAYLOAD
...
# ⚠️ CRITICAL: The payload MUST be wrapped in {"prompt": <workflow_dict>}.
# Submitting the bare workflow dict as the body gives error: no_prompt.
# The most reliable approach for remote SSH is base64-encode the payload on the
# local machine, then decode + pipe on the remote:
PAYLOAD_B64=$(base64 < /tmp/payload.json)
ssh user@host "echo '$PAYLOAD_B64' | base64 -d | \
...
- `prompt`: workflow graph in API format. ⚠️ **CRITICAL: This MUST be a dict inside the JSON body** — the outer body is `{"prompt": <workflow_dict>, ...}`. The most common mistake is submitting the bare workflow dict as the body, which produces `error: no_prompt`. Solution: wrap in `json.dumps({"prompt": workflow})` or pass via a temp file with `-d @payload.json`.
