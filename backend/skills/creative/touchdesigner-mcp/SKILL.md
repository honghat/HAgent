...
metadata:
  hagent:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, hagent-video]
...
```
Hagent Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${HAGENT_HOME:-$HOME/backend}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Hagent config (if missing)
...
3. **Restart Hagent session** to pick up the new MCP server
