...
## 5. Hagent Configuration

### Config location

`$HAGENT_HOME/config.yaml` (defaults to `$HAGENT_HOME/config.yaml` when `HAGENT_HOME` is unset)
...
Restart the Hagent session for changes to take effect. The MCP connection is
