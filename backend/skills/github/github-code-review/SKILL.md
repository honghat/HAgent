...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Hagent Agent\",
    \"comments\": [
...
```bash
source "${HAGENT_HOME:-$HAGENT_HOME}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Hagent Agent. Code looks clean — good test coverage, no security concerns."
