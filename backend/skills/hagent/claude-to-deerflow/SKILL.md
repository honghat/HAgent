...
description: "Interact with HAgent AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to HAgent for research/analysis, start a HAgent conversation thread, check HAgent status or health, list available models/skills/agents in HAgent, manage HAgent memory, upload files to HAgent threads, or delegate complex research tasks to HAgent. Also use when the user mentions hagent, deer flow, or wants to run a deep research task that HAgent can handle."
---

# HAgent Skill

Communicate with a running HAgent instance via its HTTP API. HAgent is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

HAgent exposes two API surfaces behind an Nginx reverse proxy:
...
```bash
bash /path/to/skills/claude-to-hagent/scripts/chat.sh "Your question here"
```
...
- If health check fails, HAgent is not running. Inform the user they need to start it.
