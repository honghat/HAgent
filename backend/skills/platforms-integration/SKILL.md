...
Create plugin directory in `$HAGENT_HOME/plugins/platforms/` with:
...
- Files created: `$HAGENT_HOME/plugins/platforms/omnichannel/send_zalo_by_name.py`, `zalo_telegram_bot.py` (automation examples)
...
```bash
cd $HAGENT_HOME/plugins/platforms
chmod +x scripts/*.sh
./scripts/platforms/setup-wizard.sh  # Interactive credential entry
# OR read ./scripts/platforms/quick-check.md  # Verify platform status
```
...
### Zalo-Specific Issues\n\n1. **⚠️ CRITICAL: NO API MESSAGE READING** - Zalo API does NOT support reading messages like Telegram/Facebook Messenger. **Cannot use gateway API to check inbox**. To view received messages:\n   - **Required**: Use **Zalo Desktop App** or **Zalo Web** (`https://chat.zalo.me`) directly\n   - **Why**: Zalo blocks programmatic message polling for security reasons\n   - **Workaround Pattern**: Open browser → Click tab with new badge ● → Read messages in web interface\n   - Files: `~/hagent/plugins/platforms/omnichannel/view_zalo_messages.py` (browser automation wrapper)\n2. **⚠️ NO LOCAL FILE SENDING** (`zlapi` limitation): Only `sendLocalImage()` and `sendRemoteFile()` are available. `sendRemoteFile()` requires a **publicly accessible HTTP URL** — local URLs like `http://localhost:...` will fail because Zalo's server cannot reach them. Documents (.xlsx, .docx, .pdf) cannot be sent from local disk without a public proxy. See `references/zalo-file-sending-limitations.md` for full analysis and solutions.\n3. **60s Single-Use Reply Token**: Zalo blocks immediate replies (< 60s between sends). Implement cooldown or use `_keep_typing()` pattern for mid-flight bubbles.\n4. **Cookie Expiry**: Zalo refreshes cookies periodically. Re-authenticate on connection failure (see `references/zalo-qr-workflow.md`).\n5. **Image Size Limit**: Max ~40KB images supported. Larger files need file hosting or chunked upload workaround.
...
| Deployment | `$HAGENT_HOME/plugins/` | Any port, independent server ✅ |
...
cd $HAGENT_HOME/plugins/platforms/omnichannel/backend
...
cat > $HAGENT_HOME/plugins/platforms/omnichannel/env/config.env << 'EOF'
...
**Migration Approach:**
1. **Zero Core Changes**: Use plugin path in `$HAGENT_HOME/plugins/platforms/`
2. **Database Options**: 
...
- **Zalo QR Workflow**: `$HAGENT_HOME/plugins/platforms/omnichannel/backend/references/zalo-qr-auth-guide.md` (QR code & cookie auth guide, includes DingTalk pattern from dingtalk_auth.py)
- **Zalo Message Viewing**: `$HAGENT_HOME/plugins/platforms/omnichannel/backend/references/zalo-message-viewing-workflow.md` (API limitations & browser-only reading guide) ⭐\n- **Omnichannel Hub Backend API**: `$HAGENT_HOME/plugins/platforms/omnichannel/OMNICHANNEL_BACKEND_SETUP.md` - Complete setup & integration guide
...
- **Facebook Group Feed Limitations**: `$HAGENT_HOME/plugins/platforms/omnichannel/backend/references/facebook-group-feed-scraping-limitations.md` - Content blocked even with valid cookies, requires Graph API or residential proxy ⭐ (see also [social-media-scraping skill](../social-media-scraping/SKILL.md))\n- **Zalo File-Sending Limitations**: `references/zalo-file-sending-limitations.md` - Document, spreadsheet, and PDF sending constraints via `zlapi`
...
```bash
cd $HAGENT_HOME/plugins/platforms/zalo
...
source $HAGENT_HOME/omnichannel.env

# Initialize hub (checks both Zalo and Facebook)
python $HAGENT_HOME/plugins/platforms/omnichannel/init.py

# Test connectivity
python $HAGENT_HOME/plugins/platforms/omnichannel/test_omnichannel.py $HAGENT_HOME/omnichannel.env
```
...
3. Check logs at `$HAGENT_HOME/gateway.log`
...
```bash
cd $HAGENT_HOME/plugins/platforms/omnichannel
./scripts/platforms/quick-check.md  # Available in setup files
# Or run manually:
python3 test_omnichannel.py $HAGENT_HOME/omnichannel.env
```
