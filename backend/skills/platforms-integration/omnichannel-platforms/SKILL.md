...
Create `$HAGENT_HOME/plugins/platforms/omnichannel/backend/start.sh`:
...
Add to `backend/config.yaml` under `gateway.auto_start_scripts:` section.
...
- Backend logs: `$HAGENT_HOME/plugins/platforms/omnichannel/backend/logs/`
- Gateway logs: `$HAGENT_HOME/logs/gateway.log`
- Session files: `$HAGENT_HOME/sessions/`
...
1. **Create plugin directory:**
   ```bash
   mkdir -p $HAGENT_HOME/plugins/platforms/<platform-name>/backend
   ```

2. **Clone official API client or create custom:**
   ```bash
   git clone https://github.com/<owner>/<platform>.git \
     $HAGENT_HOME/plugins/platforms/<platform-name>/backend
   ```
...
6. **Add platform to `backend/config.yaml`** under `omnichannel.enabled_platforms:` section.
...
| `scripts/omnichannel-backend-migration.sh` | Auto-start migration script |
| `$HAGENT_HOME/plugins/platforms/omnichannel/backend/api_server.py` | FastAPI backend server |
| `$HAGENT_HOME/plugins/platforms/omnichannel/backend/start.sh` | Server launch script |
