...
const pluginDir = path.join(process.env.HAGENT_HOME || '~', '.hagent', 'plugins', 'platforms', 'omnichannel/backend');
...
**Location:** `/Users/nguyenhat/HAgent/frontend/scripts/start-omnichannel-backend.js`
...
```bash
# Location: ~/.hagent/plugins/platforms/omnichannel/env/config.env
