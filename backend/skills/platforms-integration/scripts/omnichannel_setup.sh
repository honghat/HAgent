...
# Usage: source $HAGENT_HOME/plugins/platforms/omnichannel/setup_omnichannel.sh
...
OMNI_DIR="$HAGENT_HOME/plugins/platforms/omnichannel"
if [ ! -d "$OMNI_DIR" ]; then
...
OMNICHANNEL_ZALO_ADAPTER=$HAGENT_HOME/plugins/platforms/zalo/adapter.py
OMNICHANNEL_FB_ADAPTER=$HAGENT_HOME/plugins/platforms/facebook/adapter.py
EOF
...
ENV_FILE="$HAGENT_HOME/omnichannel.env"
if [ -f "$ENV_FILE" ]; then
    echo "# Status Check:"
    echo "  source $HAGENT_HOME/omnichannel.env"
    echo "  # Verify:"
