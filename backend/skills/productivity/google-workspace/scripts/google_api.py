...
"""Google Workspace API CLI for Hagent Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Hagent-facing JSON contract and falls back to the Python client
...
# Ensure sibling modules (_hagent_home) and backend module are importable when run standalone.
...
from _hagent_home import get_hagent_home, get_token_file_path, get_tokens_home
...
TOKEN_PATH = get_token_file_path("google_token.json")
...
def _account_rows() -> list[dict]:
    try:
        with get_connection() as conn:
            rows = conn.execute(
                """
                SELECT email, token_path, enabled_for_agent, is_default, last_status, updated_at
...
def _resolve_account_token_path(account: str) -> Path:
    ...
    for row in rows:
        ...
        if value == email or value in email:
            token_path = Path(row.get("token_path") or "")
            if token_path.exists():
                return token_path

    token_path = ACCOUNT_TOKEN_DIR / f"{_safe_email_filename(value)}.json"
    if token_path.exists():
        return token_path

    known = ", ".join(row.get("email", "") for row in rows if row.get("email")) or "none"
    print(f"Unknown Google account: {account}. Authorized accounts: {known}", file=sys.stderr)
    sys.exit(1)


def _select_account_from_argv() -> str:
    account = os.getenv("HAGENT_GOOGLE_ACCOUNT", "").strip()
    cleaned = [sys.argv[0]]
    skip = False
    for idx, arg in enumerate(sys.argv[1:], start=1):
        ...
        if arg == "--account":
            if idx + 1 >= len(sys.argv):
                print("--account requires an email address", file=sys.stderr)
                sys.exit(2)
            account = sys.argv[idx + 1].strip()
            skip = True
            continue
...
def _active_token_path() -> Path:
    return Path(os.getenv("HAGENT_GOOGLE_TOKEN_PATH") or TOKEN_PATH)


def _set_active_token_path(path: Path) -> None:
    os.environ["HAGENT_GOOGLE_TOKEN_PATH"] = str(path)
...
def _gws_binary() -> str | None:
    override = os.getenv("HAGENT_GWS_BIN")
    if override:
        return override
...
def accounts_list(args):
    ...
    for row in rows:
        ...
        output.append(
            {
                "email": row.get("email", ""),
                "enabledForAgent": bool(row.get("enabled_for_agent")),
                "isDefault": bool(row.get("is_default")),
                "lastStatus": row.get("last_status", ""),
                "tokenReady": token_path.exists(),
                "workspaceReady": not missing_scopes,
                "missingScopes": [SCOPE_LABELS.get(scope, scope) for scope in missing_scopes],
                "updatedAt": row.get("updated_at", ""),
            }
        )
    if not output and TOKEN_PATH.exists():
        output.append(
            {
                "email": "default",
...
def gmail_mark_read(args):
    ...
    while remaining > 0:
        ...
        message_ids.extend(msg["id"] for msg in result.get("messages", []))
    ...
    if not message_ids:
        print(json.dumps({"status": "ok", "markedRead": 0, "messageIds": []}, indent=2))
        return
    ...
    print(json.dumps({"status": "ok", "markedRead": len(message_ids), "messageIds": message_ids}, indent=2))
...
def main():
    selected_account = _select_account_from_argv()
    if selected_account:
        _set_active_token_path(_resolve_account_token_path(selected_account))

    parser = argparse.ArgumentParser(description="Google Workspace API for Hagent Agent")
    parser.epilog = "Use --account email@example.com before or after the service name to select an authorized Google account."
    ...
    p.set_defaults(func=accounts_list)
    ...
    p = gmail_sub.add_parser("mark-read")
    p.add_argument("query", nargs="?", default="is:unread", help="Gmail query to mark read")
    p.add_argument("--max", type=int, default=500, help="Maximum messages to mark read")
    p.set_defaults(func=gmail_mark_read)
