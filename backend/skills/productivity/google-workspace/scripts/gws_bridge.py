#!/usr/bin/env python3
"""Bridge between Hagent OAuth token and gws CLI.
...
from _hagent_home import get_token_file_path


def get_token_path() -> Path:
    return get_token_file_path("google_token.json")
...
def refresh_token(token_data: dict) -> dict:
    ...
    if missing:
        print(f"ERROR: Google token is missing required fields: {', '.join(missing)}", file=sys.stderr)
