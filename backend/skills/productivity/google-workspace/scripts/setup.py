#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Hagent Agent.
...
PENDING_AUTH_PATH = get_token_file_path("google_oauth_pending.json")
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Hagent profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'hagent-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Hagent home."""
...
def _client_is_web_app() -> bool:
    """Detect if the OAuth client is a web app (has client_secret)."""
    try:
...
def get_auth_url():
    ...
    _save_pending_auth(state=state, code_verifier=flow.code_verifier if use_pkce else "")
...
def exchange_auth_code(code: str):
    ...
    # Web app clients need client_id/secret in token for google.auth to work
    if _client_is_web_app():
        try:
            secret_data = json.loads(CLIENT_SECRET_PATH.read_text())
            web = secret_data.get("web", {})
    ...
    print(f"Profile-scoped token location: {display_hagent_home()}/tokens/google_token.json")

    # ──────────────────────────────────────────────────────────────────
    # Read the token back to extract email, then persist to DB + per-account file
    # so the google_accounts table has an active entry for this user.
    # ──────────────────────────────────────────────────────────────────
    _persist_account_after_oauth()


def _persist_account_after_oauth():
    """Upsert google_accounts DB row + save per-account token file after successful OAuth."""
    from datetime import datetime, timezone

    try:
        from google.oauth2.credentials import Credentials
    except ImportError:
        return  # not critical — agent can still use the default token path
    ...
    # Fetch the authenticated user's email from Gmail API
    email = ""
    try:
        from googleapiclient.discovery import build
        ...
        print(f"WARNING: Could not fetch email from Gmail API: {e}")
        email = ""

    if not email:
        return

    # Determine per-account token path
    ACCOUNT_TOKEN_DIR = get_tokens_home() / "google_tokens"
    ACCOUNT_TOKEN_DIR.mkdir(parents=True, exist_ok=True)

    import re
    safe_stem = re.sub(r"[^A-Za-z0-9_.-]+", "_", email.strip().lower()) or "account"
    account_token_path = ACCOUNT_TOKEN_DIR / f"{safe_stem}.json"

    # Copy the token to the per-account file
    try:
        token_data = json.loads(TOKEN_PATH.read_text())
        account_token_path.write_text(json.dumps(token_data, indent=2))
        try:
            os.chmod(account_token_path, 0o600)
        except Exception:
            pass
        print(f"OK: Per-account token saved to {account_token_path}")
    except Exception as e:
        print(f"WARNING: Could not save per-account token: {e}")
        account_token_path = TOKEN_PATH  # fall back to default
        ...
        with get_connection() as conn:
            conn.execute(
                """INSERT OR REPLACE INTO google_accounts
                   (email, token_path, enabled_for_agent, is_default, last_status, updated_at)
                   VALUES (?, ?, 1, 0, 'connected', ?)""",
                (email, str(account_token_path), now),
            )
            print(f"OK: Account {email} registered in DB (enabled_for_agent=1)")
    except Exception as e:
        print(f"WARNING: Could not upsert google_accounts: {e}")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Hagent")
