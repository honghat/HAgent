...
description: "Operate the Teams meeting summary pipeline via Hagent CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
Everything operator-facing is a `hagent teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `$HAGENT_HOME/.env`:
...
```bash
hagent teams-pipeline validate              # config snapshot — run first after any change
hagent teams-pipeline token-health          # Graph token status
hagent teams-pipeline token-health --force-refresh   # force a fresh token acquisition
hagent teams-pipeline list                  # recent meeting jobs
hagent teams-pipeline list --status failed  # only failed jobs
hagent teams-pipeline show <job-id>         # full detail of one job
hagent teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
hagent teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
hagent teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
hagent teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
hagent teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

hagent teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
hagent teams-pipeline delete-subscription <sub-id>
hagent teams-pipeline maintain-subscriptions            # renew near-expiry ones
hagent teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `hagent teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `hagent cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
