...
## 5. GODMODE FAST — Hagent 4 405B (Zero Refusal)

**Model:** `nousresearch/hagent-4-405b`
**Strategy:** Hagent models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Hagent Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `$HAGENT_HOME/config.yaml`:
