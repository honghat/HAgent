...
python3 ~/.hagent/plugins/platforms/omnichannel/send_name_only.py <name> '<message>'
# Example: python3 send_name_only.py Đại 'Chào bạn!'

# Interactive mode - enter names from config file
python3 ~/.hagent/plugins/platforms/omnichannel/send_name_only.py

# Zalo - API-based phone number sending (fallback only)
python3 ~/.hagent/plugins/platforms/omnichannel/send_zalo.py <phone> '<message>'
```
...
python3 ~/.hagent/plugins/platforms/omnichannel/zalo_telegram_bot.py setup

# Then run in background  
nohup python3 ~/.hagent/plugins/platforms/omnichannel/zalo_telegram_bot.py monitor >> logs/zalo-tg.log 2>&1 &
```

### View Zalo Messages (Browser Automation)

```bash
python3 ~/.hagent/plugins/platforms/omnichannel/view_zalo_messages.py
...
Store contacts in `~/.hagent/omnichannel_contacts.json`:

```bash
cat > ~/.hagent/omnichannel_contacts.json << 'EOF'
...
```bash
python3 ~/.hagent/plugins/platforms/omnichannel/send_name_only.py Đại 'Chào bạn!'
...
```python
# Cookie-based auth (store in ~/.hagent/omnichannel.env):
...
```python
# Use existing bot token from ~/.hagent/telegram_token.json or similar
...
1. **Check existing credentials** - Load from `~/.hagent/omnichannel.env` or similar
