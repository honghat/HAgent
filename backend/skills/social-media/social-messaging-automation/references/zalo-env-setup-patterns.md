...
```bash
cat > backend/omnichannel.env << 'EOF'
...
def get_auth_credentials():
    ...
    env_file = Path(os.environ['HAGENT_HOME']) / 'omnichannel.env'
    if env_file.exists():
    ...
    creds_file = Path(os.environ['HAGENT_HOME']) / 'zalo_creds.json'
    ...
    oauth_file = Path(os.environ['HAGENT_HOME']) / 'zalo_oauth.json'
...
def reuse_existing_session():
    ...
    cookies_file = Path(os.environ['HAGENT_HOME']) / 'zalo_cookies.json'
...
✅ **DO:** Use `backend/` (user-writable path, not `/etc` or system dirs)
...
cat backend/omnichannel.env | grep ZALO_COOKIE_STRING
...
with sync_playwright() as p:
    ...
    # Load saved cookies
    try:
        with open('$HAGENT_HOME/zalo_cookies.json') as f:
            cookies = json.load(f)
