...
export async function openClaudeDeepSeek() {
  try {
    ...
    const command = `osascript -e 'tell application "Terminal"
      activate
      do script "export TELEGRAM_BOT_TOKEN=\\"8079757619:AAHOsEAKci-oQ8M87fp-lysd1z3jOCKIJyo\\" && export ANTHROPIC_MODEL=\\"deepseek-v4-flash\\" && cd \\"${projectRoot}\\" && claude \\"${initialPrompt}\\" --channels plugin:telegram@claude-plugins-official"
    end tell'`;
    await execAsync(command);
