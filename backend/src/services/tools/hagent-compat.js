...
import { formatHagentPythonResult, hagentPythonCallTool } from '../hagent/python-bridge.js';
...
function patchHandler(args = {}) {
  ...
  if (typeof args.patch === 'string' || typeof args.diff === 'string') {
    ...
    const tmp = path.join(PROJECT_ROOT, 'data', `hagent-compat-${Date.now()}.patch`);
...
function unavailable(name, replacement = '') {
  return () => [
    `Tool Hagent "${name}" đã được nhận diện trong HAgent nhưng chưa được port lõi native.`,
    replacement ? `Dùng tool thay thế hiện có: ${replacement}.` : '',
  ].filter(Boolean).join('\n');
}

function hagentPythonFallback(name, replacement = '') {
  return async (args = {}, context = {}) => {
    try {
      ...
      return [
        `Hagent Python runtime không chạy được tool "${name}": ${err.message}`,
...
registerCompatTool({
  name: 'terminal',
  description: 'Hagent-compatible shell execution. Alias của bash.',
...
registerCompatTool({
  name: 'search_files',
  description: 'Hagent-compatible file search. Alias của grep.',
...
registerCompatTool({
  name: 'patch',
  description: 'Hagent-compatible patch tool. Hỗ trợ oldString/newString hoặc unified diff qua git apply.',
...
registerCompatTool({
  name: 'delegate_task',
  description: 'Hagent-compatible delegation. Alias của HAgent agent tool.',
...
registerCompatTool({
  name: 'memory',
  description: 'Hagent-compatible memory backed by HAgent wiki.',
...
registerCompatTool({
  name: 'session_search',
  description: 'Hagent-compatible session recall backed by HAgent wiki/RAG.',
  parameters: { type: 'object', properties: { query: { type: 'string' } }, required: ['query'] },
  handler: sessionSearchHandler,
});

registerCompatTool({ name: 'skills_list', description: 'Hagent-compatible skills list.', parameters: { type: 'object', properties: {} }, handler: skillsListHandler });
registerCompatTool({ name: 'skill_view', description: 'Hagent-compatible skill view.', parameters: { type: 'object', properties: { name: { type: 'string' }, skill: { type: 'string' } } }, handler: skillViewHandler });
registerCompatTool({ name: 'skill_manage', description: 'Hagent-compatible skill management backed by wiki notes.', parameters: { type: 'object', properties: { action: { type: 'string' }, name: { type: 'string' }, content: { type: 'string' } } }, handler: skillManageHandler });
registerCompatTool({ name: 'web_extract', description: 'Hagent-compatible web extraction. Alias của fetch_url/web_search.', parameters: { type: 'object', properties: { url: { type: 'string' }, query: { type: 'string' } } }, handler: webExtractHandler });

registerCompatTool({
  name: 'execute_code',
  description: 'Hagent-compatible code execution. Uses Hagent execute_code first, then HAgent native Python if the Hagent RPC sandbox is blocked.',
  ...
  handler: async (args = {}, context = {}) => {
    try {
      ...
      if (hagentExecuteCodeSandboxDenied(payload)) {
...
registerCompatTool({ name: 'process', description: 'Hagent process registry compatibility. Python fallback to Hagent process_registry.', parameters: { type: 'object', properties: { action: { type: 'string' } } }, handler: hagentPythonFallback('process', 'task_start, task_output, task_stop, task_list') });
registerCompatTool({ name: 'image_generate', description: 'Hagent image generation compatibility. Python fallback to Hagent image_generation_tool.', parameters: { type: 'object', properties: { prompt: { type: 'string' } } }, handler: hagentPythonFallback('image_generate', 'skill image-generation hoặc backend image-generation skill') });
registerCompatTool({
  name: 'vision_analyze',
  description: 'Hagent vision compatibility. Alias của view_image cho ảnh local.',
...
registerCompatTool({ name: 'video_analyze', description: 'Hagent video analysis compatibility. Python fallback to Hagent vision_tools.', parameters: { type: 'object', properties: { path: { type: 'string' } } }, handler: hagentPythonFallback('video_analyze', 'dub_video hoặc video-generation/video-dubbing skills') });
registerCompatTool({ name: 'text_to_speech', description: 'Hagent TTS compatibility. Python fallback to Hagent tts_tool.', parameters: { type: 'object', properties: { text: { type: 'string' }, voice: { type: 'string' } } }, handler: hagentPythonFallback('text_to_speech', 'backend video/tts service hoặc podcast-generation skill') });
registerCompatTool({
  name: 'cronjob',
  description: 'Hagent-compatible cron job management. Maps to HAgent cron_create/cron_delete/cron_list.',
...
registerCompatTool({
  name: 'clarify',
  description: 'Hagent-compatible clarification request. Alias của ask_user.',
...
registerCompatTool({
  name: 'send_message',
  description: 'Hagent-compatible outbound message gateway. Supports telegram/zalo/facebook through HAgent gateway.',
...
for (const name of [
  ...
  registerCompatTool({
    name,
    description: `Hagent Python compatibility bridge for ${name}.`,
    parameters: { type: 'object', properties: {} },
    handler: hagentPythonFallback(name),
  });
}
