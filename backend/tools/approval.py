...
from tools.file_placement_policy import validate_shell_file_creation
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("HAGENT_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set HAGENT_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind HAGENT_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds HAGENT_SESSION_PLATFORM via
    ...
    """
    if os.getenv("HAGENT_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $HAGENT_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HAGENT_ENV_PATH = (
    r'(?:~\/backend/|'
    r'(?:\$home|\$\{home\})/backend/|'
    r'(?:\$hagent_home|\$\{hagent_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhagent\s+gateway\s+(stop|restart)\b', "stop/restart hagent gateway (kills running agents)"),
    (r'\bhagent\s+update\b', "hagent update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart hagent-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart hagent-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(hagent|gateway|cli\.py)\b', "kill hagent/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill hagent` but not
    # `kill -9 $(pgrep -f hagent)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("HAGENT_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("HAGENT_INTERACTIVE")
    ...
    if is_gateway or os.getenv("HAGENT_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    placement_decision = validate_shell_file_creation(command)
    if not placement_decision.allowed:
        return {
            "approved": False,
            "message": (
                "BLOCKED (file placement): "
                + placement_decision.reason
                + " Use write_file/patch with the correct path instead of shell redirection."
            ),
            "pattern_key": "file_placement",
            "description": "file placement policy",
        }

    # --yolo or approvals.mode=off: bypass all approval prompts.
    # Gateway /yolo is session-scoped; CLI --yolo remains process-scoped.
    approval_mode = _get_approval_mode()
    if is_truthy_value(os.getenv("HAGENT_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("HAGENT_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = os.getenv("HAGENT_EXEC_ASK")
