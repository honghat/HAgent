...
One ``CDPSupervisor`` runs per Hagent ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "hagent-dialog-bridge.invalid"
...
(() => {
  if (window.__hagentDialogBridgeInstalled) return;
  window.__hagentDialogBridgeInstalled = true;
  const ENDPOINT = "http://hagent-dialog-bridge.invalid/";
