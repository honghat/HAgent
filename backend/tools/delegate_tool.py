...
# (hagent-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("hagent-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``hagent-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs ongateway Hat Nguyen Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. hagent-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g.gateway Hat Nguyen Portal).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.hagent/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'hagent auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (hagent_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Hagent subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
