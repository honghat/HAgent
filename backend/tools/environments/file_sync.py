...
def iter_sync_files(container_base: str = "/backend") -> list[tuple[str, str]]:
    ...
    the hardcoded /backend to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, hagent_home: Path | None = None) -> None:
        ...
        Downloads the remote ``backend/`` directory as a tar archive,
        ...
        # against an uninitialized remote backend/ directory.
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/backend/skills/a.md`` →
        ``backend/skills/a.md``, a new remote file at
        ``/backend/skills/b.md`` maps to ``backend/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
