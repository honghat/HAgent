...
from tools.file_placement_policy import validate_new_file_path
...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__HAGENT_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__HAGENT_FENCE_" in line or "\x1b]" in line
...
def _parse_search_context_line(line: str) -> Optional[tuple[str, int, str]]:
...
class ShellFileOperations(FileOperations):
    ...
    def _effective_cwd(self, cwd: str = None) -> str:
        """Return the cwd that this file operation will use for shell commands."""
    ...
    def _path_exists(self, path: str) -> bool:
        result = self._exec(f"test -e {self._escape_shell_arg(path)}")
        return result.exit_code == 0

    def _validate_new_file_placement(
        self,
        path: str,
        *,
        content: str = "",
        ...
        if self._path_exists(path):
            return None
        decision = validate_new_file_path(
            path,
            content=content,
    ...
    def move_file(self, src: str, dst: str) -> WriteResult:
        ...
        placement_error = self._validate_new_file_placement(dst)
        if placement_error:
            return WriteResult(error=placement_error)
        result = self._exec(
    ...
    def write_file(
        ...
        placement_error = self._validate_new_file_placement(
            path,
            content=content,
            placement_type=placement_type,
        )
