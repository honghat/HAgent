...
def write_file_tool(
    ...
    try:
        ...
        if _resolved is None:
            ...
            file_ops = _get_file_ops(task_id)
            result = file_ops.write_file(path, content, placement_type=placement_type)
            result_dict = result.to_dict()
            if stale_warning:
...
WRITE_FILE_SCHEMA = {
    "name": "write_file",
    "description": "Write content to a file, completely replacing existing content. Use this instead of echo/cat heredoc in terminal. Creates parent directories automatically. OVERWRITES the entire file — use 'patch' for targeted edits. For new files, choose the correct project directory and optionally set placement_type so runtime can enforce file placement. Auto-runs syntax checks on .py/.json/.yaml/.toml and other linted languages; only NEW errors introduced by this write are surfaced (pre-existing errors are filtered out).",
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "placement_type": {
                ...
                "description": "Optional intent for new-file placement validation. Existing files can be overwritten without this.",
            },
        },
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with hagent_tools.write_file() for very "
            "large files."
        )
    ...
    return write_file_tool(
        ...
        task_id=tid,
        placement_type=args.get("placement_type"),
    )
...
registry.register(name="read_file", toolset="file", schema=READ_FILE_SCHEMA, handler=_handle_read_file, check_fn=_check_file_reqs, emoji="📖", max_result_size_chars=100_000, plan_safe=True)
registry.register(name="write_file", toolset="file", schema=WRITE_FILE_SCHEMA, handler=_handle_write_file, check_fn=_check_file_reqs, emoji="✍️", max_result_size_chars=100_000)

# Aliases to help models that guess these names
registry.register(name="read", toolset="file", schema=READ_FILE_SCHEMA, handler=_handle_read_file, check_fn=_check_file_reqs, emoji="📖", max_result_size_chars=100_000, plan_safe=True)
registry.register(name="write_to_file", toolset="file", schema=WRITE_FILE_SCHEMA, handler=_handle_write_file, check_fn=_check_file_reqs, emoji="✍️", max_result_size_chars=100_000)

registry.register(name="patch", toolset="file", schema=PATCH_SCHEMA, handler=_handle_patch, check_fn=_check_file_reqs, emoji="🔧", max_result_size_chars=100_000)
registry.register(name="search_files", toolset="file", schema=SEARCH_FILES_SCHEMA, handler=_handle_search_files, check_fn=_check_file_reqs, emoji="🔎", max_result_size_chars=100_000, plan_safe=True)
