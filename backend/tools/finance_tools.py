...
async def get_gold_price() -> str:
    ...
    try:
        ...
        for tr_match in re.finditer(r"<tr[^>]*>(.*?)</tr>", html, re.DOTALL):
            ...
            if len(cols) >= 3:
                ...
                if type_text and buy_clean.replace(".", "").isdigit():
                    try:
                        buy_val = float(buy_clean)
                        sell_clean = sell_text.replace(",", "")
                        sell_val = float(sell_clean)
        ...
        if not rows:
            fallback = re.search(r"Cập nhậ[t|p].*?(\d[\d:/\s]+)", html)
            if fallback:
        ...
        time_match = re.search(r"Cập nhậ[t|p] lúc:?\s*([^<]+)", html, re.I)
