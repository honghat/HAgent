...
selectable via ``hagent tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users ongateway Hat Nguyen Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keepsNous
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (gateway Hat Nguyen Subscription)
# ---------------------------------------------------------------------------
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                f"gateway Hat Nguyen Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"thegateway Hat Nguyen Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `hagent tools` → Image Generation."
            ) from exc
        raise
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping hagent with a FAL key configured
...
IMAGE_GENERATE_SCHEMA = {
    ...
    "description": (
        ...
        "Optional `workflow` overrides the ComfyUI workflow JSON / preset — "
        "call `list_comfyui_workflows` first to see available presets "
        "(e.g. 'stickman.json' for stick-figure style)."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "workflow": {
                "type": "string",
                "description": (
                    "Optional ComfyUI workflow filename. Preset (kind=preset) "
                    "patches prompt prefix + negative on top of the user's "
                    "current model; template (kind=template) replaces the "
                    "full graph. Get list via `list_comfyui_workflows`."
                ),
            },
        },
        "required": ["prompt"],
...
def _dispatch_to_plugin_provider(prompt: str, aspect_ratio: str, workflow: str | None = None):
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `hagent plugins list` to see "
...
def _handle_image_generate(args, **kw):
    ...
    workflow = (args.get("workflow") or "").strip() or None

    # Route to a plugin-registered provider if one is active (and it's
    # not the in-tree FAL path).
    dispatched = _dispatch_to_plugin_provider(prompt, aspect_ratio, workflow)
