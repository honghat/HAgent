"""Central registry for all hagent-agent tools.
...
def get_agent_mode() -> str:
    """Return the active agent mode: ``build`` by default, ``plan`` when opted in."""
    raw_mode = (
        os.environ.get("HAGENT_AGENT_MODE")
        or os.environ.get("HAGENT_MODE")
        or ""
    ).strip().lower()
...
def is_plan_mode() -> bool:
    """Return True when runtime tool safety should enforce Plan mode rules."""
    return get_agent_mode() == "plan"
...
def _is_registry_register_call(node: ast.AST) -> bool:
    ...
    if (
        ...
        return True
    # Also accept helper wrappers like `register_llm_tool(cfg)` used by
    # the shared ask_<LLM> tools — they end up calling registry.register
    # internally.
    if isinstance(func, ast.Name) and func.id == "register_llm_tool":
        return True
    return False
...
def _module_registers_tools(module_path: Path) -> bool:
    ...
    Inspects module-level statements and descends into module-level control
    flow (for/while/with/if/try) so loop-driven registration (e.g.
    monitor_tools registers in a ``for`` loop) is detected — but NOT into
    function or class bodies, so helper modules that only call
    ``registry.register()`` inside a function are still excluded.
    """
    try:
    ...
    def _scan(stmts) -> bool:
        for stmt in stmts:
            if _is_registry_register_call(stmt):
                return True
            # Don't descend into function/class bodies — those calls run later.
            if isinstance(stmt, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                continue
            for field in ("body", "orelse", "finalbody"):
                block = getattr(stmt, field, None)
                if block and _scan(block):
                    return True
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    """Import all built-in self-registering tool modules eagerly and return their module names.

    Every tool module in the tools/ directory is imported at startup so the
    full registry is populated before the agent builds its tool list. There is
    no lazy-loading — all toolsets are always available from the first turn.
    """
    tools_path = Path(tools_dir) if tools_dir is not None else Path(__file__).resolve().parent

    # Build the full candidate list once via AST scan.
    all_modules = [
        f"tools.{path.stem}"
...
class ToolEntry:
    ...
    __slots__ = (
        ...
        "max_result_size_chars", "dynamic_schema_overrides", "plan_safe",
    )
...
# timescales. Cache results for ~30 s so env-var flips via ``hagent tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``hagent tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``hagent tools enable foo``)
        ...
        enforce_plan_safe = is_plan_mode()
        entries_by_name = {entry.name: entry for entry in self._snapshot_entries()}
        for name in sorted(tool_names):
            entry = entries_by_name.get(name)
            if not entry:
                continue
            if enforce_plan_safe and not entry.plan_safe:
                if not quiet:
                    logger.warning("Tool %s hidden in Plan mode (plan_safe=false)", name)
                continue
            if entry.check_fn:
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str:
        ...
        * Broadcasts ``agent.tool_call`` events (start + finish) for UI
          observability — failures here never break dispatch.
        """
        entry = self.get_entry(name)
        if not entry:
            return json.dumps({"error": f"Unknown tool: {name}"})
        if is_plan_mode() and not entry.plan_safe:
            try:
                from tools.tool_audit import safe_append_tool_audit
                safe_append_tool_audit(
                    phase="blocked",
                    tool_name=name,
                    ...
                    args=args,
                    error="blocked in Plan mode: plan_safe=false",
                )
            except Exception:
                pass
            return json.dumps({
                "error": (
                    f"Tool '{name}' is blocked in Plan mode because it is not "
                    "marked plan_safe. Switch to Build mode or use a read-only "
                    "planning tool."
                )
            }, ensure_ascii=False)
        ...
        def _safe_broadcast(payload: dict) -> None:
            try:
                ...
                broadcast_agent_event("agent.tool_call", payload)
            except Exception:
                pass
        ...
        try:
            ...
            safe_append_tool_audit = None
        _safe_broadcast({
            "name": name,
            "toolset": entry.toolset,
            "emoji": entry.emoji,
            "phase": "start",
            "args_preview": (json.dumps(args, ensure_ascii=False)[:160]
        ...
        try:
            ...
            _safe_broadcast({
                "name": name, "toolset": entry.toolset, "phase": "done",
                "duration_ms": int((_t.time() - _start) * 1000),
            })
            if safe_append_tool_audit is not None:
                safe_append_tool_audit(
                    phase="done",
                    tool_name=name,
                    toolset=entry.toolset,
            ...
            duration_ms = int((_t.time() - _start) * 1000)
            _safe_broadcast({
                "name": name, "toolset": entry.toolset, "phase": "error",
    ...
    def is_plan_safe(self, name: str) -> bool:
        """Return whether a registered tool is allowed in Plan mode."""
        entry = self.get_entry(name)
        return bool(entry and entry.plan_safe)
...
def tool_result(data=None, **kwargs) -> str:
    ...
    Khi ``HAGENT_TOKEN_COMPRESS=1`` (mặc định), text fields được nén qua
    ``tools.token_compress`` để tiết kiệm context tokens.
    ...
    """
    try:
        from tools.token_compress import compress_payload
    except Exception:
        compress_payload = lambda x: x  # noqa: E731
