...
# Path to tinker-atropos submodule (relative to hagent-agent root)
HAGENT_ROOT = Path(__file__).parent.parent
TINKER_ATROPOS_ROOT = HAGENT_ROOT / "tinker-atropos"
ENVIRONMENTS_DIR = TINKER_ATROPOS_ROOT / "tinker_atropos" / "environments"
CONFIGS_DIR = TINKER_ATROPOS_ROOT / "configs"
LOGS_DIR = get_hagent_home() / "logs" / "rl_training"
...
async def rl_start_training() -> str:
    ...
    if not os.getenv("TINKER_API_KEY"):
        return json.dumps({
            "error": "TINKER_API_KEY not set. Add it to ~/.hagent/.env",
        }, indent=2)
