#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Hagent Skills Hub.
...
Used by hagent_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
from hagent_constants import get_hagent_home, get_skills_dir
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hagent_meta = metadata.get("hagent", {})
            if isinstance(hagent_meta, dict):
                tags = hagent_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained byNous Research) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    backend/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hagent_meta = meta_block.get("hagent", {})
                if isinstance(hagent_meta, dict):
                    tags = hagent_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Hagent centralized index source
# ---------------------------------------------------------------------------

HAGENT_INDEX_URL = "https://hagent-agent.nousresearch.com/docs/api/skills-index.json"
HAGENT_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "hagent-index.json"
HAGENT_INDEX_TTL = 6 * 3600  # 6 hours


def _load_hagent_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for HAGENT_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if HAGENT_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - HAGENT_INDEX_CACHE_FILE.stat().st_mtime
            if age < HAGENT_INDEX_TTL:
                return json.loads(HAGENT_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(HAGENT_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Hagent index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Hagent index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        HAGENT_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        HAGENT_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class HagentIndexSource(SkillSource):
    """Skill source backed by the centralized Hagent Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_hagent_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "hagent-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "hagent-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        HagentIndexSource(auth=auth), # Centralized index (search + resolved install paths)
