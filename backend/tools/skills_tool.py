...
from hagent_constants import get_hagent_home, display_hagent_home, get_skills_dir
import os
import re
...
# All skills live in the project under backend/agent/app/skills/.
# This is the single source of truth for listing, viewing, and agent edits.
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from HAGENT_HOME/.env."""
    env_path = get_hagent_home() / ".env"
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_hagent_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: backend/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``HAGENT_PLATFORM`` environment variable
    3. ``HAGENT_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from hagent_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("HAGENT_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in backend/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``hagent skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_hagent_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from hagent_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: hagent plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (backend/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `hagent skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.hagent.* first (agentskills.io convention), fall back to top-level
        hagent_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            hagent_meta = metadata.get("hagent", {}) or {}

        tags = _parse_tags(hagent_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            hagent_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
