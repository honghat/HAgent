...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Hagent exit
...
# display_hagent_home imported lazily at call site (stale-module safety during hagent update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of hagent directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "hagent-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("HAGENT_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("HAGENT_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (HAGENT_INTERACTIVE=1).
    ...
    try:
        os.environ["HAGENT_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "HAGENT_SPINNER_PAUSE" in os.environ:
            del os.environ["HAGENT_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (HAGENT_INTERACTIVE=1):
    ...
    # interactive Hagent password prompt or the sudo -S password-pipe path.
    ...
    # silently without Hagent prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and os.getenv("HAGENT_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Hagent can track lifecycle and output.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, HagentSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.hagent/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    if env_type in ("modal", "singularity", "daytona", "vercel_sandbox") and cwd:
    ...
    return {
        ...
        # Container resource config (applies to singularity, modal,
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    Args:
        env_type: One of "local", "singularity", "modal",
            "daytona", "vercel_sandbox", "ssh"
        image: Singularity/Modal image name (ignored for local/ssh/vercel)
        ...
        host_cwd: Optional host working directory for backends that support it
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "a paidNous subscription is required for the Tool Gateway and no direct "
                    "Modal credentials/config were found. Log in with `hagent model` or "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(command):
        return (
            ...
            "Use terminal(background=true) so Hagent can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if _gw_platform:
                        ...
                        _gw_thread_id = _gse("HAGENT_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("HAGENT_SESSION_USER_ID", "")
                        _gw_user_name = _gse("HAGENT_SESSION_USER_NAME", "")
            ...
            try:
                from hagent_cli.plugins import invoke_hook
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a paidNous subscription is required for the Tool Gateway and no direct "
                        "Modal credentials/config were found. Log in with `hagent model` "
