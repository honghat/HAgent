...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/hagent-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/hagent-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/hagent-results"
HEREDOC_MARKER = "HAGENT_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/hagent-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"HAGENT_PERSIST_{uuid.uuid4().hex[:8]}"
