...
export default function AgentManager({ token, agents, onUpdate, embedded = false }) {
  ...
  const filteredAgents = agents.filter((agent) => {
    const text = `${agent.name || ''} ${agent.description || ''} ${agent.model || ''}`.toLowerCase()
        ...
        <div className="max-w-3xl mx-auto w-full">
          <div className="bg-white border border-gray-200 rounded-lg p-4 md:p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-base font-semibold text-gray-900 tracking-tight">{editing === 'new' ? 'Khởi tạo agent' : 'Chỉnh sửa agent'}</h2>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <select value={model} onChange={e => setModel(e.target.value)}
                    className="w-full bg-gray-50 border border-gray-200 focus:border-gray-400 rounded-md px-3 py-2 text-sm outline-none cursor-pointer font-medium text-gray-700 appearance-none">
              ...
              <div className="space-y-2">
                ...
                <input value={desc} onChange={e => setDesc(e.target.value)}
                  ...
                  className="w-full bg-gray-50 border border-gray-200 focus:border-gray-400 rounded-md px-3 py-2 text-sm outline-none transition-all text-gray-600" />
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea value={soul} onChange={e => setSoul(e.target.value)}
                  ...
                  className="w-full bg-gray-50 border border-gray-200 focus:border-gray-400 rounded-md px-3 py-2 text-sm font-medium text-gray-600 leading-relaxed transition-all" />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-3 rounded-md border border-gray-200">
                <div className="space-y-3">
                  ...
                  <input type="number" value={intervalSec} onChange={e => setIntervalSec(Number(e.target.value))}
                    ...
                    className="w-full bg-white border border-gray-200 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gray-200 transition-all font-medium text-gray-700" />
                </div>
                <div className="flex flex-col justify-center">
                  <label className="flex items-center gap-3 cursor-pointer group">
                    ...
                    <span className="text-sm font-medium text-gray-700 group-hover:text-black transition-colors">Tự khởi động</span>
                  </label>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-medium text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-full">{todos.length} việc</span>
                </div>
                <div className="flex gap-2">
                  <input value={todoInput} onChange={e => setTodoInput(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter') addTodo() }}
                    placeholder="Nhập đầu việc mới..."
                    className="flex-1 bg-white border border-gray-200 rounded-md px-3 py-2 text-sm outline-none focus:border-indigo-300 transition-all" />
                ...
                <div className="space-y-2 max-h-56 overflow-y-auto pr-2 custom-scrollbar">
                  {todos.length === 0 ? (
                    ...
                    todos.map(t => (
                      <div key={t.id} className="flex items-center gap-3 bg-white border border-gray-200 rounded-md px-3 py-2 group hover:border-indigo-100 transition-all">
      ...
      <div className="max-w-6xl mx-auto w-full">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
          <div className="space-y-1">
            <h1 className="text-base font-semibold text-gray-900">AI agent</h1>
          </div>
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <button onClick={startNew}
              className="bg-gray-900 hover:bg-black text-white px-3 py-2 rounded-md text-[11px] font-medium transition-all active:scale-95 shrink-0 flex items-center justify-center gap-2 w-full sm:w-auto">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="space-y-1 overflow-hidden pr-2">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <h3 className="font-semibold text-gray-900 tracking-tight truncate text-sm">{a.name}</h3>
                      </div>
                      <div className="flex flex-wrap gap-1.5">
                        <span className="text-[8px] font-semibold text-indigo-500 bg-indigo-50/50 px-1.5 py-0.5 rounded border border-indigo-100/30">{a.model || 'Model'}</span>
