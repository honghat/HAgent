...
import { 
  ...
  Menu, X, ChevronRight, LayoutGrid, Award, Settings2, Trash2, Clock
} from 'lucide-react'
import ReactMarkdown from 'react-markdown'
import remarkGfm from 'remark-gfm'
import { useSpeechToText } from '../hooks/useSpeechToText.js'
import LearnRecall from './LearnRecall.jsx'
import { fetchEnglishReviewQueue, relativeReviewLabel, strengthColor } from '../lib/recallApi.js'
...
export default function English({ token, provider, cxModel }) {
  ...
  // Recall (Feynman teach-back + shadow) state
  const [recallTarget, setRecallTarget] = useState(null)
  const [recallMode, setRecallMode] = useState('english') // 'english' | 'shadow'
  ...
  const speechToText = useSpeechToText({
    ...
    onTranscript: (text) => {
      ...
      setUserText(current => current.trim() ? `${current.trim()} ${nextText}` : nextText)
      setFeedback('')
    },
    onError: (message) => setFeedback(message || 'STT English lỗi.'),
  })
  ...
  useEffect(() => {
    ...
    async function loadDue() {
      try {
        const data = await fetchEnglishReviewQueue(token)
  ...
  function handleRecallCompleted(result) {
    setItems(prev => prev.map(item => {
      if (String(item.id) !== String(result?.itemId)) return item
  ...
  const handleDelete = async (id, e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Xoá thất bại: ${err.detail || 'Lỗi không xác định'}`)
      }
  ...
  const handleDeleteUnit = async (unitId) => {
    try {
      const res = await fetch(`/api/english/unit/${unitId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      })
      const data = await res.json()
      if (res.ok) {
        if (currentUnit?.id === unitId) setCurrentId(null)
        load()
      }
    } catch { alert('Xoá unit thất bại') }
  }
...
Câu tốt hơn:`
  ...
  const handleToggleLearned = async () => {
    if (!currentItem || busy) return
    await updateLearnedState(currentItem, !currentItem.completed)
  }

  const updateLearnedState = async (item, nextCompleted, event) => {
    if (!item || busy) return
    event?.stopPropagation()
    setBusy(`learned-${item.id}`)
    try {
      const res = await fetch('/api/english', {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: String(item.id), completed: nextCompleted, incrementLearnCount: nextCompleted })
      })
      if (!res.ok) throw new Error('Không cập nhật được bài học')
      const updated = await res.json().catch(() => null)
      if (updated?.id) {
        setItems(prev => prev.map(item => item.id === updated.id ? updated : item))
        if (currentItem?.id === updated.id || nextCompleted) setCurrentId(updated.id)
      } else {
        await load()
      }
    } catch (err) {
    ...
    <div className="flex h-full w-full bg-white text-slate-900 font-sans overflow-hidden">
      ...
      <aside className="hidden lg:flex flex-col w-52 border-r border-slate-100 bg-[#FBFBFC] shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
          {units.map(u => (
            <div key={u.id} className="flex items-center gap-1 pr-1">
              <button
                onClick={() => { setSelectedUnit(u.id); setCurrentId(null); }}
                className={`flex-1 text-left p-2 rounded-md transition-all min-w-0 ${selectedUnit === u.id || (!selectedUnit && u.id === units[0]?.id) ? 'bg-white border border-slate-200 text-slate-800' : 'text-slate-500 hover:bg-slate-50'}`}
              >
              ...
              <button
                ...
                className="shrink-0 flex items-center justify-center w-5 h-5 rounded text-slate-300 hover:text-rose-500 hover:bg-rose-50 transition-colors"
                title="Xoá toàn bộ bài học trong bài này"
              >
                <Trash2 size={10} />
              </button>
            </div>
              ...
              {filteredItems.map(item => (
                ...
                >
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-1.5">
                    <button
                      type="button"
                      onClick={(event) => updateLearnedState(item, !item.completed, event)}
                      disabled={!!busy}
                      className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[9px] font-black uppercase transition-all disabled:opacity-50 ${
                        isLearned(item)
                    ...
                    >
                      <Check size={10} />
                      {isLearned(item) ? 'Đã học' : 'Đánh dấu'}
                    </button>
                    <button
                      type="button"
                      onClick={(event) => { event.stopPropagation(); setRecallTarget(item); setRecallMode('english') }}
                      className="inline-flex items-center gap-1 rounded-md border border-indigo-200 bg-indigo-50 px-2 py-1 text-[9px] font-black uppercase text-indigo-700 transition-all hover:bg-indigo-100"
                      title="Tự giảng lại theo Feynman"
                    >
                      <Mic size={10} />
                      Giảng lại
                    </button>
                    {(item.strength > 0 || item.reviewCount > 0) && (
                      <span className={`rounded-full border px-1.5 py-0.5 text-[8px] font-black ${strengthColor(item.strength)}`}>
                        {item.strength || 0}
                      </span>
                    )}
                  </div>
              ...
              <div className="max-w-6xl mx-auto space-y-6">
                <header className="pb-4 border-b border-slate-100 flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    {(currentItem.nextReviewAt || currentItem.reviewCount > 0) && (
                      <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10px] text-slate-500">
                        <Clock size={10} />
                        <span>{relativeReviewLabel(currentItem.nextReviewAt)}</span>
                        {(currentItem.strength > 0 || currentItem.reviewCount > 0) && (
                          <span className={`rounded-full border px-1.5 py-0.5 font-black ${strengthColor(currentItem.strength)}`}>
                            strength {currentItem.strength || 0}
                          </span>
                        )}
                        {currentItem.reviewCount > 0 && <span>· {currentItem.reviewCount} lần ôn</span>}
                      </div>
                    )}
                  </div>
                  <div className="flex shrink-0 items-center gap-1.5">
                    ...
                    {(currentItem.type === 'speak' || currentItem.type === 'listen' || currentItem.type === 'vocab') && (
                      <button
                        onClick={() => { setRecallTarget(currentItem); setRecallMode('shadow') }}
                        className="flex items-center gap-1.5 rounded-md border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-[10px] font-bold text-sky-700 transition-all hover:bg-sky-100"
                        title="Nghe mẫu rồi nói lại (shadowing)"
                      >
                        <Mic size={12} />
                        Shadow
                      </button>
                    ...
                    <button
                      onClick={() => { setRecallTarget(currentItem); setRecallMode('english') }}
                      className="flex items-center gap-1.5 rounded-md border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 text-[10px] font-bold text-indigo-700 transition-all hover:bg-indigo-100"
                      title="Tự giảng lại theo Feynman"
                    >
                      <Mic size={12} />
                      Giảng lại
                    </button>
                    <button onClick={(e) => handleDelete(currentItem.id, e)}
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md font-bold text-[10px] text-rose-600 border border-rose-200 hover:bg-rose-50 transition-colors">
                      <Trash2 size={12} />
                      Xoá
                    </button>
                  </div>
                ...
                {dueCount > 0 && (
                  <div className="mt-3 flex items-center gap-2 rounded-lg border border-indigo-100 bg-indigo-50/60 px-3 py-2 text-[11px] text-indigo-700">
                    <Clock size={12} />
                    <span className="font-semibold">Có {dueCount} mục tiếng Anh đến hạn ôn.</span>
                    <span className="text-indigo-500">Bấm Mic để giảng lại theo Feynman.</span>
                  </div>
                )}
                  ...
                  {currentItem.type === 'speak' && (
                    <div className="p-6 bg-slate-900 rounded-2xl text-white flex flex-col items-center gap-4">
                      <button
                        ...
                        className={`w-12 h-12 rounded-full flex items-center justify-center disabled:opacity-50 ${speechToText.recording ? 'bg-rose-500 animate-pulse' : 'bg-white/10 hover:bg-white/20'}`}
                      >
                        {speechToText.recording ? <Square size={20} /> : speechToText.transcribing ? <RefreshCw size={20} className="animate-spin" /> : <Mic size={20} />}
                      </button>
                      <span className="text-[10px] font-bold opacity-60 uppercase tracking-widest">{speechToText.transcribing ? 'Đang nhận diện...' : speechToText.recording ? 'Đang ghi âm...' : 'Nói ngay'}</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex">
          ...
          <aside className="relative w-64 bg-white h-full flex flex-col shadow-2xl animate-in slide-in-from-left duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {units.map(u => (
                <div key={u.id} className="flex items-center gap-1 pr-1">
                  <button onClick={() => { setSelectedUnit(u.id); setCurrentId(null); setShowCurriculum(false); }} className={`flex-1 text-left p-3 rounded-xl border transition-all min-w-0 ${selectedUnit === u.id ? 'bg-indigo-50 border-indigo-100 text-indigo-700' : 'border-transparent text-slate-600'}`}>
                    <div className="text-[8px] font-bold opacity-50 uppercase">Bài {u.id}</div>
                    <div className="text-xs font-bold truncate">{u.title}</div>
                  </button>
                  <button
