...
const TYPES = [
  ...
  { id: 'vocab', label: 'Vocab', icon: <Book size={14} /> },
  { id: 'grammar', label: 'Grammar', icon: <BookOpen size={14} /> },
  { id: 'listen', label: 'Listen', icon: <Headphones size={14} /> },
  { id: 'speak', label: 'Speak', icon: <MessageSquare size={14} /> },
  { id: 'reading', label: 'Read', icon: <BookOpen size={14} /> },
  { id: 'writing', label: 'Write', icon: <PenTool size={14} /> },
]

const LEVELS = ['A1', 'A2', 'B1', 'B2', 'C1']
const MODES = [
  { id: 'coder', label: '💻 Coding' },
  { id: 'communication', label: '🗣️ Daily' },
...
export default function English({ token }) {
  ...
  const units = useMemo(() => {
    ...
    items.forEach(item => {
      ...
      const u = meta.unit || 0
      if (!uMap[u]) uMap[u] = { id: u, title: meta.unitTitle || `Unit ${u}`, items: [] }
  ...
  const handleGenNext = async () => {
    ...
    setBatchMsg('Creating next unit...')
    try {
      ...
      if (res.ok) {
        setBatchMsg('Unit created successfully!')
        load()
      } else {
        const err = await res.json()
        setBatchMsg(`Error: ${err.error || 'Failed to create'}`)
      }
    } catch (e) { setBatchMsg('Connection error') }
    finally { 
  ...
  const handleQuizSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setFeedback(`Quiz Complete! Scored ${score}/${questions.length}. SRS updated.`)
        load()
      }
    } catch { setFeedback('Update failed') }
    finally { setBusy(false) }
  }
  ...
  const handleCheckWriting = async () => {
    ...
    try {
      ...
      const data = await res.json()
      setFeedback(data.feedback || 'Check complete.')
      if (res.ok) {
        // Mark as completed
        fetch('/api/english', {
          method: 'PATCH',
  ...
  const getItemTitle = (item) => {
    ...
    return item.title || m.title || m.word || m.topic || m.prompt || (item.content?.substring(0, 30) + '...') || 'Unit Task'
  }
  ...
  return (
    <div className="flex h-full w-full bg-white text-slate-900 font-sans overflow-hidden">
      ...
      <aside className="hidden lg:flex flex-col w-52 border-r border-slate-100 bg-[#FBFBFC] shrink-0">
        <div className="p-4 border-b border-slate-100">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h1 className="font-bold text-xs tracking-tight">Curriculum</h1>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
          {units.map(u => (
            <button
              ...
              className={`w-full text-left p-2 rounded-lg transition-all ${selectedUnit === u.id || (!selectedUnit && u.id === units[0]?.id) ? 'bg-white shadow-sm border border-slate-200 text-indigo-700' : 'text-slate-500 hover:bg-slate-50'}`}
            >
              <div className="text-[9px] font-bold opacity-50 uppercase tracking-tighter">Unit {u.id}</div>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
          <button 
            ...
            className="w-full py-2.5 rounded-xl bg-slate-900 text-white text-[10px] font-black shadow-lg hover:bg-slate-800 disabled:opacity-50 transition-all flex items-center justify-center gap-2"
          >
            <RefreshCw size={12} className={batchRunning ? 'animate-spin' : ''} />
            {batchRunning ? 'Generating...' : 'New Lesson'}
          </button>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-white">
        ...
        {/* Skill Tabs */}
        <nav className="h-11 border-b border-slate-100 flex items-center gap-1.5 px-4 overflow-x-auto no-scrollbar bg-slate-50/50">
          {TYPES.map(t => (
            <button
              key={t.id}
              onClick={() => { setType(t.id); setShowSkills(true); }}
              className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-bold whitespace-nowrap transition-all ${type === t.id ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:bg-white border border-transparent hover:border-slate-200'}`}
            >
          ...
          <aside className={`absolute inset-y-0 right-0 z-20 w-72 bg-white border-l border-slate-100 transform transition-transform duration-300 md:relative md:translate-x-0 ${showSkills ? 'translate-x-0' : 'translate-x-full md:translate-x-0'}`}>
            <div className="p-3 border-b border-slate-50 flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search tasks..." className="w-full pl-9 pr-3 py-2 bg-slate-100 border-none rounded-xl text-xs font-bold outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1.5 custom-scrollbar">
              {filteredItems.map(item => (
                <button
                  ...
                  className={`group w-full text-left p-3.5 rounded-2xl border transition-all ${currentItem?.id === item.id ? 'bg-white border-indigo-200 shadow-md ring-1 ring-indigo-50' : 'border-transparent text-slate-600 hover:bg-slate-50'}`}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[9px] font-black uppercase text-indigo-500 tracking-wider">{item.type}</span>
            ...
            {currentItem ? (
              <div className="max-w-2xl mx-auto space-y-6">
                <header className="pb-4 border-b border-slate-100 flex items-center justify-between gap-4">
                  <div>
                    <div className="text-[9px] font-black text-indigo-500 uppercase tracking-widest mb-1">Unit {currentUnit?.id} • {currentItem.type}</div>
                    <h1 className="text-lg font-black text-slate-900 leading-tight tracking-tight">{getItemTitle(currentItem)}</h1>
                  </div>
                  <button onClick={() => speak(currentItem.content || meta.word)} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-black text-[10px] text-white shadow-lg ${isPlaying ? 'bg-slate-900' : 'bg-indigo-600 shadow-indigo-100'}`}>
                ...
                <div className="space-y-6">
                  {currentItem.type === 'vocab' && meta.word && (
                    <div className="p-8 bg-slate-50 rounded-[1.5rem] text-center border border-slate-100 shadow-inner">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {meta.def && (
                      <div className="p-4 bg-white border border-slate-100 rounded-xl shadow-sm">
                        <div className="text-[9px] font-black uppercase text-slate-400 mb-2 tracking-widest">Definition</div>
                        <p className="text-sm font-bold text-slate-800 leading-relaxed">{meta.def}</p>
                      </div>
                    )}
                    {meta.ex && (
                      <div className="p-4 bg-white border border-slate-100 rounded-xl shadow-sm">
                        <div className="text-[9px] font-black uppercase text-slate-400 mb-2 tracking-widest">Example</div>
                    ...
                    <div className="pt-8 border-t border-slate-100 space-y-6">
                      ...
                      <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 gap-2">
                              {q.options?.map((opt, oIdx) => (
                                <button
                                  ...
                                  className={`text-left p-3 rounded-xl border-2 font-bold transition-all text-xs ${quizAnswers[qIdx] === oIdx ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-white border-slate-100 text-slate-600 hover:border-slate-200'}`}
                                >
                                  {opt}
                                </button>
                              ))}
                        ...
                        <button 
                          ...
                          className="w-full py-3 rounded-xl bg-slate-900 text-white font-black hover:bg-black disabled:opacity-50 transition-all text-sm shadow-xl"
                        >
                          {busy ? 'Evaluating...' : 'Submit Quiz'}
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  {currentItem.type === 'writing' && (
                    <div className="pt-8 border-t border-slate-100 space-y-4">
                      ...
                      <textarea
                        ...
                        className="w-full h-32 p-4 rounded-xl bg-slate-50 border-2 border-transparent focus:bg-white focus:border-indigo-500 transition-all text-sm font-medium outline-none resize-none"
                      />
                      <button 
                        ...
                        className="w-full py-3 rounded-xl bg-slate-900 text-white font-black hover:bg-black disabled:opacity-50 transition-all text-sm shadow-xl"
                      >
                        {busy ? 'Evaluating...' : 'Check My Writing'}
                      </button>
                    </div>
                  )}
                  ...
                  {currentItem.type === 'speak' && (
                    <div className="p-6 bg-slate-900 rounded-2xl text-white flex flex-col items-center gap-4">
                      ...
                      <span className="text-[10px] font-bold opacity-60 uppercase tracking-widest">{isRecording ? 'Recording...' : 'Speak Now'}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-300">
                ...
                <p className="text-xs font-bold">Select a lesson to begin</p>
              </div>
            )}
          </article>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex">
          ...
          <aside className="relative w-64 bg-white h-full flex flex-col shadow-2xl animate-in slide-in-from-left duration-300">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="font-bold text-xs uppercase tracking-widest">Curriculum</h2>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {units.map(u => (
                <button key={u.id} onClick={() => { setSelectedUnit(u.id); setCurrentId(null); setShowCurriculum(false); }} className={`w-full text-left p-3 rounded-xl border transition-all ${selectedUnit === u.id ? 'bg-indigo-50 border-indigo-100 text-indigo-700' : 'border-transparent text-slate-600'}`}>
                  <div className="text-[8px] font-bold opacity-50 uppercase">Unit {u.id}</div>
