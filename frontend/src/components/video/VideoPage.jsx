...
export default function VideoPage({ token: tokenProp, provider: providerProp }) {
      ...
      <div className="max-w-6xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 mb-4">
          <div className="space-y-1">
            <h1 className="text-base font-semibold text-gray-900 tracking-tight">Dịch Video</h1>
...
function TaskList({ api, onNew, onOpen }) {
  ...
  useEffect(() => { refresh(); const t = setInterval(() => { if (!document.hidden) refresh() }, 5000); return () => clearInterval(t); }, [refresh]);
    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-white border border-gray-200 rounded-lg p-4 sm:p-5">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-semibold text-gray-400 ml-1">Model dịch</label>
            <div className="grid grid-cols-1 gap-1.5">
              {[
                { provider: 'groq', model: 'llama-3.3-70b-versatile', label: 'Llama 3.3 70B', sub: 'Groq · Nhanh' },
                ...
                return (
                  <button key={o.provider + o.model}
                    onClick={() => { setTranslateProvider(o.provider); setTranslateModel(o.model); }}
                    className={`flex items-center justify-between px-3 py-2 rounded-md text-left border transition-all ${active ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-200 text-gray-600 hover:border-gray-400'}`}
                  >
                    <span className="text-[12px] font-semibold">{o.label}</span>
                    <span className={`text-[10px] ${active ? 'text-gray-300' : 'text-gray-400'}`}>{o.sub}</span>
                  </button>
                );
              })}
            </div>
...
function TaskDetail({ api, token, taskId, onBack }) {
  ...
  const parseDownloadPct = useCallback((text) => {
    const m = text.match(/Đang tải:\s*(\d+)%/);
    if (m) setDownloadProgress(parseInt(m[1]));
  }, []);
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setLogs(history);
      history.forEach(l => parseDownloadPct(l.text));
    } catch {}
  }, [api, taskId, parseDownloadPct]);
  ...
  useEffect(() => {
    ...
    es.onmessage = (ev) => {
      try {
        ...
        if (d.message) {
          parseDownloadPct(d.message);
          setLogs(prev => [...prev.slice(-300), { ts: Date.now(), text: d.message }]);
        }
      } catch {}
    };
    return () => es.close();
  }, [taskId, token, parseDownloadPct]);
            ...
            {downloadProgress > 0 && downloadProgress < 100 && (
              <div className="mb-4">
                ...
                <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                  <div className="bg-indigo-500 h-2 rounded-full transition-all duration-500 ease-out" style={{ width: `${downloadProgress}%` }} />
                </div>
              </div>
            )}
