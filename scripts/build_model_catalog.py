#!/usr/bin/env python3
"""Build the Hagent Model Catalog — a centralized JSON manifest of curated models.
...
Hagent CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Hagent release.
...
REPO_ROOT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "backend")
sys.path.insert(0, REPO_ROOT)

# Ensure HAGENT_HOME is set for imports that touch it at module level.
os.environ.setdefault("HAGENT_HOME", REPO_ROOT)
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "hagent-agent repo",
            "docs": "https://hagent-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
            ...
            "nous": {
                "metadata": {
                    "display_name": "gateway Hat Nguyen Portal",
                    "note": (
