...
用法：
  python scripts/chatgpt2api/migrate_storage.py --from json --to postgres
  python scripts/chatgpt2api/migrate_storage.py --from postgres --to git
  python scripts/chatgpt2api/migrate_storage.py --export accounts.json
  python scripts/chatgpt2api/migrate_storage.py --import accounts.json
"""
...
# 添加项目根目录到 Python 路径
PROJECT_ROOT = Path(__file__).resolve().parents[2] / "projects" / "chatgpt2api"
...
示例:
  ...
  # 导出当前数据到 JSON 文件
  python scripts/chatgpt2api/migrate_storage.py --export backup.json
  
  # 从 JSON 文件导入数据
  python scripts/chatgpt2api/migrate_storage.py --import backup.json
