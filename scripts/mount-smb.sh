...
# Pre-warm kết nối Tailscale: lần đầu lạnh đi qua DERP relay, retry tới khi
# port 445 mở (đã chuyển sang direct P2P) thì mount mới không timeout.
for attempt in 1 2 3 4 5; do
    nc -G 5 -z "${SHARE_IP}" 445 >/dev/null 2>&1 && break
    echo "⏳ Đang đánh thức kết nối tới ${SHARE_IP} (lần ${attempt})..."
done
