...
CONFIG_FILE="${BACKEND_DIR}/plugins/platforms/omnichannel/backend/config.json"
...
ensure_omnichannel_backend_started() {
    ...
    mkdir -p "${BACKEND_DIR}/plugins/platforms/omnichannel/backend"
    
    # Create config file
    cat > "${BACKEND_DIR}/plugins/platforms/omnichannel/backend/config.json" << EOF
{
  "enabled": true,
  "port": 8080,
  "qr_login_enabled": true,
  "session_file": "${BACKEND_DIR}/plugins/platforms/omnichannel/backend/omni_chat_session.json",
  "log_level": "INFO"
}
EOF
    
    # Create and run the server
    cd "${BACKEND_DIR}/plugins/platforms/omnichannel/backend"
