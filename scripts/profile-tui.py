#!/usr/bin/env python3
"""Drive the Hagent TUI under HAGENT_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes backend/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with HAGENT_TUI_RESUME set so it
bypasses the hagent_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  HAGENT_PERF_LOG     (default backend/perf.log)
  HAGENT_PERF_NODE    (default node from $PATH)
  HAGENT_TUI_DIR      (default /home/bb/hagent-agent/ui-tui)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests HAGENT_DEV_PERF wiring is broken).
"""
...
try:
    from hagent_constants import get_hagent_home, get_data_home
except ImportError:
    def get_hagent_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("HAGENT_HOME") or "").strip()
        return Path(val) if val else Path(__file__).resolve().parents[1] / "backend"
    from pathlib import Path as _P
    _p = _P(__file__).resolve().parents[1]
    get_data_home = lambda: _p / ".." / "data"  # type: ignore

DEFAULT_TUI_DIR = Path(os.environ.get("HAGENT_TUI_DIR", "/home/bb/hagent-agent/ui-tui"))
DEFAULT_LOG = Path(os.environ.get("HAGENT_PERF_LOG", str(get_hagent_home() / "perf.log")))
DEFAULT_STATE_DB = get_data_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — HAGENT_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["HAGENT_DEV_PERF"] = "1"
    env["HAGENT_DEV_PERF_MS"] = str(args.threshold_ms)
    env["HAGENT_DEV_PERF_LOG"] = str(log)
    env["HAGENT_TUI_RESUME"] = sid
    ...
    node = os.environ.get("HAGENT_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="HAGENT_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "hagent-ink" / "src"
