...
# Bootstrap Open WebUI against Hagent Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures $HAGENT_HOME/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-hagent.sh
...
#   HAGENT_API_PORT=8642
#   HAGENT_API_HOST=127.0.0.1
#   HAGENT_API_MODEL_NAME='Hagent Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Hagent Agent WebUI}"
...
DEFAULT_HAGENT_HOME="$(cd "$SCRIPT_DIR/../backend" && pwd)"
HAGENT_HOME="${HAGENT_HOME:-$DEFAULT_HAGENT_HOME}"
HAGENT_ENV_FILE="${HAGENT_ENV_FILE:-$HAGENT_HOME/.env}"
HAGENT_API_PORT="${HAGENT_API_PORT:-8642}"
HAGENT_API_HOST="${HAGENT_API_HOST:-127.0.0.1}"
HAGENT_API_CONNECT_HOST="${HAGENT_API_CONNECT_HOST:-127.0.0.1}"
HAGENT_API_MODEL_NAME="${HAGENT_API_MODEL_NAME:-Hagent Agent}"
HAGENT_API_BASE_URL="http://${HAGENT_API_CONNECT_HOST}:${HAGENT_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-hagent.sh"
LOG_DIR="$HAGENT_HOME/logs"
...
ensure_env_permissions() {
  chmod 600 "$HAGENT_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.hagent.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.hagent</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.hagent"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.hagent"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-hagent.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Hagent Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-hagent.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
...
EOF
  ...
  systemctl --user enable --now openwebui-hagent.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$HAGENT_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Hagent API server is configured...'
  upsert_env API_SERVER_ENABLED true "$HAGENT_ENV_FILE"
  upsert_env API_SERVER_HOST "$HAGENT_API_HOST" "$HAGENT_ENV_FILE"
  upsert_env API_SERVER_PORT "$HAGENT_API_PORT" "$HAGENT_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$HAGENT_API_MODEL_NAME" "$HAGENT_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$HAGENT_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Hagent gateway so API server settings take effect...'
  hagent gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HAGENT_API_CONNECT_HOST}:${HAGENT_API_PORT}/health" >/dev/null; then
    log 'Hagent API server did not answer on the first check. Trying to start gateway in the background...'
    nohup hagent gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${HAGENT_API_CONNECT_HOST}:${HAGENT_API_PORT}/health" >/dev/null
  ...
  log "Hagent API endpoint: ${HAGENT_API_BASE_URL}"
