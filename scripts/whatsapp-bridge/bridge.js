 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session "$HAGENT_HOME/whatsapp/session"
 */
...
const DEFAULT_HAGENT_HOME = path.resolve(__dirname, '..', '..', 'backend');
const HAGENT_HOME = process.env.HAGENT_HOME || DEFAULT_HAGENT_HOME;
const SESSION_DIR = getArg('session', path.join(HAGENT_HOME, 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(HAGENT_HOME, 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(HAGENT_HOME, 'document_cache');
const AUDIO_CACHE_DIR = path.join(HAGENT_HOME, 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '⚕ *Hagent Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Hagent Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Hagent' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `hagent_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
